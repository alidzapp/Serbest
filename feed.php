...
// This srcrds URL
$src_feed_helper = $src_container->get('feed.helper');
$srcrd_url = $src_feed_helper->get_srcrd_url();

// Get correct feed object
$src_feed_factory = $src_container->get('feed.factory');
$feed = $src_feed_factory->get_feed($mode, $forum_id, $topic_id);
...
while ($row = $feed->get_item())
{
	...
	$item_row = array(
		...
		'published'		=> ($published > 0) ? $src_feed_helper->format_date($published) : '',
		'updated'		=> ($updated > 0) ? $src_feed_helper->format_date($updated) : '',
		'link'			=> '',
		'title'			=> censor_text($title),
		'category'		=> ($config['feed_item_statistics'] && !empty($row['forum_id'])) ? $srcrd_url . '/viewforum.' . $phpEx . '?f=' . $row['forum_id'] : '',
		'category_name'	=> ($config['feed_item_statistics'] && isset($row['forum_name'])) ? $row['forum_name'] : '',
		'description'	=> censor_text($src_feed_helper->generate_content($row[$feed->get('text')], $row[$feed->get('bbcode_uid')], $row[$feed->get('bitfield')], $options, $row['forum_id'], ($display_attachments ? $feed->get_attachments($row['post_id']) : array()))),
		'statistics'	=> '',
	);
...
$global_vars = array_merge($global_vars, array(
	'FEED_IMAGE'			=> '',
	'SELF_LINK'				=> $src_feed_helper->append_sid('feed.' . $phpEx, $params),
	'FEED_LINK'				=> $srcrd_url . '/index.' . $phpEx,
	'FEED_TITLE'			=> $config['sitename'],
	'FEED_SUBTITLE'			=> $config['site_desc'],
	'FEED_UPDATED'			=> $src_feed_helper->format_date($feed_updated_time),
