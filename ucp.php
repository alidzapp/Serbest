...
require($src_root_path . 'common.' . $phpEx);
require($src_root_path . 'includes/functions_user.' . $phpEx);
require($src_root_path . 'includes/functions_module.' . $phpEx);
...
switch ($mode)
{
	...
	case 'logout':
		...
		{
			meta_refresh(3, append_sid("{$src_root_path}index.$phpEx"));

			$message = $user->lang['LOGOUT_FAILED'] . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("{$src_root_path}index.$phpEx") . '">', '</a> ');
	...
	case 'privacy':
		...
		$template->assign_vars(array(
			...
			'AGREEMENT_TEXT'		=> sprintf($user->lang[$message], $config['sitename'], generate_srcrd_url()),
			'U_BACK'				=> append_sid("{$src_root_path}ucp.$phpEx", 'mode=login'),
	...
	case 'delete_cookies':
		...
		if (confirm_box(true))
		{
			...
			foreach ($request->variable_names(\src\request\request_interface::COOKIE) as $cookie_name)
			{
				$cookie_data = $request->variable($cookie_name, '', true, \src\request\request_interface::COOKIE);

				// Only delete srcrd cookies, no other ones...
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.ucp_delete_cookies', compact($vars)));
				if ($retain_cookie)
				{
					continue;
				}
			...
			$message = $user->lang['COOKIES_DELETED'] . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("{$src_root_path}index.$phpEx") . '">', '</a>');
			trigger_error($message);
		}
		else
		{
	...
	case 'switch_perm':
		...
		include($src_root_path . 'includes/acp/auth.' . $phpEx);
		...
		$message = sprintf($user->lang['PERMISSIONS_TRANSFERRED'], $user_row['username']) . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("{$src_root_path}index.$phpEx") . '">', '</a>');
		trigger_error($message);
	...
	case 'restore_perm':
		...
		$message = $user->lang['PERMISSIONS_RESTORED'] . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("{$src_root_path}index.$phpEx") . '">', '</a>');
		trigger_error($message);
...
if (!$user->data['is_registered'])
{
	...
	{
		$redirect_url = append_sid("{$src_root_path}ucp.$phpEx?i=pm&p=" . request_var('p', 0));
...
extract($src_dispatcher->trigger_event('core.ucp_display_module_before', compact($vars)));
...
$module->assign_tpl_vars(append_sid("{$src_root_path}ucp.$phpEx"));
