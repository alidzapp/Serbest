...
if ($post_id)
{
	$redirect_url = append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;p=$post_id") . "#p$post_id";
	$return_forum_url = append_sid("{$src_root_path}viewforum.$phpEx", "f=$forum_id");
	$pm_id = 0;
}
else
{
	$redirect_url = append_sid("{$src_root_path}ucp.$phpEx", "i=pm&mode=view&p=$pm_id");
	$return_forum_url = '';
	$post_id = 0;
	$forum_id = 0;
}
...
if ($post_id)
{
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.report_post_auth', compact($vars)));
...
if ($submit && $reason_id)
{
	...
	if (!sizeof($error))
	{
		...
		if ($post_id)
		{
			...
			$src_notifications->add_notifications('notification.type.report_post', array_merge($report_data, $row, $forum_data, array(
		...
		else
		{
			...
			$src_notifications->add_notifications('notification.type.report_pm', array_merge($report_data, $row, array(
				'report_text'	=> $report_text,
...
$template->assign_vars(array(
	...
	'REPORT_TEXT'		=> $report_text,
	'S_REPORT_ACTION'	=> append_sid("{$src_root_path}report.$phpEx", 'f=' . $forum_id . '&amp;p=' . $post_id . '&amp;pm=' . $pm_id),
