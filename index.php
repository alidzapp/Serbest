...
if (($mark_notification = $request->variable('mark_notification', 0)))
{
	...
	if (check_link_hash($request->variable('hash', ''), 'mark_notification_read'))
	{
		...
		$notification = $src_notifications->load_notifications(array(
		...
		if (isset($notification['notifications'][$mark_notification]))
		{
			...
			{
				redirect(append_sid($src_root_path . $redirect));
			}
...
while ($row = $db->sql_fetchrow($result))
{
	...
	{
		$legend[] = '<a' . $colour_text . ' href="' . append_sid("{$src_root_path}memberlist.$phpEx", 'mode=group&amp;g=' . $row['group_id']) . '">' . $group_name . '</a>';
	}
}
...
if ($config['load_birthdays'] && $config['allow_birthdays'] && $auth->acl_gets('u_viewprofile', 'a_user', 'a_useradd', 'a_userdel'))
{
	...
	$now = src_gmgetdate($time->getTimestamp() + $time->getOffset());
...
$template->assign_vars(array(
	...
	'S_LOGIN_ACTION'			=> append_sid("{$src_root_path}ucp.$phpEx", 'mode=login'),
	...
	'U_MCP'				=> ($auth->acl_get('m_') || $auth->acl_getf_global('m_')) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=main&amp;mode=front', true, $user->session_id) : '')
);

$page_title = ($config['srcrd_index_text'] !== '') ? $config['srcrd_index_text'] : $user->lang['INDEX'];
...
extract($src_dispatcher->trigger_event('core.index_modify_page_title', compact($vars)));
