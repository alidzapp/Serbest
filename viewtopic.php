...
include($src_root_path . 'includes/bbcode.' . $phpEx);
		...
		else
		{
			$sql = 'SELECT topic_id, forum_id
				...
				WHERE forum_id = ' . $row['forum_id'] . "
					...
					AND " . $src_content_visibility->get_visibility_sql('topic', $row['forum_id']) . "
...
if (!$topic_data)
{
	...
	if ($post_id && $topic_id)
	{
		redirect(append_sid("{$src_root_path}viewtopic.$phpEx", "t=$topic_id" . (($forum_id) ? "&amp;f=$forum_id" : '')));
	}
...
if ($post_id)
{
	...
	if ($post_id == $topic_data['topic_first_post_id'] || $post_id == $topic_data['topic_last_post_id'])
	{
		...
		{
			$topic_data['prev_posts'] = $src_content_visibility->get_count('topic_posts', $topic_data, $forum_id) - 1;
		}
		else
		{
	...
	else
	{
		$sql = 'SELECT COUNT(p.post_id) AS prev_posts
			...
			WHERE p.topic_id = {$topic_data['topic_id']}
				AND " . $src_content_visibility->get_visibility_sql('post', $forum_id, 'p.');
...
$topic_replies = $src_content_visibility->get_count('topic_posts', $topic_data, $forum_id) - 1;
...
/**
* Event to apply extra permissions and to override original src's f_read permission and forum password check
* on viewtopic access
*
...
extract($src_dispatcher->trigger_event('core.viewtopic_before_f_read_check', compact($vars)));
...
foreach($quickmod_array as $option => $qm_ary)
{
	if (!empty($qm_ary[1]))
	{
		src_add_quickmod_option($s_quickmod_action, $option, $qm_ary[0]);
	}
}
...
$server_path = (!$view) ? $src_root_path : generate_srcrd_url() . '/';
...
$base_url = append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id" . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : '') . (($highlight_match) ? "&amp;hilit=$highlight" : ''));
...
extract($src_dispatcher->trigger_event('core.viewtopic_assign_template_vars_before', compact($vars)));
...
$template->assign_vars(array(
	...
	'U_MCP' 		=> ($auth->acl_get('m_', $forum_id)) ? append_sid("{$src_root_path}mcp.$phpEx", "i=main&amp;mode=topic_view&amp;f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start") . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : ''), true, $user->session_id) : '',
	...
	'S_TOPIC_ACTION' 		=> append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start")),
	...
	'S_SEARCHBOX_ACTION'	=> append_sid("{$src_root_path}search.$phpEx"),
	'S_SEARCH_LOCAL_HIDDEN_FIELDS'	=> build_hidden_fields($s_search_hidden_fields),

	'S_DISPLAY_POST_INFO'	=> ($topic_data['forum_type'] == FORUM_POST && ($auth->acl_get('f_post', $forum_id) || $user->data['user_id'] == ANONYMOUS)) ? true : false,
	'S_DISPLAY_REPLY_INFO'	=> ($topic_data['forum_type'] == FORUM_POST && ($auth->acl_get('f_reply', $forum_id) || $user->data['user_id'] == ANONYMOUS)) ? true : false,
	'S_ENABLE_FEEDS_TOPIC'	=> ($config['feed_topic'] && !src_optionget(FORUM_OPTION_FEED_EXCLUDE, $topic_data['forum_options'])) ? true : false,

	'U_TOPIC'				=> "{$server_path}viewtopic.$phpEx?f=$forum_id&amp;t=$topic_id",
	'U_FORUM'				=> $server_path,
	'U_VIEW_TOPIC' 			=> $viewtopic_url,
	'U_CANONICAL'			=> generate_srcrd_url() . '/' . append_sid("viewtopic.$phpEx", "t=$topic_id" . (($start) ? "&amp;start=$start" : ''), true, ''),
	'U_VIEW_FORUM' 			=> append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $forum_id),
	'U_VIEW_OLDER_TOPIC'	=> append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id&amp;view=previous"),
	'U_VIEW_NEWER_TOPIC'	=> append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id&amp;view=next"),
	'U_PRINT_TOPIC'			=> ($auth->acl_get('f_print', $forum_id)) ? $viewtopic_url . '&amp;view=print' : '',
	'U_EMAIL_TOPIC'			=> ($auth->acl_get('f_email', $forum_id) && $config['email_enable']) ? append_sid("{$src_root_path}memberlist.$phpEx", "mode=email&amp;t=$topic_id") : '',
	...
	'U_POST_NEW_TOPIC' 		=> ($auth->acl_get('f_post', $forum_id) || $user->data['user_id'] == ANONYMOUS) ? append_sid("{$src_root_path}posting.$phpEx", "mode=post&amp;f=$forum_id") : '',
	'U_POST_REPLY_TOPIC' 	=> ($auth->acl_get('f_reply', $forum_id) || $user->data['user_id'] == ANONYMOUS) ? append_sid("{$src_root_path}posting.$phpEx", "mode=reply&amp;f=$forum_id&amp;t=$topic_id") : '',
	'U_BUMP_TOPIC'			=> (bump_topic_allowed($forum_id, $topic_data['topic_bumped'], $topic_data['topic_last_post_time'], $topic_data['topic_poster'], $topic_data['topic_last_poster_id'])) ? append_sid("{$src_root_path}posting.$phpEx", "mode=bump&amp;f=$forum_id&amp;t=$topic_id&amp;hash=" . generate_link_hash("topic_$topic_id")) : '')
);
...
if (!empty($topic_data['poll_start']))
{
	...
	else
	{
		...
		if ($request->is_set($config['cookie_name'] . '_poll_' . $topic_id, \src\request\request_interface::COOKIE))
		{
			$cur_voted_id = explode(',', $request->variable($config['cookie_name'] . '_poll_' . $topic_id, '', true, \src\request\request_interface::COOKIE));
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.viewtopic_modify_poll_data', compact($vars)));
	...
	if ($update && $s_can_vote)
	{
		...
		{
			$redirect_url = append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start"));
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.viewtopic_modify_poll_template_data', compact($vars)));
...
while ($row = $db->sql_fetchrow($result))
{
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.viewtopic_post_rowset_data', compact($vars)));
	...
	if (!isset($user_cache[$poster_id]))
	{
		if ($poster_id == ANONYMOUS)
		{
			$user_cache_data = array(
				...
				'online'			=> false,
				'avatar'			=> ($user->optionget('viewavatars')) ? src_get_user_avatar($row) : '',
				'rank_title'		=> '',
				'rank_image'		=> '',
				'rank_image_src'	=> '',
				'sig'				=> '',
			...
			*/
			...
			extract($src_dispatcher->trigger_event('core.viewtopic_cache_guest_data', compact($vars)));

			$user_cache[$poster_id] = $user_cache_data;

			$user_rank_data = src_get_user_rank($row, false);
		...
		else
		{
			...
			$user_cache_data = array(
				...
				'online'		=> false,
				'jabber'		=> ($config['jab_enable'] && $row['user_jabber'] && $auth->acl_get('u_sendim')) ? append_sid("{$src_root_path}memberlist.$phpEx", "mode=contact&amp;action=jabber&amp;u=$poster_id") : '',
				'search'		=> ($config['load_search'] && $auth->acl_get('u_search')) ? append_sid("{$src_root_path}search.$phpEx", "author_id=$poster_id&amp;sr=posts") : '',
...
extract($src_dispatcher->trigger_event('core.viewtopic_modify_post_data', compact($vars)));
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.viewtopic_modify_post_action_conditions', compact($vars)));
	...
	{
		$u_pm = append_sid("{$src_root_path}ucp.$phpEx", 'i=pm&amp;mode=compose&amp;action=quotepost&amp;p=' . $row['post_id']);
	}
	...
	$post_row = array(
		...
		'U_EDIT'			=> ($edit_allowed) ? append_sid("{$src_root_path}posting.$phpEx", "mode=edit&amp;f=$forum_id&amp;p={$row['post_id']}") : '',
		'U_QUOTE'			=> ($quote_allowed) ? append_sid("{$src_root_path}posting.$phpEx", "mode=quote&amp;f=$forum_id&amp;p={$row['post_id']}") : '',
		'U_INFO'			=> ($auth->acl_get('m_info', $forum_id)) ? append_sid("{$src_root_path}mcp.$phpEx", "i=main&amp;mode=post_details&amp;f=$forum_id&amp;p=" . $row['post_id'], true, $user->session_id) : '',
		'U_DELETE'			=> ($delete_allowed) ? append_sid("{$src_root_path}posting.$phpEx", "mode=delete&amp;f=$forum_id&amp;p={$row['post_id']}") : '',
		...
		'U_APPROVE_ACTION'		=> append_sid("{$src_root_path}mcp.$phpEx", "i=queue&amp;p={$row['post_id']}&amp;f=$forum_id&amp;redirect=" . urlencode(str_replace('&amp;', '&', $viewtopic_url . '&amp;p=' . $row['post_id'] . '#p' . $row['post_id']))),
		'U_REPORT'			=> ($auth->acl_get('f_report', $forum_id)) ? append_sid("{$src_root_path}report.$phpEx", 'f=' . $forum_id . '&amp;p=' . $row['post_id']) : '',
		'U_MCP_REPORT'		=> ($auth->acl_get('m_report', $forum_id)) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=reports&amp;mode=report_details&amp;f=' . $forum_id . '&amp;p=' . $row['post_id'], true, $user->session_id) : '',
		'U_MCP_APPROVE'		=> ($auth->acl_get('m_approve', $forum_id)) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=queue&amp;mode=approve_details&amp;f=' . $forum_id . '&amp;p=' . $row['post_id'], true, $user->session_id) : '',
		'U_MCP_RESTORE'		=> ($auth->acl_get('m_approve', $forum_id)) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=queue&amp;mode=' . (($topic_data['topic_visibility'] != ITEM_DELETED) ? 'deleted_posts' : 'deleted_topics') . '&amp;f=' . $forum_id . '&amp;p=' . $row['post_id'], true, $user->session_id) : '',
		'U_MINI_POST'		=> append_sid("{$src_root_path}viewtopic.$phpEx", 'p=' . $row['post_id']) . '#p' . $row['post_id'],
		'U_NEXT_POST_ID'	=> ($i < $i_total && isset($rowset[$post_list[$i + 1]])) ? $rowset[$post_list[$i + 1]]['post_id'] : '',
		'U_PREV_POST_ID'	=> $prev_post_id,
		'U_NOTES'			=> ($auth->acl_getf_global('m_')) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=notes&amp;mode=user_notes&amp;u=' . $poster_id, true, $user->session_id) : '',
		'U_WARN'			=> ($auth->acl_get('m_warn') && $poster_id != $user->data['user_id'] && $poster_id != ANONYMOUS) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=warn&amp;mode=warn_post&amp;f=' . $forum_id . '&amp;p=' . $row['post_id'], true, $user->session_id) : '',
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.viewtopic_modify_post_row', compact($vars)));
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.viewtopic_post_row_after', compact($vars)));
...
if ($all_marked_read)
{
	...
	{
		$template->assign_vars(array(
			'U_VIEW_UNREAD_POST'	=> append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id&amp;view=unread") . '#unread',
		));
	}
}
...
if ($s_can_vote || $s_quick_reply)
{
	...
	if ($s_quick_reply)
	{
		...
		$template->assign_vars(array(
			'S_QUICK_REPLY'			=> true,
			'U_QR_ACTION'			=> append_sid("{$src_root_path}posting.$phpEx", "mode=reply&amp;f=$forum_id&amp;t=$topic_id"),
...
extract($src_dispatcher->trigger_event('core.viewtopic_modify_page_title', compact($vars)));
...
make_jumpbox(append_sid("{$src_root_path}viewforum.$phpEx"), $forum_id);
