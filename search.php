...
if ($keywords || $author || $author_id || $search_id || $submit)
{
	...
	$m_approve_posts_fid_sql = $src_content_visibility->get_global_visibility_sql('post', $ex_fid_ary, 'p.');
	$m_approve_topics_fid_sql = $src_content_visibility->get_global_visibility_sql('topic', $ex_fid_ary, 't.');
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.search_modify_param_before', compact($vars)));
...
*/
	...
	$hilit = src_clean_search_string(str_replace(array('+', '-', '|', '(', ')', '&quot;'), ' ', $keywords));
	...
	$u_search = append_sid("{$src_root_path}search.$phpEx", $u_sort_param . $u_show_results);
	...
	if ($sql_where)
	{
		if ($show_results == 'posts')
		{
			...
			*/
			...
			extract($src_dispatcher->trigger_event('core.search_get_posts_data', compact($vars)));
		...
		else
		{
			...
			*/
			...
			extract($src_dispatcher->trigger_event('core.search_get_topic_data', compact($vars)));
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.search_modify_rowset', compact($vars)));
		...
		foreach ($rowset as $row)
		{
			...
			if ($show_results == 'topics')
			{
				...
				$u_mcp_queue = ($topic_unapproved || $posts_unapproved) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=queue&amp;mode=' . (($topic_unapproved) ? 'approve_details' : 'unapproved_posts') . "&amp;t=$result_topic_id", true, $user->session_id) : '';
				$u_mcp_queue = (!$u_mcp_queue && $topic_deleted) ? append_sid("{$src_root_path}mcp.$phpEx", "i=queue&amp;mode=deleted_topics&amp;t=$result_topic_id", true, $user->session_id) : $u_mcp_queue;
				...
				$tpl_ary = array(
					...
					'U_MCP_REPORT'			=> append_sid("{$src_root_path}mcp.$phpEx", 'i=reports&amp;mode=reports&amp;t=' . $result_topic_id, true, $user->session_id),
			...
			$tpl_ary = array_merge($tpl_ary, array(
				...
				'U_VIEW_POST'		=> (!empty($row['post_id'])) ? append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=" . $row['topic_id'] . '&amp;p=' . $row['post_id'] . (($u_hilit) ? '&amp;hilit=' . $u_hilit : '')) . '#p' . $row['post_id'] : '',
			));
			...
			*/
			...
			extract($src_dispatcher->trigger_event('core.search_modify_tpl_ary', compact($vars)));
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.search_results_modify_search_title', compact($vars)));
...
$template->assign_vars(array(
	'S_SEARCH_ACTION'		=> append_sid("{$src_root_path}search.$phpEx", false, true, 0), // We force no ?sid= appending by using 0
...
if ($auth->acl_get('a_search'))
{
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		$template->assign_block_vars('recentsearch', array(
			...
			'U_KEYWORDS'	=> append_sid("{$src_root_path}search.$phpEx", 'keywords=' . urlencode(htmlspecialchars_decode($keywords)))
		));
	}
	$db->sql_freeresult($result);
}
