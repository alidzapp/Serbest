...
*/
class log implements \src\log\log_interface
{
	/**
	...
	*/
	public function __construct($db, $user, $auth, $src_dispatcher, $src_root_path, $relative_admin_path, $php_ext, $log_table)
	{
		$this->db = $db;
		$this->user = $user;
		$this->auth = $auth;
		...
		$this->src_admin_path = $this->src_root_path . $relative_admin_path;
		...
		* it will not work for the \src\log\log object we create in common.php
		*/
		...
		*/
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			$log_entry_data = array(
				...
				'viewforum'			=> ($row['forum_id'] && $this->auth->acl_get('f_read', $row['forum_id'])) ? append_sid("{$this->src_root_path}viewforum.{$this->php_ext}", 'f=' . $row['forum_id']) : false,
