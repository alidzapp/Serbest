...
class plupload
{
	...
	* @param \src\user $user
	* @param \src\php\ini $php_ini
	* @param \src\mimetype\guesser $mimetype_guesser
	*/
	public function __construct($src_root_path, \src\config\config $config, \src\request\request_interface $request, \src\user $user, \src\php\ini $php_ini, \src\mimetype\guesser $mimetype_guesser)
	{
	...
	*/
	...
	{
		return $this->request->header('X-src-USING-PLUPLOAD', false);
	}
	...
	*/
	...
	{
		$this->upload_directory = $this->src_root_path . $this->config['upload_path'];
