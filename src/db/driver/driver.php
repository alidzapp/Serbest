...
abstract class driver implements driver_interface
{
	...
	*/
	function __construct()
	{
		...
		$this->sql_layer = substr(get_class($this), strlen('src\db\driver\\'));
		...
		if (!$this->return_on_error)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						$message .= '<br /><br />' . sprintf($user->lang['SQL_ERROR_OCCURRED'], '<a href="mailto:' . htmlspecialchars($config['srcrd_contact']) . '">', '</a>');
					}
					else
					{
	...
	*/
	...
	{
		global $cache, $starttime, $src_root_path, $src_path_helper, $user;
		global $request;
			...
			case 'display':
				...
				echo '<!DOCTYPE html>
					...
					<head>
						...
						<link href="' . htmlspecialchars($src_path_helper->update_web_root_path($src_root_path) . $src_path_helper->get_adm_relative_path()) . 'style/admin.css" rel="stylesheet" type="text/css" media="screen" />
					</head>
					<body id="errorpage">
					<div id="wrap">
						<div id="page-header">
