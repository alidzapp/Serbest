...
*/
class permission implements \src\db\migration\tool\tool_interface
{
	/** @var \src\auth\auth */
	protected $auth;
	...
	*/
	public function __construct(\src\db\driver\driver_interface $db, \src\cache\service $cache, \src\auth\auth $auth, $src_root_path, $php_ext)
	{
		$this->db = $db;
		$this->cache = $cache;
		$this->auth = $auth;
	...
	*/
	public function add($auth_option, $global = true, $copy_from = false)
	{
		...
		{
			include($this->src_root_path . 'includes/acp/auth.' . $this->php_ext);
		}
	...
	*/
	public function role_update($old_role_name, $new_role_name)
	{
		...
		if (!$role_id)
		{
			throw new \src\db\migration\exception('ROLE_NOT_EXIST', $old_role_name);
		}
