...
class release_3_0_8_rc1 extends \src\db\migration\migration
{
	public function effectively_installed()
	{
		return src_version_compare($this->config['version'], '3.0.8-RC1', '>=');
	}

	static public function depends_on()
	{
		return array('\src\db\migration\data\v30x\release_3_0_7_pl1');
	}
	...
	public function update_file_extension_group_names()
	{
		...
		while ($lang_dir = $this->db->sql_fetchfield('lang_dir'))
		{
			...
			// On an already updated srcrd, they can also already be in language/.../acp/attachments.php
			// in the srcrd root.
			$lang_files = array(
				"{$this->src_root_path}install/update/new/language/$lang_dir/acp/attachments.{$this->php_ext}",
				"{$this->src_root_path}language/$lang_dir/install.{$this->php_ext}",
				"{$this->src_root_path}language/$lang_dir/acp/attachments.{$this->php_ext}",
			);
