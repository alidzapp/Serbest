...
class release_3_0_6_rc1 extends \src\db\migration\migration
{
	public function effectively_installed()
	{
		return src_version_compare($this->config['version'], '3.0.6-RC1', '>=');
	}

	static public function depends_on()
	{
		return array('\src\db\migration\data\v30x\release_3_0_5');
	}
	...
	{
		return array(
			array('config.add', array('captcha_plugin', 'src_captcha_nogd')),
			array('if', array(
				($this->config['captcha_gd']),
				array('config.update', array('captcha_plugin', 'src_captcha_gd')),
			)),
	...
	public function add_newly_registered_group()
	{
		...
		// Clear permissions...
		include_once($this->src_root_path . 'includes/acp/auth.' . $this->php_ext);
