...
class profilefield_types extends \src\db\migration\migration
{
	...
	public function update_profile_fields_type()
	{
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			$sql = 'UPDATE ' . $this->table_prefix . "profile_fields
				SET field_type = '" . $this->db->sql_escape($this->convert_src30_field_type($row['field_type'])) . "'
	...
	/**
	* Determine the new field type for a given src 3.0 field type
	*
	*	@param	$field_type	string		Field type in 3.0
	*	@return		string		Field new type which is used since 3.1
	*/
	public function convert_src30_field_type($field_type)
	{
		switch ($field_type)
		{
			case FIELD_INT:
