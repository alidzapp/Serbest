...
class dev extends \src\db\migration\migration
{
	...
	{
		return array(
			'\src\db\migration\data\v310\extensions',
			'\src\db\migration\data\v310\style_update_p2',
			'\src\db\migration\data\v310\timezone_p2',
			'\src\db\migration\data\v310\reported_posts_display',
			'\src\db\migration\data\v310\migrations_table',
		);
	}
	...
	public function update_data()
	{
		return array(
			array('if', array(
				(strpos('src_search_', $this->config['search_type']) !== 0),
				array('config.update', array('search_type', 'src_search_' . $this->config['search_type'])),
			)),
	...
	public function move_customise_modules()
	{
		...
		{
			include($this->src_root_path . 'includes/functions_acp.' . $this->php_ext);
		}
	...
	public function rename_module_basenames()
	{
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			{
				// all the class names start with class name or with src_ for auto loading
				if (strpos($row['module_basename'], $row['module_class'] . '_') !== 0 &&
					strpos($row['module_basename'], 'src_') !== 0)
				{
