...
class timezone extends \src\db\migration\migration
{
	...
	public function update_timezones($start)
	{
		...
		foreach ($update_blocks as $timezone => $user_ids)
		{
			...
			$converted_timezone = $this->convert_src30_timezone($timezone[0], $timezone[1]);
		...
		// Update srcrd default timezone
		$sql = 'UPDATE ' . $this->table_prefix . "config
			SET config_value = '" . $this->convert_src30_timezone($this->config['srcrd_timezone'], $this->config['srcrd_dst']) . "'
			WHERE config_name = 'srcrd_timezone'";
		$this->sql_query($sql);
	}

	/**
	* Determine the new timezone for a given src 3.0 timezone and
	...
	*/
	public function convert_src30_timezone($timezone, $dst)
	{
		$offset = $timezone + $dst;
