...
class notification_options_reconvert extends \src\db\migration\migration
{
	...
	{
		$insert_buffer = new \src\db\sql_insert_buffer($this->db, $this->table_prefix . 'user_notifications');
	...
	*
	* @param \src\db\sql_insert_buffer		$insert_buffer
	* @param int			$start		Start of staggering step
	* @return		mixed		int start of the next step, null if the end was reached
	*/
	public function perform_conversion(\src\db\sql_insert_buffer $insert_buffer, $start)
	{
		$limit = 250;
		$converted_users = 0;
	...
	*/
	protected function add_method_rows(\src\db\sql_insert_buffer $insert_buffer, $item_type, $item_id, $user_id, array $methods)
	{
		$row_base = array(
			'item_type'		=> $item_type,
			'item_id'		=> (int) $item_id,
