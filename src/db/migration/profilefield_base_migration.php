...
abstract class profilefield_base_migration extends container_aware_migration
{
	...
	*/
	...
	public function create_custom_field()
	{
		...
		$insert_buffer = new \src\db\sql_insert_buffer($this->db, PROFILE_LANG_TABLE);

		$sql = 'SELECT lang_id
			FROM ' . LANG_TABLE;
		$result = $this->db->sql_query($sql);
		$lang_name = (strpos($this->profilefield_name, 'src_') === 0) ? strtoupper(substr($this->profilefield_name, 6)) : strtoupper($this->profilefield_name);
	...
	*/
	public function create_language_entries()
	{
		...
		$insert_buffer = new \src\db\sql_insert_buffer($this->db, PROFILE_FIELDS_LANG_TABLE);
