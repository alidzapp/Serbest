...
class tools
{
	...
	*/
	public function __construct(\src\db\driver\driver_interface $db, $return_statements = false)
	{
		$this->db = $db;
	...
	*/
	public function get_existing_indexes($table_name, $column_name, $unique = false)
	{
		...
		switch ($this->sql_layer)
		{
			...
			case 'mssqlnative':
				if ($this->mssql_is_sql_server_2000())
				{
					// http://msdn.microsoft.com/en-us/library/aa175912%28v=sql.80%29.aspx
					// Deprecated in SQL Server 2005
					$sql = "SELECT DISTINCT ix.name AS src_index_name
					FROM sysindexes ix
			...
			case 'oracle':
				$sql = "SELECT ix.index_name  AS src_index_name, ix.uniqueness AS is_unique
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			{
				$existing_indexes[$row['src_index_name']] = array();
			}
		}
		$this->db->sql_freeresult($result);
		...
		switch ($this->sql_layer)
		{
			...
			case 'mssqlnative':
				if ($this->mssql_is_sql_server_2000())
				{
					$sql = "SELECT DISTINCT ix.name AS src_index_name, cols.name AS src_column_name
						FROM sysindexes ix
						INNER JOIN sysindexkeys ixc
							ON ixc.id = ix.id
								AND ixc.indid = ix.indid
			...
			case 'oracle':
				$sql = "SELECT index_name AS src_index_name, column_name AS src_column_name
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			$existing_indexes[$row['src_index_name']][] = $row['src_column_name'];
		}
		$this->db->sql_freeresult($result);
