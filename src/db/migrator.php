...
class migrator
{
	...
	*/
	public function __construct(ContainerInterface $container, \src\config\config $config, \src\db\driver\driver_interface $db, \src\db\tools $db_tools, $migrations_table, $src_root_path, $php_ext, $table_prefix, $tools, \src\db\migration\helper $helper)
	{
	...
	*/
	protected function try_apply($name)
	{
		...
		foreach ($state['migration_depends_on'] as $depend)
		{
			...
			{
				throw new \src\db\migration\exception('MIGRATION_NOT_FULFILLABLE', $name, $depend);
			}
	...
	protected function get_callable_from_step(array $step, $last_result = 0, $reverse = false)
	{
		...
		switch ($class)
		{
			case 'if':
				...
				{
					throw new \src\db\migration\exception('MIGRATION_INVALID_DATA_MISSING_CONDITION', $step);
				}

				if (!isset($parameters[1]))
				{
					throw new \src\db\migration\exception('MIGRATION_INVALID_DATA_MISSING_STEP', $step);
				}
			...
			case 'custom':
				...
				{
					throw new \src\db\migration\exception('MIGRATION_INVALID_DATA_CUSTOM_NOT_CALLABLE', $step);
				}
			...
			default:
				if (!$method)
				{
					throw new \src\db\migration\exception('MIGRATION_INVALID_DATA_UNKNOWN_TYPE', $step);
				}

				if (!isset($this->tools[$class]))
				{
					throw new \src\db\migration\exception('MIGRATION_INVALID_DATA_UNDEFINED_TOOL', $step);
				}

				if (!method_exists(get_class($this->tools[$class]), $method))
				{
					throw new \src\db\migration\exception('MIGRATION_INVALID_DATA_UNDEFINED_METHOD', $step);
				}
	...
	*
	* @param string $name Name of the migration
	* @return \src\db\migration\migration
	*/
	protected function get_migration($name)
	{
		$migration = new $name($this->config, $this->db, $this->db_tools, $this->src_root_path, $this->php_ext, $this->table_prefix);
	...
	*
	* THIS SHOULD NOT GENERALLY BE USED! THIS IS FOR THE src INSTALLER.
