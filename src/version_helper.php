...
class version_helper
{
	 ...
	 * @var string Host
	 */
	protected $host = 'version.src.com';
	 ...
	 * @param \src\file_downloader $file_downloader
	 * @param \src\user $user
	 */
	public function __construct(\src\cache\service $cache, \src\config\config $config, \src\file_downloader $file_downloader, \src\user $user)
	{
		$this->cache = $cache;
		$this->config = $config;
	 ...
	 *
	 * @param string $host Host (e.g. version.src.com)
	 * @param string $path Path to file (e.g. /src)
	...
	{
		return src_version_compare($version1, $version2, $operator);
	}
