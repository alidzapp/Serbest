...
namespace src\auth\provider;

/**
* Apache authentication provider for src3
*/
class apache extends \src\auth\provider\base
{
	/**
	...
	*/
	 ...
	 * @param	\src\passwords\manager	$passwords_manager		Passwords Manager object
	 ...
	 */
	public function __construct(\src\db\driver\driver_interface $db, \src\config\config $config, \src\passwords\manager $passwords_manager, \src\request\request $request, \src\user $user, $src_root_path, $php_ext)
	{
		$this->db = $db;
		$this->config = $config;
	...
	{
		if (!$this->request->is_set('PHP_AUTH_USER', \src\request\request_interface::SERVER) || $this->user->data['username'] !== htmlspecialchars_decode($this->request->server('PHP_AUTH_USER')))
		{
	...
	public function login($username, $password)
	{
		...
		if (!$this->request->is_set('PHP_AUTH_USER', \src\request\request_interface::SERVER))
		{
			return array(
