...
/**
* OAuth storage wrapper for src's cache
*/
class token_storage implements TokenStorageInterface
{
	...
	*/
	...
	/**
	* Creates token storage for src.
	*
	* @param	\src\db\driver\driver_interface	$db
	* @param	\src\user		$user
	* @param	string			$auth_provider_oauth_table
	*/
	public function __construct(\src\db\driver\driver_interface $db, \src\user $user, $auth_provider_oauth_table)
	{
		$this->db = $db;
		$this->user = $user;
