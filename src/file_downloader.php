...
class file_downloader
{
	 ...
	 *
	 * @throws \src\exception\runtime_exception If data can't be retrieved and no error
	 *		message is returned
	 */
	public function get($host, $directory, $filename, $port = 80, $timeout = 6)
	{
		...
		if ($socket = @fsockopen($host, $port, $this->error_number, $this->error_string, $timeout))
		{
			...
			while (!@feof($socket))
			{
				...
				else
				{
					...
					{
						throw new \src\exception\runtime_exception('FILE_NOT_FOUND', array($filename));
					}
				}
				...
				{
					throw new \src\exception\runtime_exception('FSOCK_TIMEOUT');
				}
			}
			@fclose($socket);
		}
		else
		{
			...
			{
				throw new \src\exception\runtime_exception('FSOCK_DISABLED');
			}
		}
