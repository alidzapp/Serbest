...
class content_visibility
{
	...
	*/
	public function __construct(\src\auth\auth $auth, \src\config\config $config, \src\event\dispatcher_interface $src_dispatcher, \src\db\driver\driver_interface $db, \src\user $user, $src_root_path, $php_ext, $forums_table, $posts_table, $topics_table, $users_table)
	{
		$this->auth = $auth;
		$this->config = $config;
	...
	*/
	public function get_forums_visibility_sql($mode, $forum_ids = array(), $table_alias = '')
	{
		...
		*
		* @event core.src_content_visibility_get_forums_visibility_before
		...
		*/
		...
		extract($this->src_dispatcher->trigger_event('core.src_content_visibility_get_forums_visibility_before', compact($vars)));
