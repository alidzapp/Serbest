...
class session
{
	...
	*/
	...
	{
		global $phpEx, $SID, $_SID, $_EXTRA_URL, $db, $config, $src_root_path;
		global $request, $src_container;
		...
		$this->page					= $this->extract_current_page($src_root_path);
		...
		if ($request->is_set($config['cookie_name'] . '_sid', \src\request\request_interface::COOKIE) || $request->is_set($config['cookie_name'] . '_u', \src\request\request_interface::COOKIE))
		{
		...
		foreach ($ips as $ip)
		{
			if (function_exists('src_ip_normalise'))
			{
				// Normalise IP address
				$ip = src_ip_normalise($ip);
...
//				$quadcheck = ($config['ip_check_bot'] && $this->data['user_type'] & USER_BOT) ? 4 : $config['ip_check'];
				...
				if ($u_ip === $s_ip && $s_browser === $u_browser && $s_forwarded_for === $u_forwarded_for && $referer_valid)
				{
					...
					if (!($provider instanceof \src\auth\provider\provider_interface))
					{
						throw new \RuntimeException($provider . ' must implement \src\auth\provider\provider_interface');
					}
					...
					if (!$session_expired)
					{
						...
						// Else check the autologin length... and also removing those having autologin enabled but no longer allowed srcrd-wide.
	...
	*/
	...
	{
		global $SID, $_SID, $db, $config, $cache, $src_root_path, $phpEx, $src_container;
		...
		// Do we allow autologin on this srcrd? No? Then override anything
...
//		$db->sql_return_on_error(false);
		...
		// We are not able to simply have an empty session_page btw, therefore we need to tell src how to detect this special case.
	...
	*/
	...
	{
		global $SID, $_SID, $db, $config, $src_root_path, $phpEx, $src_container;
	...
	*/
	function session_gc()
	{
		global $db, $config, $src_root_path, $phpEx, $src_container;
		...
		if ($del_sessions < $batch_size)
		{
			...
			$captcha_factory = $src_container->get('captcha.factory');
	...
	*
	* @param string $name		Name of the cookie, will be automatically prefixed with the src cookie name. track becomes [cookie_name]_track then.
	...
	*/
	...
	{
		global $config, $db, $src_dispatcher;
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.session_set_custom_ban', compact($vars)));
...
//				$this->session_create(ANONYMOUS);
			...
			// We show a login box here to allow founders accessing the srcrd if banned by IP
			...
			$message = ($ban_row['ban_end']) ? 'srcRD_BAN_TIME' : 'srcRD_BAN_PERM';

			$contact_link = src_get_srcrd_contact_link($config, $src_root_path, $phpEx);
			$message = sprintf($this->lang[$message], $till_date, '<a href="' . $contact_link . '">', '</a>');
			$message .= ($ban_row['ban_give_reason']) ? '<br /><br />' . sprintf($this->lang['srcRD_BAN_REASON'], $ban_row['ban_give_reason']) : '';
	...
	* @return true if uri is on blacklist, else false. Only blacklist is checked (~zero FP), no grey lists
	function check_uribl($uri)
	{
		...
		if ($uri)
		{
			...
			if (src_checkdnsrr($uri . '.multi.uribl.com.', 'A') === true)
			{
				return true;
			}
		}
