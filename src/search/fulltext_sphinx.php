...
class fulltext_sphinx
{
	...
	/**
	 * src event dispatcher object
	 * @var \src\event\dispatcher_interface
	 */
	protected $src_dispatcher;
	...
	/**
	 * Constructor
	 * Creates a new \src\search\fulltext_postgres, which is used as a search backend
	 *
	 ...
	 * @param \src\db\driver\driver_interface Database object
	 * @param \src\user $user User object
	 * @param \src\event\dispatcher_interface	$src_dispatcher	Event dispatcher object
	 */
	public function __construct(&$error, $src_root_path, $phpEx, $auth, $config, $db, $user, $src_dispatcher)
	{
		$this->src_root_path = $src_root_path;
		$this->php_ext = $phpEx;
		$this->config = $config;
		$this->src_dispatcher = $src_dispatcher;
		$this->user = $user;
		$this->db = $db;
		$this->auth = $auth;

		// Initialize \src\db\tools object
		$this->db_tools = new \src\db\tools($this->db);
		...
		$this->indexes = 'index_src_' . $this->id . '_delta;index_src_' . $this->id . '_main';

		if (!class_exists('SphinxClient'))
		{
			require($this->src_root_path . 'includes/sphinxapi.' . $this->php_ext);
		}
	...
	*/
	...
	protected function config_generate()
	{
		...
		include($this->src_root_path . 'config.' . $this->php_ext);
		...
		$config_object = new \src\search\sphinx\config($this->config_file_data);
		$config_data = array(
			'source source_src_' . $this->id . '_main' => array(
			...
			'source source_src_' . $this->id . '_delta : source_src_' . $this->id . '_main' => array(
				array('sql_query_pre',				''),
			...
			'index index_src_' . $this->id . '_main' => array(
				array('path',						$this->config['fulltext_sphinx_data_path'] . 'index_src_' . $this->id . '_main'),
				array('source',						'source_src_' . $this->id . '_main'),
			...
			'index index_src_' . $this->id . '_delta : index_src_' . $this->id . '_main' => array(
				array('path',						$this->config['fulltext_sphinx_data_path'] . 'index_src_' . $this->id . '_delta'),
				array('source',						'source_src_' . $this->id . '_delta'),
			),
			'indexer' => array(
