...
*/
class user extends \src\session
{
	var $lang = array();
	var $help = array();
	var $style = array();
	...
	*/
	function __construct($datetime_class)
	{
		...
		$this->lang_path = $src_root_path . 'language/';
		$this->datetime = $datetime_class;
	}

	/**
	* Function to set custom language path (able to use directory outside of src)
	*
	...
	*/
	...
	{
		global $db, $request, $template, $config, $auth, $phpEx, $src_root_path, $cache;
		...
		else
		{
			...
			{
				$lang_override = $request->variable($config['cookie_name'] . '_lang', '', true, \src\request\request_interface::COOKIE);
			}
			if ($lang_override)
			{
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.user_setup', compact($vars)));
		...
		// Call src_user_session_handler() in case external application want to "bend" some variables or replace classes...
		// After calling it we continue script execution...
		src_user_session_handler();
		...
		// Disable srcrd if the install/ directory is still present
		// For the brave development army we do not care about this, else we need to comment out this everytime we develop locally
		if (!defined('DEBUG') && !defined('ADMIN_START') && !defined('IN_INSTALL') && !defined('IN_LOGIN') && file_exists($src_root_path . 'install') && !is_file($src_root_path . 'install'))
		{
			...
			{
				$message = (!empty($config['srcrd_disable_msg'])) ? $config['srcrd_disable_msg'] : 'srcRD_DISABLE';
			}
			trigger_error($message);
		}

		// Is srcrd disabled and user not an admin or moderator?
		if ($config['srcrd_disable'] && !defined('IN_LOGIN') && !defined('SKIP_CHECK_DISABLED') && !$auth->acl_gets('a_', 'm_') && !$auth->acl_getf_global('m_'))
		{
			if ($this->data['is_bot'])
			{
		...
		if ($config['limit_load'] && $this->load !== false)
		{
			...
			{
				// Set srcrd disabled to true to let the admins/mods get the proper notification
				$config['srcrd_disable'] = '1';
				...
				if (!$auth->acl_gets('a_', 'm_') && !$auth->acl_getf_global('m_'))
				{
					...
					trigger_error('srcRD_UNAVAILABLE');
				}
			}
		}
		...
		if (!defined('IN_ADMIN') && !defined('ADMIN_START') && $config['chg_passforce'] && !empty($this->data['is_registered']) && $auth->acl_get('u_chgpasswd') && $this->data['user_passchg'] < time() - ($config['chg_passforce'] * 86400))
		{
			...
			{
				redirect(append_sid("{$src_root_path}ucp.$phpEx", 'i=profile&amp;mode=reg_details'));
			}
		}
	...
	* Function to mimic sprintf() with the possibility of using src's language system to substitute nullar/singular/plural forms.
	...
	*/
	function get_plural_form($number, $force_rule = false)
	{
		...
		return src_get_plural_form($plural_rule, $number);
	}
	...
	*/
	function set_lang(&$lang, &$help, $lang_file, $use_db = false, $use_help = false, $ext_name = '')
	{
		...
		if (!$use_db)
		{
			...
			if ($ext_name)
			{
				global $src_extension_manager;
				$ext_path = $src_extension_manager->get_extension_path($ext_name, true);
			...
			if (strpos($src_root_path . $filename, $lang_path) === 0)
			{
				$language_filename = $src_root_path . $filename;
			}
			else
			{
			...
			if (!file_exists($language_filename))
			{
				...
				{
					// The user's selected language is missing the file, the srcrd default's language is missing the file, and the file doesn't exist in /en.
				...
				{
					// Fall back to the srcrd default language
					$reset_lang_name = $this->lang_name;
					$this->lang_name = basename($config['default_lang']);
					$this->set_lang($lang, $help, $lang_file, $use_db, $use_help, $ext_name);
					$this->lang_name = $reset_lang_name;
	...
	*/
	...
	/**
	* Create a \src\datetime object in the context of the current user
	*
	* @since 3.1
	* @param string $time String in a format accepted by strtotime().
	* @param DateTimeZone $timezone Time zone of the time.
	* @return \src\datetime Date time object linked to the current users locale
	*/
	...
	*/
	function optionget($key, $data = false)
	{
		...
		return src_optionget($this->keyoptions[$key], $var);
	}
