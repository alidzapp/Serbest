...
class container_builder
{
	...
	*/
	...
	/**
	* Defines a custom path to find the configuration of the container (default to $this->src_root_path . 'config')
	*
	* @var string
	*/
	...
	/**
	* Indicates whether the src compile pass should be used (default to true).
	*
	* @var bool
	*/
	...
	* Default to true:
	* 	array(
	* 		'core.root_path', $this->src_root_path,
	...
	* Constructor
	*
	* @param \src\config_php_file $config_php_file
	* @param string $src_root_path Path to the src includes directory.
	* @param string $php_ext php file extension
	*/
	function __construct(\src\config_php_file $config_php_file, $src_root_path, $php_ext)
	{
	...
	*
	* @return \src_cache_container|ContainerBuilder
	*/
	public function get_container()
	{
		...
		if (!defined('DEBUG_CONTAINER') && $this->dump_container && file_exists($container_filename))
		{
			...
			$this->container = new \src_cache_container();
		}
		else
		{
			if ($this->config_path === null)
			{
				$this->config_path = $this->src_root_path . 'config';
			}
			$container_extensions = array(new \src\di\extension\core($this->config_path));

			if ($this->use_extensions)
			{
				$installed_exts = $this->get_installed_extensions();
				$container_extensions[] = new \src\di\extension\ext($installed_exts);
			}

			if ($this->inject_config)
			{
				$container_extensions[] = new \src\di\extension\config($this->config_php_file);
			}
			...
			if ($this->use_custom_pass)
			{
				...
				$this->container->addCompilerPass(new \src\di\pass\collection_pass());
	...
	*/
	...
	/**
	* Set if the src compile pass have to be used.
	*
	...
	protected function get_dbal_connection()
	{
		if ($this->dbal_connection === null)
		{
			...
			$this->dbal_connection->sql_connect(
				...
				defined('src_DB_NEW_LINK') && src_DB_NEW_LINK
			);
		}
	...
	*/
	protected function get_installed_extensions()
	{
		...
		foreach ($rows as $row)
		{
			$exts[$row['ext_name']] = $this->src_root_path . 'ext/' . $row['ext_name'] . '/';
		}
	...
	*/
	...
	{
		$filename = str_replace(array('/', '.'), array('slash', 'dot'), $this->src_root_path);
		return $this->src_root_path . 'cache/container_' . $filename . '.' . $this->php_ext;
	}
}
