...
*/
class file extends \src\cache\driver\base
{
	...
	* Set cache path
	*
	* @param string $cache_dir Define the path to the cache directory (default: $src_root_path . 'cache/')
	*/
	function __construct($cache_dir = null)
	{
		global $src_root_path;
		$this->cache_dir = !is_null($cache_dir) ? $cache_dir : $src_root_path . 'cache/';
	}
	...
	*/
	function _write($filename, $data = null, $expires = 0, $query = '')
	{
		...
		if ($handle = @fopen($file, 'wb'))
		{
			...
			if (!function_exists('src_chmod'))
			{
				global $src_root_path;
				include($src_root_path . 'includes/functions.' . $phpEx);
			}

			src_chmod($file, CHMOD_READ | CHMOD_WRITE);
