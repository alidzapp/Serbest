...
class topic_form extends form
{
	...
	*/
	public function submit(\messenger $messenger)
	{
		...
		$this->message->set_template_vars(array(
			...
			'U_TOPIC'		=> generate_srcrd_url() . '/viewtopic.' . $this->phpEx . '?f=' . $this->topic_row['forum_id'] . '&t=' . $this->topic_id,
		));
	...
	*/
	...
	{
		return sprintf($this->user->lang['RETURN_TOPIC'],  '<a href="' . append_sid($this->src_root_path . 'viewtopic.' . $this->phpEx, 'f=' . $this->topic_row['forum_id'] . '&amp;t=' . $this->topic_id) . '">', '</a>');
	}
	...
	*/
	public function render(\src\template\template $template)
	{
		...
		$template->assign_vars(array(
			...
			'S_POST_ACTION'			=> append_sid($this->src_root_path . 'memberlist.' . $this->phpEx, 'mode=email&amp;t=' . $this->topic_id))
		);
	}
}
