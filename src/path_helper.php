...
class path_helper
{
	...
	* Constructor
	*
	* @param \src\symfony_request $symfony_request
	...
	*/
	public function __construct(\src\symfony_request $symfony_request, \src\filesystem $filesystem, \src\request\request_interface $request, $src_root_path, $php_ext, $adm_relative_path = null)
	{
	...
	*
	* This replaces $src_root_path . some_url with
	...
	*/
	...
	{
		if (strpos($path, $this->src_root_path) === 0)
		{
			$path = substr($path, strlen($this->src_root_path));
	...
	*/
	public function remove_web_root_path($path)
	{
		if (strpos($path, $this->get_web_root_path()) === 0)
		{
			...
			return $this->src_root_path . $path;
		}
	...
	public function get_web_root_path()
	{
		...
		*/
		...
		$this->web_root_path = $this->filesystem->clean_path(
			'./' . str_repeat('../', $corrections) . $this->src_root_path
		);
		return $this->web_root_path;
	}
	...
	* @param string $absolute_srcrd_url
	* @return string
	*/
	public function get_web_root_path_from_ajax_referer($absolute_referer_url, $absolute_srcrd_url)
	{
		// If the srcrd URL is in the beginning of the referer, this means
		// we the referer is in the srcrd URL or a subdirectory of it.
		...
		if (strpos($absolute_referer_url, $absolute_srcrd_url) === 0)
		{
			$relative_referer_path = substr($absolute_referer_url, strlen($absolute_srcrd_url));
			...
			return $this->src_root_path . str_repeat('../', $corrections - 1);
		}

		// If not, it's a bit more complicated. We go to the parent directory
		// of the referer until we find the remaining referer in the srcrd URL.
		...
		// from the srcrd URL, to get the srcrds root path.
		...
		while (($dir_position = strpos($absolute_srcrd_url, $referer_dir)) !== 0)
		{
			$fixed_root_path .= '../';
			$referer_dir = dirname($referer_dir);

			// Just return src_root_path if we reach the top directory
			if ($referer_dir === '.')
			{
				return $this->src_root_path;
			}
		...
		$fixed_root_path .= substr($absolute_srcrd_url, strlen($referer_dir) + 1);
		// Add trailing slash
		return $this->src_root_path . $fixed_root_path . '/';
	}
	...
	*/
	...
	public function get_valid_page($page, $mod_rewrite = false)
	{
		...
		// For a relative path, let's prefix it with $src_root_path to point to the correct location,
		...
		if ($url_parts === false || empty($url_parts['scheme']) || empty($url_parts['host']))
		{
			...
			// even if it might not be in the src root.
			...
			$page = $this->get_src_root_path() . ltrim($page, '/\\');
		}
