...
function swap_options(pmask, fmask, cat, adv, view) {
	...
	if (adv_block.style.display === 'block' && adv === true) {
		src.toggleDisplay('advanced' + pmask + fmask, -1);
		reset_opacity(1);
	...
	if (!view) {
		src.toggleDisplay('advanced' + active_pmask + active_fmask, -1);
	}

	if (!view) {
		src.toggleDisplay('advanced' + pmask + fmask, 1);
	}
	src.toggleDisplay('options' + id, 1);
