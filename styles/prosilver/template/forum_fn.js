...
function parseDocument($container) {
	...
	*/
	$container.find('.linklist:not(.navlinks, [data-skip-responsive]), .postbody .post-buttons:not([data-skip-responsive])').each(function() {
		...
		if (!persistent) {
			src.registerDropdown($menu.find('a.responsive-menu-link'), $menu.find('.dropdown'), false);
		}
	...
	*/
	$container.find('#tabs, #minitabs').not('[data-skip-responsive]').each(function() {
		...
		function check() {
			...
			if (height <= maxHeight) {
				if ($item.hasClass('dropdown-visible')) {
					src.toggleDropdown.call($item.find('a.responsive-tab-link').get(0));
				}
				return;
			}
		...
		src.registerDropdown($item.find('a.responsive-tab-link'), $item.find('.dropdown'), {visibleClass: 'activetab'});
...
jQuery(function($) {
	...
	// Swap .nojs and .hasjs
	$('#src.nojs').toggleClass('nojs hasjs');
	$('#src').toggleClass('hastouch', src.isTouch);
	$('#src.hastouch').removeClass('notouch');
