...
function move_topics($topic_ids, $forum_id, $auto_sync = true)
{
	...
	extract($src_dispatcher->trigger_event('core.move_topics_before_query', compact($vars)));
...
{
	global $db, $config, $src_container, $src_dispatcher;
	...
	extract($src_dispatcher->trigger_event('core.delete_topics_before_query', compact($vars)));
	...
	extract($src_dispatcher->trigger_event('core.delete_topics_after_query', compact($vars)));
	...
	$src_notifications->delete_notifications(array(
...
{
	global $db, $config, $src_root_path, $phpEx, $auth, $user, $src_container, $src_dispatcher;
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.delete_posts_before', compact($vars)));
...
/*		Possible code for splitting post deletion
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.delete_posts_in_transaction', compact($vars)));
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.delete_posts_after', compact($vars)));
	...
	$src_notifications->delete_notifications($delete_notifications_types, $post_ids);
...
function delete_attachments($mode, $ids, $resync = true)
{
	...
	{
		if (src_unlink($file_ary['filename'], 'file', true) && !$file_ary['is_orphan'])
		{
		...
		{
			src_unlink($file_ary['filename'], 'thumbnail', true);
		}
	}
...
*/
function src_unlink($filename, $mode = 'file', $entry_removed = false)
{
	global $db, $src_root_path, $config;
	...
	return @unlink($src_root_path . $config['upload_path'] . '/' . $filename);
}
...
function prune($forum_id, $prune_mode, $prune_date, $prune_flags = 0, $auto_sync = true)
{
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.prune_sql', compact($vars)));
...
* @param \src\cache\driver\driver_interface Cache driver
* @param \src\auth\auth $auth Authentication object
* @return null
*/
function src_cache_moderators($db, $cache, $auth)
{
...
function view_log($mode, &$log, &$log_count, $limit = 0, $offset = 0, $forum_id = 0, $topic_id = 0, $user_id = 0, $limit_days = 0, $sort_by = 'l.log_time DESC', $keywords = '')
{
	...
	$log = $src_log->get_logs($mode, $count_logs, $limit, $offset, $forum_id, $topic_id, $user_id, $limit_days, $sort_by, $keywords);
	$log_count = $src_log->get_log_count();

	return $src_log->get_valid_offset();
}
...
* @return null
*/
function src_update_foes($db, $auth, $group_id = false, $user_id = false)
{
...
function get_remote_file($host, $directory, $filename, &$errstr, &$errno, $port = 80, $timeout = 6)
{
	...
	$file_downloader = $src_container->get('file_downloader');
...
{
	global $user, $phpEx, $src_extension_manager;
