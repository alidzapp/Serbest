...
function send_avatar_to_browser($file, $browser)
{
	...
	$file_path = $src_root_path . $image_dir . '/' . $prefix . $file;
	...
	if ((@file_exists($file_path) && @is_readable($file_path)) && !headers_sent())
	{
		...
		if ((strpos(strtolower($browser), 'msie') !== false) && !src_is_greater_ie_version($browser, 7))
		{
...
{
	global $user, $db, $config, $src_root_path;

	$filename = $src_root_path . $upload_dir . '/' . $attachment['physical_filename'];
	...
	if (src_is_greater_ie_version($user->browser, 7))
	{
	...
	{
		if (empty($user->browser) || ((strpos(strtolower($user->browser), 'msie') !== false) && !src_is_greater_ie_version($user->browser, 7)))
		{
		...
		else
		{
			...
			if (src_is_greater_ie_version($user->browser, 7) && (strpos($attachment['mimetype'], 'image') !== 0))
			{
	...
	if (!set_modified_headers($attachment['filetime'], $user->browser))
	{
		...
		if (defined('src_ENABLE_X_ACCEL_REDIRECT') && src_ENABLE_X_ACCEL_REDIRECT)
		{
		...
		else if (defined('src_ENABLE_X_SENDFILE') && src_ENABLE_X_SENDFILE && !src_http_byte_range($size))
		{
		...
		if ($fp !== false)
		{
			...
			if ($range = src_http_byte_range($size))
			{
...
*/
function src_http_byte_range($filesize)
{
	...
	{
		$request_array = src_find_range_request();
	}

	return (empty($request_array)) ? false : src_parse_range_request($request_array, $filesize);
}
...
*/
function src_find_range_request()
{
	global $request;
...
*/
function src_parse_range_request($request_array, $filesize)
{
...
*/
function src_is_greater_ie_version($user_agent, $version)
{
