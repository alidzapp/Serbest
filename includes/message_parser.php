...
if (!class_exists('bbcode'))
{
	...
	// while $src_root_path and $phpEx are out of the script scope
	// which may lead to the 'Undefined variable' and 'failed to open stream' errors
	if (!isset($src_root_path))
	{
	...
	*/
	function bbcode_init($allow_custom_bbcode = true)
	{
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.modify_bbcode_init', compact($vars)));
	...
	*/
	function validate_url($var1, $var2)
	{
		...
		if (preg_match('#^' . get_preg_expression('url') . '$#iu', $url) ||
			...
			preg_match('#^' . preg_quote(generate_srcrd_url(), '#') . get_preg_expression('relative_url') . '$#iu', $url))
		{
			$valid = true;
		}
		...
		if ($valid)
		{
			...
			// Is this a link to somewhere inside this srcrd? If so then remove the session id from the url
			if (strpos($url, generate_srcrd_url()) !== false && strpos($url, 'sid=') !== false)
			{
...
class parse_message extends bbcode_firstpass
{
	...
	*/
	...
	{
		global $config, $db, $user, $src_dispatcher;
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.message_parser_check_message', compact($vars)));
		$this->message = $message;
		...
		if ($allow_magic_url)
		{
			$this->magic_url(generate_srcrd_url());
	...
	function smilies($max_smilies = 0)
	{
		...
		if (sizeof($match))
		{
			if ($max_smilies)
			{
				...
				// For details: http://tracker.src.com/browse/src3-10117
	...
	*/
	...
	{
		global $config, $auth, $user, $src_root_path, $phpEx, $db, $request;
			...
			else if (($add_file || $preview) && $upload_file)
			{
				if ($num_attachments < $cfg['max_attachments'] || $auth->acl_gets('m_', 'a_', $forum_id))
				{
					...
					if (!sizeof($error))
					{
						...
						if (isset($this->plupload) && $this->plupload->is_active())
						{
							$download_url = append_sid("{$src_root_path}download/file.{$phpEx}", 'mode=view&amp;id=' . $new_entry['attach_id']);
	...
	*/
	...
	{
		global $user, $db, $src_root_path, $phpEx, $config;
		global $request;

		$this->filename_data['filecomment'] = utf8_normalize_nfc(request_var('filecomment', '', true));
		$attachment_data = $request->variable('attachment_data', array(0 => array('' => '')), true, \src\request\request_interface::POST);
	...
	* @return null
	*/
	public function set_plupload(\src\plupload\plupload $plupload)
	{
	...
	*
	* @param \src\mimetype\guesser $mimetype_guesser The mimetype_guesser object
	*
	* @return null
	*/
	public function set_mimetype_guesser(\src\mimetype\guesser $mimetype_guesser)
	{
	...
	*/
	public function validate_bbcode_by_extension()
	{
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.validate_bbcode_by_extension', compact($vars)));
