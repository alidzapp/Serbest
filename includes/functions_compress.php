...
class compress
{
	...
	*/
	function add_file($src, $src_rm_prefix = '', $src_add_prefix = '', $skip_files = '')
	{
		...
		if (is_file($src_root_path . $src))
		{
			$this->data($src_path, file_get_contents("$src_root_path$src"), stat("$src_root_path$src"), false);
		}
		else if (is_dir($src_root_path . $src))
		{
			// Clean up path, add closing / if not present
			$src_path = ($src_path && substr($src_path, -1) != '/') ? $src_path . '/' : $src_path;

			$filelist = array();
			$filelist = filelist("$src_root_path$src", '', '*');
			krsort($filelist);
			...
			* Commented out, as adding the folders produces corrupted archives
			if ($src_path)
			{
				$this->data($src_path, '', true, stat("$src_root_path$src"));
			}
			*/
			...
			foreach ($filelist as $path => $file_ary)
			{
				...
				* Commented out, as adding the folders produces corrupted archives
				if ($path)
				{
					...
					$this->data("$src_path$path", '', true, stat("$src_root_path$src$path"));
				}
				*/
				...
				foreach ($file_ary as $file)
				{
					...
					$this->data("$src_path$path$file", file_get_contents("$src_root_path$src$path$file"), stat("$src_root_path$src$path$file"), false);
				}
			}
		}
		else
...
*
* Modified extensively by psoTFX and DavidMJ, (c) src Limited, 2003
*
...
class compress_zip extends compress
{
						...
						if (!is_dir($target_filename))
						{
							...
							foreach ($folders as $folder)
							{
								...
								if (!is_dir($str))
								{
									...
									src_chmod($str, CHMOD_READ | CHMOD_WRITE);
								}
							}
						}
	...
	*/
	function download($filename, $download_name = false)
	{
		...
		$fp = @fopen("{$src_root_path}store/$filename.zip", 'rb');
		if ($fp)
		{
...
class compress_tar extends compress
{
		...
		while ($buffer = $fzread($this->fp, 512))
		{
			...
			if (trim($tmp['magic']) == 'ustar')
			{
				...
				else if ($filesize >= 0 && ($filetype == 0 || $filetype == "\0"))
				{
					...
					src_chmod($target_filename, CHMOD_READ);
	...
	*/
	function download($filename, $download_name = false)
	{
		...
		$fp = @fopen("{$src_root_path}store/$filename$this->type", 'rb');
		if ($fp)
		{
			while ($buffer = fread($fp, 1024))
			{
