...
class ucp_profile
{
	...
	{
		global $cache, $config, $db, $user, $auth, $template, $src_root_path, $phpEx;
		global $request, $src_container, $src_dispatcher;

		$user->add_lang('posting');

		$preview	= $request->variable('preview', false, false, \src\request\request_interface::POST);
		$submit		= $request->variable('submit', false, false, \src\request\request_interface::POST);
		$delete		= $request->variable('delete', false, false, \src\request\request_interface::POST);
		$error = $data = array();
		$s_hidden_fields = '';
		...
		switch ($mode)
		{
			case 'reg_details':
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.ucp_profile_reg_details_data', compact($vars)));
				...
				if ($submit)
				{
					...
					*/
					...
					extract($src_dispatcher->trigger_event('core.ucp_profile_reg_details_validate', compact($vars)));
					...
					if (!sizeof($error))
					{
						$sql_ary = array(
							...
							'user_email_hash'	=> ($auth->acl_get('u_chgemail')) ? src_email_hash($data['email']) : $user->data['user_email_hash'],
						...
						*/
						...
						extract($src_dispatcher->trigger_event('core.ucp_profile_reg_details_sql_ary', compact($vars)));
						...
						{
							meta_refresh(5, append_sid($src_root_path . 'index.' . $phpEx));
							$message = $user->lang[$message] . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid($src_root_path . 'index.' . $phpEx) . '">', '</a>');
			...
			case 'profile_info':
				...
				*/
				$vars = array('data', 'submit');
				extract($src_dispatcher->trigger_event('core.ucp_profile_modify_profile_info', compact($vars)));
				...
				if ($submit)
				{
					...
					*/
					$vars = array('data', 'submit', 'error');
					extract($src_dispatcher->trigger_event('core.ucp_profile_validate_profile_info', compact($vars)));
					...
					if (!sizeof($error))
					{
						...
						*/
						...
						extract($src_dispatcher->trigger_event('core.ucp_profile_info_modify_sql_ary', compact($vars)));
			...
			case 'avatar':
				...
				if ($config['allow_avatar'] && $auth->acl_get('u_chgavatar'))
				{
					...
					$avatar_data = \src\avatar\manager::clean_row($user->data, 'user');
					...
					$selected_driver = $src_avatar_manager->clean_driver_name($request->variable('avatar_driver', $user->data['user_avatar_type']));
				...
				$avatar = src_get_user_avatar($user->data, 'USER_AVATAR', true);
				...
				$template->assign_vars(array(
					...
					'L_AVATAR_EXPLAIN'	=> src_avatar_explanation_string(),
