...
/**
* ucp_login_link
* Allows users of external accounts link those accounts to their src accounts
...
class ucp_login_link
{
	...
	*/
	...
	{
		global $src_container, $request, $template, $user;
		...
		if (!$login_link_error)
		{
			...
			{
				$login_username = $request->variable('login_username', '', true, \src\request\request_interface::POST);
				$login_password = $request->untrimmed_variable('login_password', '', true, \src\request\request_interface::POST);
		...
		$template->assign_vars(array(
			...
			'REGISTER_ACTION'	=> append_sid("{$src_root_path}ucp.$phpEx", 'mode=register'),
	...
	*/
	protected function get_login_link_data_array()
	{
		...
		$var_names = $request->variable_names(\src\request\request_interface::GET);
		...
		foreach ($var_names as $var_name)
		{
			if (strpos($var_name, 'login_link_') === 0)
			{
				...
				$login_link_data[$key_name] = $request->variable($var_name, '', false, \src\request\request_interface::GET);
			}
		}
	...
	*/
	...
	{
		global $config, $request, $template, $user, $src_container;
			...
			switch ($result['status'])
			{
				...
				case LOGIN_ERROR_PASSWORD_CONVERT:
					$login_error = sprintf(
						...
						($config['srcrd_contact']) ? '<a href="mailto:' . htmlspecialchars($config['srcrd_contact']) . '">' : '',
						($config['srcrd_contact']) ? '</a>' : ''
					);
				break;
				...
				default:
					...
					{
						$login_error = (!$config['srcrd_contact']) ? sprintf($user->lang[$result['error_msg']], '', '') : sprintf($user->lang[$result['error_msg']], '<a href="mailto:' . htmlspecialchars($config['srcrd_contact']) . '">', '</a>');
					}
	...
	*/
	...
	{
		global $src_root_path, $phpEx;
		$url = append_sid($src_root_path . 'index.' . $phpEx);
		redirect($url);
	}
}
