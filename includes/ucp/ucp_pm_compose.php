...
function compose_pm($id, $mode, $action, $user_folders = array())
{
	...
	if ($cancel || ($current_time - $lastclick < 2 && $submit))
	{
		if ($msg_id)
		{
			redirect(append_sid("{$src_root_path}ucp.$phpEx", 'i=pm&amp;mode=view&amp;action=view_message&amp;p=' . $msg_id));
		}
		redirect(append_sid("{$src_root_path}ucp.$phpEx", 'i=pm'));
	}
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.ucp_pm_compose_modify_data', compact($vars)));
	...
	if ($action != 'edit')
	{
		...
		$template->assign_vars(array(
			...
			'U_FIND_USERNAME'	=> append_sid("{$src_root_path}memberlist.$phpEx", "mode=searchuser&amp;form=postform&amp;field=username_list&amp;select_single=$select_single"),
		));
	}
	...
	if ($sql)
	{
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.ucp_pm_compose_compose_pm_basic_info_query_before', compact($vars)));
		...
		if ($action == 'quotepost')
		{
			...
			*/
			...
			extract($src_dispatcher->trigger_event('core.ucp_pm_compose_quotepost_query_after', compact($vars)));
	...
	$s_action = append_sid("{$src_root_path}ucp.$phpEx", "i=$id&amp;mode=$mode&amp;action=$action", true, $user->session_id);
	...
	if ($action == 'delete')
	{
		...
		else
		{
			...
			// "{$src_root_path}ucp.$phpEx?i=pm&amp;mode=compose"
	...
	if ($save && $auth->acl_get('u_savedrafts'))
	{
		...
		if ($subject && $message)
		{
			if (confirm_box(true))
			{
				...
				$redirect_url = append_sid("{$src_root_path}ucp.$phpEx", "i=pm&amp;mode=$mode");
	...
	if ($submit || $preview || $refresh)
	{
		...
		if (!sizeof($error) && $submit)
		{
			...
			$return_message_url = append_sid("{$src_root_path}ucp.$phpEx", 'i=pm&amp;mode=view&amp;p=' . $msg_id);
			$inbox_folder_url = append_sid("{$src_root_path}ucp.$phpEx", 'i=pm&amp;folder=inbox');
			$outbox_folder_url = append_sid("{$src_root_path}ucp.$phpEx", 'i=pm&amp;folder=outbox');

			$folder_url = '';
			if (($folder_id > 0) && isset($user_folders[$folder_id]))
			{
				$folder_url = append_sid("{$src_root_path}ucp.$phpEx", 'i=pm&amp;folder=' . $folder_id);
			}
	...
	if (($action == 'quote' || $action == 'quotepost') && !$preview && !$refresh && !$submit)
	{
		if ($action == 'quotepost')
		{
			...
			{
				$message_link = "[url=" . generate_srcrd_url() . "/viewtopic.$phpEx?p={$post_id}#p{$post_id}]{$user->lang['SUBJECT']}{$user->lang['COLON']} {$message_subject}[/url]\n\n";
			}
			else
			{
				$message_link = $user->lang['SUBJECT'] . $user->lang['COLON'] . ' ' . $message_subject . " (" . generate_srcrd_url() . "/viewtopic.$phpEx?p={$post_id}#p{$post_id})\n\n";
			}
		}
		else
		{
	...
	if ($action == 'forward' && !$preview && !$refresh && !$submit)
	{
		...
		if ($config['allow_post_links'])
		{
			$quote_username_text = '[url=' . generate_srcrd_url() . "/memberlist.$phpEx?mode=viewprofile&amp;u={$post['author_id']}]{$quote_username}[/url]";
		}
		else
		{
			$quote_username_text = $quote_username . ' (' . generate_srcrd_url() . "/memberlist.$phpEx?mode=viewprofile&amp;u={$post['author_id']})";
		}
	...
	if (sizeof($address_list))
	{
		...
		foreach ($address_list as $type => $adr_ary)
		{
			foreach ($adr_ary as $id => $field)
			{
				...
				{
					$tpl_ary = array_merge($tpl_ary, array(
						'U_VIEW'		=> append_sid("{$src_root_path}memberlist.$phpEx", 'mode=group&amp;g=' . $id),
					));
				}
	...
	$template->assign_vars(array(
		...
		'U_PROGRESS_BAR'			=> append_sid("{$src_root_path}posting.$phpEx", 'f=0&amp;mode=popup'),
		'UA_PROGRESS_BAR'			=> addslashes(append_sid("{$src_root_path}posting.$phpEx", 'f=0&amp;mode=popup')),
	));
