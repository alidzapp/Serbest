...
class ucp_main
{
	...
	{
		global $config, $db, $user, $auth, $template, $src_root_path, $phpEx;
		global $request;
			...
			case 'front':
				...
				foreach ($topic_list as $topic_id)
				{
					...
					$template->assign_block_vars('topicrow', array(
						...
						'U_LAST_POST'			=> append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id&amp;p=" . $row['topic_last_post_id']) . '#p' . $row['topic_last_post_id'],
						'U_LAST_POST_AUTHOR'	=> get_username_string('profile', $row['topic_last_poster_id'], $row['topic_last_poster_name'], $row['topic_last_poster_colour']),
						'U_NEWEST_POST'			=> append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id&amp;view=unread") . '#unread',
						'U_VIEW_TOPIC'			=> append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id"))
					);
				}
...
//					'S_GROUP_OPTIONS'	=> $group_options,
					...
					'U_SEARCH_USER'		=> ($auth->acl_get('u_search')) ? append_sid("{$src_root_path}search.$phpEx", 'author_id=' . $user->data['user_id'] . '&amp;sr=posts') : '',
				));
			...
			case 'subscribed':
				...
				if ($unwatch)
				{
					...
					$message = $msg . '<br /><br />' . sprintf($user->lang['RETURN_UCP'], '<a href="' . append_sid("{$src_root_path}ucp.$phpEx", "i=$id&amp;mode=subscribed") . '">', '</a>');
					meta_refresh(3, append_sid("{$src_root_path}ucp.$phpEx", "i=$id&amp;mode=subscribed"));
				...
				if ($config['allow_forum_notify'])
				{
					...
					while ($row = $db->sql_fetchrow($result))
					{
						...
						{
							$forum_check = (isset($tracking_topics['f'][$forum_id])) ? (int) (base_convert($tracking_topics['f'][$forum_id], 36, 10) + $config['srcrd_startdate']) : $user->data['user_lastmark'];
						}
						...
						if ($row['forum_last_post_id'])
						{
							...
							$last_post_url = append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;p=" . $row['forum_last_post_id']) . '#p' . $row['forum_last_post_id'];
						}
						else
						{
			...
			case 'drafts':
				...
				foreach ($draftrows as $draft)
				{
					...
					{
						$link_topic = true;
						$view_url = append_sid("{$src_root_path}viewtopic.$phpEx", 'f=' . $topic_rows[$draft['topic_id']]['forum_id'] . '&amp;t=' . $draft['topic_id']);
						$title = $topic_rows[$draft['topic_id']]['topic_title'];

						$insert_url = append_sid("{$src_root_path}posting.$phpEx", 'f=' . $topic_rows[$draft['topic_id']]['forum_id'] . '&amp;t=' . $draft['topic_id'] . '&amp;mode=reply&amp;d=' . $draft['draft_id']);
					}
					...
					{
						$link_pm = true;
						$insert_url = append_sid("{$src_root_path}ucp.$phpEx", "i=$id&amp;mode=compose&amp;d=" . $draft['draft_id']);
					}
	...
	*/
	...
	{
		global $user, $db, $template, $config, $cache, $auth, $src_root_path, $phpEx, $src_container;
		...
		foreach ($topic_list as $topic_id)
		{
			...
			// Replies
			$replies = $src_content_visibility->get_count('topic_posts', $row, $forum_id) - 1;
			...
			$pagination->generate_template_pagination(append_sid("{$src_root_path}viewtopic.$phpEx", 'f=' . $row['forum_id'] . "&amp;t=$topic_id"), 'topicrow.pagination', 'start', $replies + 1, $config['posts_per_page'], 1, true, true);
		}
	}
}
