...
class ucp_resend
{
	...
	function main($id, $mode)
	{
		...
		if ($submit)
		{
			...
			if ($config['require_activation'] == USER_ACTIVATION_SELF || $coppa)
			{
				...
				$messenger->assign_vars(array(
					...
					'U_ACTIVATE'	=> generate_srcrd_url() . "/ucp.$phpEx?mode=activate&u={$user_row['user_id']}&k={$user_row['user_actkey']}")
				);
			...
			if ($config['require_activation'] == USER_ACTIVATION_ADMIN)
			{
				...
				while ($row = $db->sql_fetchrow($result))
				{
					...
					$messenger->assign_vars(array(
						'USERNAME'			=> htmlspecialchars_decode($user_row['username']),
						'U_USER_DETAILS'	=> generate_srcrd_url() . "/memberlist.$phpEx?mode=viewprofile&u={$user_row['user_id']}",
						'U_ACTIVATE'		=> generate_srcrd_url() . "/ucp.$phpEx?mode=activate&u={$user_row['user_id']}&k={$user_row['user_actkey']}")
					);
			...
			$message .= '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("{$src_root_path}index.$phpEx") . '">', '</a>');
		...
		$template->assign_vars(array(
			'USERNAME'			=> $username,
			'EMAIL'				=> $email,
			'S_PROFILE_ACTION'	=> append_sid($src_root_path . 'ucp.' . $phpEx, 'mode=resend_act'))
		);
