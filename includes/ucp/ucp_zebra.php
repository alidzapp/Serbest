...
class ucp_zebra
{
	...
	{
		global $config, $db, $user, $auth, $template, $src_root_path, $phpEx, $request, $src_dispatcher;
				...
				if (confirm_box(true))
				{
					...
					if (!empty($data['usernames']))
					{
						...
						*/
						...
						extract($src_dispatcher->trigger_event('core.ucp_remove_zebra', compact($vars)));
							...
							if (sizeof($user_id_ary))
							{
								...
								if (sizeof($user_id_ary))
								{
									...
									*/
									...
									extract($src_dispatcher->trigger_event('core.ucp_add_zebra', compact($vars)));
		...
		$template->assign_vars(array(
			...
			'U_FIND_USERNAME'	=> append_sid("{$src_root_path}memberlist.$phpEx", 'mode=searchuser&amp;form=ucp&amp;field=add'),
