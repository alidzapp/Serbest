...
class ucp_register
{
	...
	function main($id, $mode)
	{
		...
		if (!$agreed || ($coppa === false && $config['coppa_enable']) || ($coppa && !$config['coppa_enable']))
		{
			...
			if ($change_lang)
			{
				...
				$s_hidden_fields = array_merge($s_hidden_fields, array(
					...
					'lang'				=> $user->lang_name,
					'tz'				=> request_var('tz', $config['srcrd_timezone']),
				));
			...
			if ($coppa === false && $config['coppa_enable'])
			{
				...
				$template->assign_vars(array(
					...
					'U_COPPA_NO'		=> append_sid("{$src_root_path}ucp.$phpEx", 'mode=register&amp;coppa=0'),
					'U_COPPA_YES'		=> append_sid("{$src_root_path}ucp.$phpEx", 'mode=register&amp;coppa=1'),

					'S_SHOW_COPPA'		=> true,
					'S_HIDDEN_FIELDS'	=> build_hidden_fields($s_hidden_fields),
					'S_UCP_ACTION'		=> append_sid("{$src_root_path}ucp.$phpEx", 'mode=register'),
			...
			else
			{
				$template->assign_vars(array(
					'S_LANG_OPTIONS'	=> (sizeof($lang_row) > 1) ? language_select($user_lang) : '',
					'L_TERMS_OF_USE'	=> sprintf($user->lang['TERMS_OF_USE_CONTENT'], $config['sitename'], generate_srcrd_url()),

					'S_SHOW_COPPA'		=> false,
					'S_REGISTRATION'	=> true,
					'S_HIDDEN_FIELDS'	=> build_hidden_fields($s_hidden_fields),
					'S_UCP_ACTION'		=> append_sid("{$src_root_path}ucp.$phpEx", 'mode=register' . $add_coppa),
		...
		$timezone = $config['srcrd_timezone'];
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.ucp_register_data_before', compact($vars)));
		...
		if ($submit)
		{
			...
			*/
			...
			extract($src_dispatcher->trigger_event('core.ucp_register_data_after', compact($vars)));
			...
			if (!sizeof($error))
			{
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.ucp_register_user_row_after', compact($vars)));
				...
				if ($config['require_activation'] == USER_ACTIVATION_ADMIN)
				{
					...
					$src_notifications->add_notifications('notification.type.admin_activate_user', array(
						'user_id'		=> $user_id,
				...
				$message = $message . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("{$src_root_path}index.$phpEx") . '">', '</a>');
		...
		$timezone_selects = src_timezone_select($template, $user, $data['tz'], true);
		$template->assign_vars(array(
	...
	*/
	protected function get_login_link_data_array()
	{
		...
		$var_names = $request->variable_names(\src\request\request_interface::POST);
		...
		foreach ($var_names as $var_name)
		{
			if (strpos($var_name, 'login_link_') === 0)
			{
				...
				$login_link_data[$key_name] = $request->variable($var_name, '', false, \src\request\request_interface::POST);
			}
		}
