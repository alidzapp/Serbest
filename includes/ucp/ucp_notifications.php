...
class ucp_notifications
{
	...
	{
		global $config, $template, $user, $request, $src_container;
		...
		switch ($mode)
		{
			case 'notification_options':
				$subscriptions = $src_notifications->get_global_subscriptions(false);
					...
					foreach($src_notifications->get_subscription_types() as $group => $subscription_types)
					{
						foreach($subscription_types as $type => $data)
						{
							foreach($notification_methods as $method => $method_data)
							{
								...
								{
									$src_notifications->add_subscription($type, 0, $method_data['id']);
								}
				...
				$this->output_notification_methods($src_notifications, $template, $user, 'notification_methods');

				$this->output_notification_types($subscriptions, $src_notifications, $template, $user, 'notification_types');
			...
			default:
				...
				{
					$src_notifications->mark_notifications_read(false, false, $user->data['user_id'], $form_time);
				...
				if ($request->is_set_post('submit'))
				{
					...
					if (!empty($mark_read))
					{
						$src_notifications->mark_notifications_read_by_id($mark_read, $form_time);
					}
				}
				...
				$base_url = append_sid("{$src_root_path}ucp.$phpEx", "i=ucp_notifications&amp;mode=notification_list");
