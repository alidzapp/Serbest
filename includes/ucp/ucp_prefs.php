...
class ucp_prefs
{
	...
	{
		global $config, $db, $user, $auth, $template, $src_dispatcher, $src_root_path, $phpEx;
		...
		switch ($mode)
		{
			case 'personal':
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.ucp_prefs_personal_data', compact($vars)));
				...
				if ($submit)
				{
					...
					else if (!src_style_is_active($data['user_style']))
					{
					...
					if (!sizeof($error))
					{
						...
						*/
						...
						extract($src_dispatcher->trigger_event('core.ucp_prefs_personal_update_data', compact($vars)));
			...
			case 'view':
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.ucp_prefs_view_data', compact($vars)));
				...
				if ($submit)
				{
					...
					if (!sizeof($error))
					{
						...
						*/
						$vars = array('data', 'sql_ary');
						extract($src_dispatcher->trigger_event('core.ucp_prefs_view_update_data', compact($vars)));
			...
			case 'post':
				...
				*/
				$vars = array('submit', 'data');
				extract($src_dispatcher->trigger_event('core.ucp_prefs_post_data', compact($vars)));
				...
				if ($submit)
				{
					if (check_form_key('ucp_prefs_post'))
					{
						...
						*/
						$vars = array('data', 'sql_ary');
						extract($src_dispatcher->trigger_event('core.ucp_prefs_post_update_data', compact($vars)));
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.ucp_prefs_modify_common', compact($vars)));
