...
class ucp_groups
{
	...
	{
		global $config, $src_root_path, $phpEx, $src_admin_path;
			...
			case 'membership':
				...
				while ($row = $db->sql_fetchrow($result))
				{
					...
					$template->assign_block_vars($block, array(
						...
						'U_VIEW_GROUP'	=> append_sid("{$src_root_path}memberlist.$phpEx", 'mode=group&amp;g=' . $row['group_id']),
			...
			case 'manage':
				...
				if ($group_id)
				{
					...
					$template->assign_vars(array(
						...
						'AVATAR_IMAGE'			=> (empty($avatar) ? '<img src="' . $src_admin_path . 'images/no_avatar.gif" alt="" />' : $avatar),
				...
				switch ($action)
				{
					...
					case 'list':
						...
						$template->assign_vars(array(
							...
							'U_FIND_USERNAME'	=> append_sid("{$src_root_path}memberlist.$phpEx", 'mode=searchuser&amp;form=ucp&amp;field=usernames'),
						));
