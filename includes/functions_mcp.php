...
*/
function src_module__url($mode, &$module_row)
{
	return src_extra_url();
}

function src_module_notes_url($mode, &$module_row)
{
	if ($mode == 'front')
	{
		return '';
...
function src_module_warn_url($mode, &$module_row)
{
...
function src_module_main_url($mode, &$module_row)
{
	return src_extra_url();
}

function src_module_logs_url($mode, &$module_row)
{
	return src_extra_url();
}

function src_module_ban_url($mode, &$module_row)
{
	return src_extra_url();
}

function src_module_queue_url($mode, &$module_row)
{
	return src_extra_url();
}

function src_module_reports_url($mode, &$module_row)
{
	return src_extra_url();
}

function src_extra_url()
{
...
*/
function src_get_topic_data($topic_ids, $acl_list = false, $read_tracking = false)
{
...
*/
function src_get_post_data($post_ids, $acl_list = false, $read_tracking = false)
{
...
*/
function src_get_forum_data($forum_id, $acl_list = 'f_list', $read_tracking = false)
{
	global $auth, $db, $user, $config, $src_container;
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		$row['forum_topics_approved'] = $src_content_visibility->get_count('forum_topics', $row, $row['forum_id']);
...
* Get simple pm data
*/
function src_get_pm_data($pm_ids)
{
	global $db;
...
*/
function src_mcp_sorting($mode, &$sort_days, &$sort_key, &$sort_dir, &$sort_by_sql, &$sort_order_sql, &$total, $forum_id = 0, $topic_id = 0, $where_sql = 'WHERE')
{
	global $db, $user, $auth, $template, $src_dispatcher;
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.mcp_sorting_query_before', compact($vars)));
...
*/
function src_check_ids(&$ids, $table, $sql_id, $acl_list = false, $single_forum = false)
{
	global $db, $auth;
