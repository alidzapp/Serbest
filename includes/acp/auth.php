...
*/
class auth_admin extends \src\auth\auth
{
	/**
	* Init auth settings
	*/
	...
	*/
	...
	{
		global $template, $user, $db, $src_root_path, $phpEx, $src_container;
		$src_permissions = $src_container->get('acl.permissions');
		...
		$l_acl_type = $src_permissions->get_type_lang($permission_type, (($local) ? 'local' : 'global'));
	...
	*/
	...
	{
		global $db, $template, $user, $src_root_path, $src_admin_path, $phpEx;
			...
			if (isset($auth_ary['groups']) && sizeof($auth_ary['groups']))
			{
				...
				while ($row = $db->sql_fetchrow($result))
				{
					$template->assign_block_vars('role_mask.groups', array(
						...
						'U_PROFILE'		=> append_sid("{$src_root_path}memberlist.$phpEx", "mode=group&amp;g={$row['group_id']}"))
					);
				}
				$db->sql_freeresult($result);
			}
	...
	*/
	...
	{
		global $template, $user, $src_admin_path, $phpEx, $src_container;
		...
		while (list($cat, $cat_array) = each($category_array))
		{
			$template->assign_block_vars($tpl_cat, array(
				...
				'CAT_NAME'	=> $src_permissions->get_category_lang($cat),
			));
			...
			while (list($permission, $allowed) = each($cat_array['permissions']))
			{
				if ($s_view)
				{
					$template->assign_block_vars($tpl_cat . '.' . $tpl_mask, array(
						...
						'U_TRACE'		=> ($show_trace) ? append_sid("{$src_admin_path}index.$phpEx", "i=permissions&amp;mode=trace&amp;u=$ug_id&amp;f=$forum_id&amp;auth=$permission") : '',
						'UA_TRACE'		=> ($show_trace) ? append_sid("{$src_admin_path}index.$phpEx", "i=permissions&mode=trace&u=$ug_id&f=$forum_id&auth=$permission", false) : '',

						'PERMISSION'	=> $src_permissions->get_permission_lang($permission),
					));
				}
				else
				{
	...
	*/
	function build_permission_array(&$permission_row, &$content_array, &$categories, $key_sort_array)
	{
		...
		foreach ($key_sort_array as $forum_id)
		{
			...
			{
				$cat = $src_permissions->get_permission_category($permission);

				// Build our categories array
				if (!isset($categories[$cat]))
				{
					$categories[$cat] = $src_permissions->get_category_lang($cat);
				}
