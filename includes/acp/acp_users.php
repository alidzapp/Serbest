...
class acp_users
{
	...
	function main($id, $mode)
	{
		...
		global $src_root_path, $src_admin_path, $phpEx, $table_prefix, $file_uploads;
		global $src_dispatcher, $request;
		...
		if ($action == 'whois')
		{
			...
			$user_ip = src_ip_normalise(request_var('user_ip', ''));
		...
		if (!$username && !$user_id)
		{
			...
			$template->assign_vars(array(
				...
				'S_SELECT_USER'		=> true,
				'U_FIND_USERNAME'	=> append_sid("{$src_root_path}memberlist.$phpEx", 'mode=searchuser&amp;form=select_user&amp;field=username&amp;select_single=true'),
			));
		...
		$template->assign_vars(array(
			...
			'U_MODE_SELECT'		=> append_sid("{$src_admin_path}index.$phpEx", "i=$id&amp;u=$user_id"),
		...
		switch ($mode)
		{
			case 'overview':
				...
				extract($src_dispatcher->trigger_event('core.acp_users_overview_before', compact($vars)));
				...
				if ($submit)
				{
					...
					switch ($action)
					{
						...
						case 'active':
							...
							if ($user_row['user_type'] == USER_INACTIVE)
							{
								if ($config['require_activation'] == USER_ACTIVATION_ADMIN)
								{
									...
									$src_notifications->delete_notifications('notification.type.admin_activate_user', $user_row['user_id']);
						...
						case 'delavatar':
							...
							$src_avatar_manager->handle_avatar_delete($db, $user, $src_avatar_manager->clean_row($user_row, 'user'), USERS_TABLE, 'user_');
						...
						default:
							...
							*/
							...
							extract($src_dispatcher->trigger_event('core.acp_users_overview_run_quicktool', compact($vars)));
						break;
					}
					...
					if (!sizeof($error))
					{
						...
						*/
						...
						extract($src_dispatcher->trigger_event('core.acp_users_overview_modify_data', compact($vars)));
						...
						if ($update_email !== false)
						{
							$sql_ary += array(
								...
								'user_email_hash'	=> src_email_hash($update_email),
							);
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.acp_users_display_overview', compact($vars)));
				...
				$template->assign_vars(array(
					...
					'U_MCP_QUEUE'	=> ($auth->acl_getf_global('m_approve')) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=queue', true, $user->session_id) : '',
					'U_SEARCH_USER'	=> ($config['load_search'] && $auth->acl_get('u_search')) ? append_sid("{$src_root_path}search.$phpEx", "author_id={$user_row['user_id']}&amp;sr=posts") : '',

					'U_SWITCH_PERMISSIONS'	=> ($auth->acl_get('a_switchperm') && $user->data['user_id'] != $user_row['user_id']) ? append_sid("{$src_root_path}ucp.$phpEx", "mode=switch_perm&amp;u={$user_row['user_id']}&amp;hash=" . generate_link_hash('switchperm')) : '',
			...
			case 'profile':
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.acp_users_modify_profile', compact($vars)));
				...
				if ($submit)
				{
					...
					*/
					...
					extract($src_dispatcher->trigger_event('core.acp_users_profile_validate', compact($vars)));
					...
					if (!sizeof($error))
					{
						...
						*/
						...
						extract($src_dispatcher->trigger_event('core.acp_users_profile_modify_sql_ary', compact($vars)));
			...
			case 'prefs':
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.acp_users_prefs_modify_data', compact($vars)));
				...
				if ($submit)
				{
					...
					if (!sizeof($error))
					{
						...
						*/
						...
						extract($src_dispatcher->trigger_event('core.acp_users_prefs_modify_sql', compact($vars)));
				...
				src_timezone_select($template, $user, $data['tz'], true);
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.acp_users_prefs_modify_template_data', compact($vars)));
			...
			case 'avatar':
				...
				{
					$src_avatar_manager = $src_container->get('avatar.manager');
					$avatar_drivers = $src_avatar_manager->get_enabled_drivers();

					// This is normalised data, without the user_ prefix
					$avatar_data = \src\avatar\manager::clean_row($user_row, 'user');

					if ($submit)
					{
						if (check_form_key($form_name))
						{
							$driver_name = $src_avatar_manager->clean_driver_name($request->variable('avatar_driver', ''));

							if (in_array($driver_name, $avatar_drivers) && !$request->is_set_post('avatar_delete'))
							{
								$driver = $src_avatar_manager->get_driver($driver_name);
					...
					if ($request->is_set_post('avatar_delete'))
					{
						...
						{
							$src_avatar_manager->handle_avatar_delete($db, $user, $avatar_data, USERS_TABLE, 'user_');

							trigger_error($user->lang['USER_AVATAR_UPDATED'] . adm_back_link($this->u_action . '&amp;u=' . $user_id));
						}
					}

					$selected_driver = $src_avatar_manager->clean_driver_name($request->variable('avatar_driver', $user_row['user_avatar_type']));

					foreach ($avatar_drivers as $current_driver)
					{
						$driver = $src_avatar_manager->get_driver($current_driver);

						$avatars_enabled = true;
						$config_name = $src_avatar_manager->get_driver_config_name($driver);
						...
						{
							$driver_name = $src_avatar_manager->prepare_driver_name($current_driver);
				...
				// Replace "error" strings with their real, localised form
				$error = $src_avatar_manager->localize_errors($user, $error);

				$avatar = src_get_user_avatar($user_row, 'USER_AVATAR', true);

				$template->assign_vars(array(
					'S_AVATAR'	=> true,
					'ERROR'			=> (!empty($error)) ? implode('<br />', $error) : '',
					'AVATAR'		=> (empty($avatar) ? '<img src="' . $src_admin_path . 'images/no_avatar.gif" alt="" />' : $avatar),
			...
			case 'sig':
				...
				include_once($src_root_path . 'includes/functions_display.' . $phpEx);
				...
				if ($submit || $preview)
				{
					include_once($src_root_path . 'includes/message_parser.' . $phpEx);
				...
				$template->assign_vars(array(
					...
					'BBCODE_STATUS'			=> ($config['allow_sig_bbcode']) ? sprintf($user->lang['BBCODE_IS_ON'], '<a href="' . append_sid("{$src_root_path}faq.$phpEx", 'mode=bbcode') . '">', '</a>') : sprintf($user->lang['BBCODE_IS_OFF'], '<a href="' . append_sid("{$src_root_path}faq.$phpEx", 'mode=bbcode') . '">', '</a>'),
			...
			case 'attach':
				...
				while ($row = $db->sql_fetchrow($result))
				{
					if ($row['in_message'])
					{
						$view_topic = append_sid("{$src_root_path}ucp.$phpEx", "i=pm&amp;p={$row['post_msg_id']}");
					}
					else
					{
						$view_topic = append_sid("{$src_root_path}viewtopic.$phpEx", "t={$row['topic_id']}&amp;p={$row['post_msg_id']}") . '#p' . $row['post_msg_id'];
					}
					...
					$template->assign_block_vars('attach', array(
						...
						'U_DOWNLOAD'		=> append_sid("{$src_root_path}download/file.$phpEx", 'mode=view&amp;id=' . $row['attach_id']),
			...
			case 'groups':
				...
				foreach ($group_data as $group_type => $data_ary)
				{
					...
					{
						$template->assign_block_vars('group', array(
							'U_EDIT_GROUP'		=> append_sid("{$src_admin_path}index.$phpEx", "i=groups&amp;mode=manage&amp;action=edit&amp;u=$user_id&amp;g={$data['group_id']}&amp;back_link=acp_users_groups"),
			...
			case 'perm':
				...
				include_once($src_root_path . 'includes/acp/auth.' . $phpEx);
				...
				$template->assign_vars(array(
					...
					'U_ACTION'					=> $this->u_action . '&amp;u=' . $user_id,
					'U_USER_PERMISSIONS'		=> append_sid("{$src_admin_path}index.$phpEx" ,'i=permissions&amp;mode=setting_user_global&amp;user_id[]=' . $user_id),
					'U_USER_FORUM_PERMISSIONS'	=> append_sid("{$src_admin_path}index.$phpEx", 'i=permissions&amp;mode=setting_user_local&amp;user_id[]=' . $user_id))
				);
	...
	*/
	function optionget(&$user_row, $key, $data = false)
	{
		...
		$var = ($data !== false) ? $data : $user_row['user_options'];
		return src_optionget($user->keyoptions[$key], $var);
	}
}
