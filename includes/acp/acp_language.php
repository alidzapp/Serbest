...
class acp_language
{
	...
	function main($id, $mode)
	{
		...
		global $src_root_path, $phpEx, $request;
		...
		switch ($action)
		{
			...
			case 'details':
				...
				if ($lang_iso != $config['default_lang'])
				{
						...
						$iterator = new \RecursiveIteratorIterator(
							new \src\recursive_dot_prefix_filter_iterator(
								new \RecursiveDirectoryIterator(
									$src_root_path . 'language/' . $config['default_lang'] . '/',
					...
					foreach ($iterator as $file_info)
					{
						...
						if (file_exists($src_root_path . 'language/' . $lang_iso . '/' . $relative_path))
						{
			...
			case 'install':
				...
				if (!$lang_iso || !file_exists("{$src_root_path}language/$lang_iso/iso.txt"))
				{
					trigger_error($user->lang['LANGUAGE_PACK_NOT_EXIST'] . adm_back_link($this->u_action), E_USER_WARNING);
				}

				$file = file("{$src_root_path}language/$lang_iso/iso.txt");
		...
		$new_ary = $iso = array();
		$dp = @opendir("{$src_root_path}language");

		if ($dp)
		{
			while (($file = readdir($dp)) !== false)
			{
				if ($file[0] == '.' || !is_dir($src_root_path . 'language/' . $file))
				{
					continue;
				}

				if (file_exists("{$src_root_path}language/$file/iso.txt"))
				{
					if (!in_array($file, $installed))
					{
						if ($iso = file("{$src_root_path}language/$file/iso.txt"))
						{
							if (sizeof($iso) == 3)
							{
								$new_ary[$file] = array(
	...
	*/
	function compare_language_files($source_lang, $dest_lang, $file)
	{
		...
		$source_file = $src_root_path . 'language/' . $source_lang . '/' . $file;
		$dest_file = $src_root_path . 'language/' . $dest_lang . '/' . $file;
