...
class acp_permissions
{
	...
	function main($id, $mode)
	{
		...
		$this->permissions = $src_container->get('acl.permissions');
		...
		foreach ($permission_victim as $victim)
		{
			switch ($victim)
			{
				...
				case 'user':
					...
					$template->assign_vars(array(
						'S_SELECT_USER'			=> true,
						'U_FIND_USERNAME'		=> append_sid("{$src_root_path}memberlist.$phpEx", 'mode=searchuser&amp;form=select_victim&amp;field=username&amp;select_single=true'),
					));
				...
				case 'usergroup_view':
					...
					$template->assign_vars(array(
						...
						'U_FIND_USERNAME'			=> append_sid("{$src_root_path}memberlist.$phpEx", 'mode=searchuser&amp;form=add_user&amp;field=username&amp;select_single=true'),
					));
	...
	*/
	function set_permissions($mode, $permission_type, &$auth_admin, &$user_id, &$group_id)
	{
		...
		$settings = $request->variable('setting', array(0 => array(0 => array('' => 0))), false, \src\request\request_interface::POST);
		...
		$roles = $request->variable('role', array(0 => array(0 => 0)), false, \src\request\request_interface::POST);
