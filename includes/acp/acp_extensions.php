...
class acp_extensions
{
	...
	{
		// Start the page
		global $config, $user, $template, $request, $src_extension_manager, $db, $src_root_path, $phpEx, $src_log, $cache;
		...
		if ($ext_name)
		{
			$md_manager = new \src\extension\metadata_manager($ext_name, $config, $src_extension_manager, $template, $user, $src_root_path);
		...
		switch ($action)
		{
			...
			default:
				...
				$this->list_enabled_exts($src_extension_manager);
				$this->list_disabled_exts($src_extension_manager);
				$this->list_available_exts($src_extension_manager);
			...
			case 'enable_pre':
				...
				$extension = $src_extension_manager->get_extension($ext_name);
				if (!$extension->is_enableable())
				{
					trigger_error($user->lang['EXTENSION_NOT_ENABLEABLE'] . adm_back_link($this->u_action), E_USER_WARNING);
				}

				if ($src_extension_manager->is_enabled($ext_name))
				{
			...
			case 'enable':
				...
				try
				{
					while ($src_extension_manager->enable_step($ext_name))
					{
			...
			case 'disable':
				...
				while ($src_extension_manager->disable_step($ext_name))
				{
					// Are we approaching the time limit? If so we want to pause the update and continue after refreshing
					if ((time() - $start_time) >= $safe_time_limit)
					{
			...
			case 'delete_data':
				...
				try
				{
					while ($src_extension_manager->purge_step($ext_name))
					{
						// Are we approaching the time limit? If so we want to pause the update and continue after refreshing
						if ((time() - $start_time) >= $safe_time_limit)
						{
	...
	* @return null
	*/
	public function list_enabled_exts(\src\extension\manager $src_extension_manager)
	{
		$enabled_extension_meta_data = array();

		foreach ($src_extension_manager->all_enabled() as $name => $location)
		{
	...
	* @return null
	*/
	public function list_disabled_exts(\src\extension\manager $src_extension_manager)
	{
		$disabled_extension_meta_data = array();

		foreach ($src_extension_manager->all_disabled() as $name => $location)
		{
			$md_manager = $src_extension_manager->create_extension_metadata_manager($name, $this->template);
	...
	*
	* @param \src\extension\metadata_manager $md_manager The metadata manager for the version to check.
	* @param bool $force_update Ignores cached data. Defaults to false.
	* @param bool $force_cache Force the use of the cache. Override $force_update.
	* @return string
	* @throws RuntimeException
	*/
	protected function version_check(\src\extension\metadata_manager $md_manager, $force_update = false, $force_cache = false)
	{
		$meta = $md_manager->get_metadata('all');
		...
		$version_helper = new \src\version_helper($this->cache, $this->config, new \src\file_downloader(), $this->user);
