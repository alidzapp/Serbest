	...
	function main($id, $mode)
	{
		...
		switch ($action)
		{
			case 'save':
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.acp_ranks_save_modify_sql_ary', compact($vars)));
			...
			case 'add':
				...
				$imglist = filelist($src_root_path . $config['ranks_path'], '');
				...
				$tpl_ary = array(
					...
					'RANKS_PATH'		=> $src_root_path . $config['ranks_path'],
					'U_ACTION'			=> $this->u_action . '&amp;id=' . $rank_id,

					'RANK_TITLE'		=> (isset($ranks['rank_title'])) ? $ranks['rank_title'] : '',
					'S_FILENAME_LIST'	=> $filename_list,
					'RANK_IMAGE'		=> ($edit_img) ? $src_root_path . $config['ranks_path'] . '/' . $edit_img : htmlspecialchars($src_admin_path) . 'images/spacer.gif',
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.acp_ranks_edit_modify_tpl_ary', compact($vars)));
		...
		while ($row = $db->sql_fetchrow($result))
		{
			$rank_row = array(
				...
				'RANK_IMAGE'		=> $src_root_path . $config['ranks_path'] . '/' . $row['rank_image'],
			...
			*/
			...
			extract($src_dispatcher->trigger_event('core.acp_ranks_list_modify_rank_row', compact($vars)));
