...
class acp_forums
{
	...
	{
		global $db, $user, $auth, $template, $cache, $request, $src_dispatcher;
			...
			switch ($action)
			{
				...
				case 'add':
					...
					*/
					...
					extract($src_dispatcher->trigger_event('core.acp_manage_forums_request_data', compact($vars)));
...
*/
						...
						{
							$message .= '<br /><br />' . sprintf($user->lang['REDIRECT_ACL'], '<a href="' . append_sid("{$src_admin_path}index.$phpEx", 'i=permissions' . $acl_url) . '">', '</a>');

							meta_refresh(4, append_sid("{$src_admin_path}index.$phpEx", 'i=permissions' . $acl_url));
						}
		...
		switch ($action)
		{
			...
			case 'edit':
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.acp_manage_forums_initialise_data', compact($vars)));
				...
				$template_data = array(
					...
					'FORUM_IMAGE_SRC'			=> ($forum_data['forum_image']) ? $src_root_path . $forum_data['forum_image'] : '',
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.acp_manage_forums_display_form', compact($vars)));
		...
		if ($row = $db->sql_fetchrow($result))
		{
			do
			{
				...
				$template->assign_block_vars('forums', array(
					...
					'FORUM_IMAGE'		=> ($row['forum_image']) ? '<img src="' . $src_root_path . $row['forum_image'] . '" alt="" />' : '',
	...
	*/
	...
	{
		global $db, $user, $cache, $src_root_path, $src_container, $src_dispatcher;
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.acp_manage_forums_validate_data', compact($vars)));
		...
		if (!empty($forum_data['forum_image']) && !file_exists($src_root_path . $forum_data['forum_image']))
		{
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.acp_manage_forums_update_data_before', compact($vars)));
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.acp_manage_forums_update_data_after', compact($vars)));
	...
	*/
	function move_forum($from_id, $to_id)
	{
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.acp_manage_forums_move_children', compact($vars)));
	...
	*/
	...
	{
		global $db, $config, $src_root_path, $phpEx;
	...
	*/
	...
	{
		global $phpEx, $src_admin_path, $template, $user;

		$acl_url = '&amp;mode=setting_forum_local&amp;forum_id[]=' . $forum_data['forum_id'];
		$action = append_sid($this->u_action . "&amp;parent_id={$this->parent_id}&amp;f={$forum_data['forum_id']}&amp;action=copy_perm");

		$l_acl = sprintf($user->lang['COPY_TO_ACL'], '<a href="' . append_sid("{$src_admin_path}index.$phpEx", 'i=permissions' . $acl_url) . '">', '</a>');

		$this->tpl_name = 'acp_forums_copy_perm';

		$template->assign_vars(array(
			'U_ACL'				=> append_sid("{$src_admin_path}index.$phpEx", 'i=permissions' . $acl_url),
			'L_ACL_LINK'		=> $l_acl,
