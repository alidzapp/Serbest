...
class acp_email
{
	...
	function main($id, $mode)
	{
		...
		global $src_root_path, $src_admin_path, $phpEx, $table_prefix, $src_dispatcher;
		...
		if ($submit)
		{
			...
			if (!sizeof($error))
			{
				...
				*/
				$vars = array('sql_ary');
				extract($src_dispatcher->trigger_event('core.acp_email_modify_sql', compact($vars)));
				...
				include_once($src_root_path . 'includes/functions_user.' . $phpEx);
				...
				$template_data = array(
					'CONTACT_EMAIL' => src_get_srcrd_contact($config, $phpEx),
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.acp_email_send_before', compact($vars)));
				...
				{
					$message = sprintf($user->lang['EMAIL_SEND_ERROR'], '<a href="' . append_sid("{$src_admin_path}index.$phpEx", 'i=logs&amp;mode=critical') . '">', '</a>');
		...
		$template_data = array(
			...
			'USERNAMES'				=> implode("\n", $usernames),
			'U_FIND_USERNAME'		=> append_sid("{$src_root_path}memberlist.$phpEx", 'mode=searchuser&amp;form=acp_email&amp;field=usernames'),
			'SUBJECT'				=> $subject,
			'MESSAGE'				=> $message,
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.acp_email_display', compact($vars)));
