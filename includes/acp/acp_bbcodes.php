...
class acp_bbcodes
{
	...
	function main($id, $mode)
	{
		...
		switch ($action)
		{
			...
			case 'add':
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.acp_bbcodes_edit_add', compact($vars)));
				...
				foreach ($bbcode_tokens as $token)
				{
					$template->assign_block_vars('token', array(
						'TOKEN'		=> '{' . $token . '}',
						'EXPLAIN'	=> ($token === 'LOCAL_URL') ? $user->lang(array('tokens', $token), generate_srcrd_url() . '/') : $user->lang(array('tokens', $token)),
					));
				}
			...
			case 'create':
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.acp_bbcodes_modify_create', compact($vars)));
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.acp_bbcodes_display_form', compact($vars)));
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			*/
			...
			extract($src_dispatcher->trigger_event('core.acp_bbcodes_display_bbcodes', compact($vars)));
