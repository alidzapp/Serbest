...
class acp_search
{
				...
				if (!$this->init_search($cfg_array['search_type'], $search, $error))
				{
					if (confirm_box(true))
					{
						if (!method_exists($search, 'init') || !($error = $search->init()))
						{
							...
							$extra_message = '<br />' . $user->lang['SWITCHED_SEARCH_BACKEND'] . '<br /><a href="' . append_sid("{$src_admin_path}index.$phpEx", 'i=search&amp;mode=index') . '">&raquo; ' . $user->lang['GO_TO_SEARCH_INDEX'] . '</a>';
						}
						else
						{
	...
	function index($id, $mode)
	{
			...
			switch ($action)
			{
				case 'delete':
					if (method_exists($this->search, 'delete_index'))
					{
						...
						if ($error = $this->search->delete_index($this, append_sid("{$src_admin_path}index.$phpEx", "i=$id&mode=$mode&action=delete", false)))
						{
							$this->state = array('');
							$this->save_state();
				...
				case 'create':
					if (method_exists($this->search, 'create_index'))
					{
						...
						if ($error = $this->search->create_index($this, append_sid("{$src_admin_path}index.$phpEx", "i=$id&mode=$mode&action=create", false)))
						{
							$this->state = array('');
							$this->save_state();
							trigger_error($error . adm_back_link($this->u_action) . $this->close_popup_js(), E_USER_WARNING);
		...
		$template->assign_vars(array(
			'S_INDEX'				=> true,
			'U_ACTION'				=> $this->u_action,
			'U_PROGRESS_BAR'		=> append_sid("{$src_admin_path}index.$phpEx", "i=$id&amp;mode=$mode&amp;action=progress_bar"),
			'UA_PROGRESS_BAR'		=> addslashes(append_sid("{$src_admin_path}index.$phpEx", "i=$id&amp;mode=$mode&amp;action=progress_bar")),
		));
	...
	{
		global $src_root_path, $phpEx, $src_extension_manager;
	...
	*/
	...
	{
		global $src_root_path, $phpEx, $user, $auth, $config, $db, $src_dispatcher;
		...
		$error = false;
		$search = new $type($error, $src_root_path, $phpEx, $auth, $config, $db, $user, $src_dispatcher);
