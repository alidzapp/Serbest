	...
	function main($id, $mode)
	{
		...
		if ($action == 'edit' || $action == 'add' || $action == 'import')
		{
			...
			foreach ($imglist as $path => $img_ary)
			{
				...
				{
					$img_size = getimagesize($src_root_path . $img_path . '/' . $path . $img);
			...
			if ($dir = @opendir($src_root_path . $img_path))
			{
				while (($file = readdir($dir)) !== false)
				{
					if (is_file($src_root_path . $img_path . '/' . $file) && preg_match('#\.pak$#i', $file))
					{
						$_paks[] = $file;
					}
				}
		...
		switch ($action)
		{
			...
			case 'add':
				...
				foreach ($data as $img => $img_row)
				{
					$template->assign_block_vars('items', array(
						...
						'IMG_SRC'	=> $src_root_path . $img_path . '/' . $img,
				...
				if ($action == 'add' && $mode == 'smilies' && sizeof($smilies))
				{
					$template->assign_vars(array(
						...
						'IMG_SRC'			=> $src_root_path . $img_path . '/' . $default_row['smiley_url'],
						'IMG_PATH'			=> $img_path,
			...
			case 'modify':
				...
				if ($request->variable('add_additional_code', false, false, \src\request\request_interface::POST))
				{
					...
					if ($add_image && $add_emotion && $add_code)
					{
						...
						if ($request->variable('add_display_on_posting', false, false, \src\request\request_interface::POST))
						{
				...
				foreach ($images as $image)
				{
					...
					else if (!file_exists($src_root_path . $img_path . '/' . $image))
					{
					...
					else
					{
						...
						{
							$img_size = getimagesize($src_root_path . $img_path . '/' . $image);
			...
			case 'import':
				...
				if ($pak != '')
				{
					...
					if (!($pak_ary = @file($src_root_path . $img_path . '/' . $pak)))
					{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			$template->assign_block_vars('items', array(
				...
				'ALT_TEXT'		=> $alt_text,
				'IMG_SRC'		=> $src_root_path . $img_path . '/' . $row[$fields . '_url'],
