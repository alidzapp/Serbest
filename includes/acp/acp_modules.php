...
class acp_modules
{
	...
	*/
	...
	{
		global $src_extension_manager, $src_root_path, $phpEx;

		$module_class = ($module_class === false) ? $this->module_class : $module_class;

		$directory = $src_root_path . 'includes/' . $module_class . '/info/';
		$fileinfo = array();

		$finder = $src_extension_manager->get_finder($use_all_available);
		...
		foreach ($modules as $cur_module)
		{
			...
			// format. src_acp_info_acp_foo needs to be turned into
			// acp_foo_info and the respective file has to be included
			// manually because it does not support auto loading
			$old_info_class_file = str_replace("src_{$module_class}_info_", '', $cur_module);
	...
	*/
	function remove_cache_file()
	{
		...
		$src_container->get('cache.driver')->destroy('_modules_' . $p_class);

		// Additionally remove sql cache
		$src_container->get('cache.driver')->destroy('sql', MODULES_TABLE);
	}
