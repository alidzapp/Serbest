...
class acp_groups
{
		...
		switch ($action)
		{
			...
			case 'add':
				...
				if ($config['allow_avatar'])
				{
					...
					$avatar_data = \src\avatar\manager::clean_row($group_row, 'group');
				...
				if ($request->is_set_post('avatar_delete'))
				{
					if (confirm_box(true))
					{
						...
						$src_avatar_manager->handle_avatar_delete($db, $user, $avatar_data, GROUPS_TABLE, 'group_');
				...
				if ($update)
				{
					...
					if ($config['allow_avatar'])
					{
						...
						{
							$driver = $src_avatar_manager->get_driver($avatar_data['avatar_type']);
							if ($driver)
							{
						...
						$error = array_merge($error, $src_avatar_manager->localize_errors($user, $avatar_error));
					}
					...
					* supported by src3. Also validate the submitted colour value.
					*/
					...
					*/
					...
					extract($src_dispatcher->trigger_event('core.acp_manage_group_request_data', compact($vars)));
					...
					if (!sizeof($error))
					{
						...
						*/
						...
						extract($src_dispatcher->trigger_event('core.acp_manage_group_initialise_data', compact($vars)));
				...
				{
					$avatars_enabled = false;
					$selected_driver = $src_avatar_manager->clean_driver_name($request->variable('avatar_driver', $avatar_data['avatar_type']));
				...
				$avatar = src_get_group_avatar($group_row, 'GROUP_AVATAR', true);

				if (isset($src_avatar_manager) && !$update)
				{
					// Merge any avatar errors into the primary error array
					$error = array_merge($error, $src_avatar_manager->localize_errors($user, $avatar_error));
				}
				...
				switch ($back_link)
				{
					case 'acp_users_groups':
						$u_back = append_sid("{$src_admin_path}index.$phpEx", 'i=users&amp;mode=groups&amp;u=' . request_var('u', 0));
					break;
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.acp_manage_group_display_form', compact($vars)));
			...
			case 'list':
				...
				while ($row = $db->sql_fetchrow($result))
				{
					$template->assign_block_vars('leader', array(
						'U_USER_EDIT'		=> append_sid("{$src_admin_path}index.$phpEx", "i=users&amp;action=edit&amp;u={$row['user_id']}"),
				...
				$template->assign_vars(array(
					...
					'U_BACK'			=> $this->u_action,
					'U_FIND_USERNAME'	=> append_sid("{$src_root_path}memberlist.$phpEx", 'mode=searchuser&amp;form=list&amp;field=usernames'),
	...
	{
		global $config, $db, $template, $user, $request, $src_container;
		...
		else if ($field && in_array($field, array('legend', 'teampage')))
		{
			...
			$group_position = $src_container->get('groupposition.' . $field);
		}
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			if ($row['group_legend'])
			{
				$template->assign_block_vars('legend', array(
					...
					'GROUP_TYPE'	=> $user->lang[\src\groupposition\legend::group_type_language($row['group_type'])],
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			{
				$group_name = ($row['group_type'] == GROUP_SPECIAL) ? $user->lang['G_' . $row['group_name']] : $row['group_name'];
				$group_type = $user->lang[\src\groupposition\teampage::group_type_language($row['group_type'])];
			}
			else
			{
