...
class acp_database
{
	...
	function main($id, $mode)
	{
		...
		$this->db_tools = new \src\db\tools($db);
			...
			case 'restore':
				...
				switch ($action)
				{
					case 'submit':
						...
						$file_name = $src_root_path . 'store/' . $matches[0];
...
class base_extractor
{
	...
	function base_extractor($format, $filename, $time, $download = false, $store = false)
	{
		...
		if ($store == true)
		{
			...
			$file = $src_root_path . 'store/' . $filename . $ext;
