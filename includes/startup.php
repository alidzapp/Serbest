...
//    automatically used by src. This is the default mode that src
//    will use when shipped.
// 2. To disable composer autoloading, src_NO_COMPOSER_AUTOLOAD can be specified.
// 	  Additionally specify src_AUTOLOAD=/path/to/autoload.php in the
...
//    for all of src's dependencies.
// 3. You can also set src_NO_COMPOSER_AUTOLOAD without setting src_AUTOLOAD.
//    In this case autoloading needs to be defined before running any src
//    script. This might be useful in cases when src is integrated into a
//    larger program.
if (getenv('src_NO_COMPOSER_AUTOLOAD'))
{
	if (getenv('src_AUTOLOAD'))
	{
		require(getenv('src_AUTOLOAD'));
	}
}
else
{
	if (!file_exists($src_root_path . 'vendor/autoload.php'))
	{
		trigger_error(
			'Composer dependencies have not been set up yet, run ' .
			"'php ../composer.phar install' from the src directory to do so.",
			E_USER_ERROR
		);
	}
	require($src_root_path . 'vendor/autoload.php');
}
