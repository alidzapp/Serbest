...
function get_available_dbms($dbms = false, $return_unavailable = false, $only_20x_options = false)
{
	...
	$available_dbms = array(
		...
		'mssql_odbc'=>	array(
			...
			'MODULE'		=> 'odbc',
			'DELIM'			=> 'GO',
			'DRIVER'		=> 'src\db\driver\mssql_odbc',
			'AVAILABLE'		=> true,
			'2.0.x'			=> true,
		),
		'mssqlnative'		=> array(
			...
			'MODULE'		=> 'sqlsrv',
			'DELIM'			=> 'GO',
			'DRIVER'		=> 'src\db\driver\mssqlnative',
			'AVAILABLE'		=> true,
			'2.0.x'			=> false,
		),
		'oracle'	=>	array(
...
*/
function src_remove_comments($input)
{
...
/**
* Creates the output to be stored in a src config.php file
*
...
*/
function src_create_config_file_data($data, $dbms, $debug = false, $debug_container = false, $debug_test = false)
{
	$config_data = "<?php\n";
	$config_data .= "// src 3.1.x auto-generated configuration file\n// Do not change anything in this file!\n";
	...
	$config_data_array = array(
		...
		'src_adm_relative_path'	=> 'adm/',

		'acm_type'		=> 'src\cache\driver\file',
	);
	...
	$config_data .= "\n@define('src_INSTALLED', true);\n";
	$config_data .= "// @define('src_DISPLAY_LOAD_TIME', true);\n";
...
* @param	string	$file				File including path from src root
* @return	bool		Should we ignore the new file or add it to the srcrd?
*/
function src_ignore_new_file_on_update($src_root_path, $file)
{
	...
	if (!$ignore_new_file && strpos($file, 'language/') === 0)
	{
		...
		$ignore_new_file = !file_exists($src_root_path . $language_dir . '/' . $language_iso);
	}
	...
	if (!$ignore_new_file && strpos($file, 'styles/') === 0)
	{
		...
		$ignore_new_file = !file_exists($src_root_path . $styles_dir . '/' . $style_name);
	}
	...
	if (!$ignore_new_file && strpos($file, 'styles/') === 0)
	{
		...
		if (sizeof($dirs) >= 5)
		{
			...
			{
				$ignore_new_file = !file_exists($src_root_path . 'language/' . $language_iso);
			}
		}
	}
...
*
* @param string $src_root_path	Path to the src srcrd root.
* @param string $php_ext			PHP file extension.
*
* @return bool Returns true if src is installed.
*/
function src_check_installation_exists($src_root_path, $php_ext)
{
	// Try opening config file
	if (file_exists($src_root_path . 'config.' . $php_ext))
	{
		include($src_root_path . 'config.' . $php_ext);
	}

	return defined('src_INSTALLED');
}
