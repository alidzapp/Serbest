...
class p_master
{
	...
	*/
	function p_master($include_path = false)
	{
		...
		$this->include_path = ($include_path !== false) ? $include_path : $src_root_path . 'includes/';
	...
	*/
	function list_modules($p_class)
	{
		...
		global $config, $src_root_path, $phpEx, $src_dispatcher;
		...
		foreach ($this->module_cache['modules'] as $key => $row)
		{
			...
			$url_func = 'src_module_' . $short_name . '_url';
			...
			$lang_func = 'src_module_' . $short_name . '_lang';
			...
			$custom_func = 'src_module_' . $short_name;
			...
			*/
			...
			extract($src_dispatcher->trigger_event('core.modify_module_row', compact($vars)));
	...
	*/
	...
	{
		global $auth, $config;
		global $request, $src_extension_manager, $src_dispatcher;
		...
		$valid_tokens = array(
			...
			'ext_([a-zA-Z0-9_/]+)'			=> 'array_key_exists(\'\\1\', $src_extension_manager->all_enabled())',
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.module_auth', compact($vars)));
	...
	*/
	...
	{
		global $src_root_path, $src_admin_path, $phpEx, $user, $template;
		...
		// new modules use the full class names, old ones are always called <type>_<name>, e.g. acp_srcrd
		...
		if (defined('IN_ADMIN'))
		{
			...
			*/
			...
			{
				$module_style_dir = $src_root_path . 'ext/' . $module_dir[0] . '/' . $module_dir[1] . '/adm/style';
				...
				if (is_dir($module_style_dir))
				{
					...
					), array($module_style_dir, $src_admin_path . 'style'));
				}
			}
			...
			$this->module->u_action = append_sid("{$src_admin_path}index.$phpEx", 'i=' . $this->get_module_identifier($this->p_name)) . (($icat) ? '&amp;icat=' . $icat : '') . "&amp;mode={$this->p_mode}";
		}
		else
		{
			/*
			...
			*/
			...
			if (isset($module_dir[1]))
			{
				...
				if (is_dir($src_root_path . $module_style_dir))
				{
			...
			{
				$this->module->u_action = $src_root_path . (($user->page['page_dir']) ? $user->page['page_dir'] . '/' : '') . $user->page['page_name'];
			}
	...
	*                     src_ext_foo_acp_bar_module, ucp_zebra or zebra)
	* @param string $mode mode, as passed through to the module
	*
	*/
	function load($class, $name, $mode = false)
	{
		// new modules use the full class names, old ones are always called <class>_<name>, e.g. acp_srcrd
		// in the latter case this function may be called as load('acp', 'srcrd')
	...
	*/
	...
	{
		global $config, $user, $phpEx, $src_extension_manager;

		$finder = $src_extension_manager->get_finder();
		...
		// Search for srcrd default language if it's not the user language
	...
	* @return string The basename if it starts with src_ or the basename with
	*                the current p_class (e.g. acp_) stripped.
	*/
	protected function get_short_name($basename)
	{
		if (substr($basename, 0, 6) === 'src\\' || strpos($basename, '\\') !== false)
		{
			return $basename;
		}
	...
	*
	* @param string $basename A module basename
	* @return bool True if the basename starts with src_ or (x)cp_, false otherwise
	*/
