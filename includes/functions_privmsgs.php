...
function get_folder($user_id, $folder_id = false)
{
	...
	foreach ($folder as $f_id => $folder_ary)
	{
		...
		$template->assign_block_vars('folder', array(
			...
			'U_FOLDER'			=> ($f_id > 0) ? append_sid("{$src_root_path}ucp.$phpEx", 'i=pm&amp;folder=' . $f_id) : append_sid("{$src_root_path}ucp.$phpEx", 'i=pm&amp;folder=' . $folder_id_name),
		...
		!in_array($cur_folder_id, array(PRIVMSGS_NO_BOX, PRIVMSGS_OUTBOX)) && $cur_folder_id != $dest_folder)
	{
		...
		if ($dest_folder != PRIVMSGS_INBOX)
		{
			...
			if ($message_limit && $row['pm_count'] + sizeof($move_msg_ids) > $message_limit)
			{
				...
				$message .= sprintf($user->lang['CLICK_RETURN_FOLDER'], '<a href="' . append_sid("{$src_root_path}ucp.$phpEx", 'i=pm&amp;folder=' . $row['folder_id']) . '">', '</a>', $row['folder_name']);
		...
		else
		{
			...
			if ($message_limit && $num_messages + sizeof($move_msg_ids) > $message_limit)
			{
				...
				$message .= sprintf($user->lang['CLICK_RETURN_FOLDER'], '<a href="' . append_sid("{$src_root_path}ucp.$phpEx", 'i=pm&amp;folder=inbox') . '">', '</a>', $user->lang['PM_INBOX']);
				trigger_error($message);
			}
		}
...
function update_unread_status($unread, $msg_id, $user_id, $folder_id)
{
	...
	global $db, $user, $src_container;

	$src_notifications = $src_container->get('notification_manager');

	$src_notifications->mark_notifications_read('notification.type.pm', $msg_id, $user_id);
	...
	switch ($mark_action)
	{
		...
		case 'delete_marked':
			...
			if (confirm_box(true))
			{
				...
				$redirect = append_sid("{$src_root_path}ucp.$phpEx", 'i=pm&amp;folder=' . $cur_folder_id);
...
{
	global $db, $user, $src_root_path, $phpEx, $src_container, $src_dispatcher;
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.delete_pm_before', compact($vars)));
	...
	$src_notifications->delete_notifications('notification.type.pm', array_keys($delete_rows));
...
* @return	boolean		False if there were no pms found, true otherwise.
*/
function src_delete_users_pms($user_ids)
{
	global $db, $user, $src_root_path, $phpEx, $src_container;
	...
	if (!empty($undelivered_msg))
	{
		...
		if (!empty($delivered_msg))
		{
			...
			$src_notifications->delete_notifications('notification.type.pm', $delivered_msg);
		}
		...
		if (!empty($undelivered_msg))
		{
			...
			$src_notifications->delete_notifications('notification.type.pm', $undelivered_msg);
		}
	}
	...
	if (!empty($delete_ids))
	{
		...
		if (!empty($delete_ids))
		{
			...
			$src_notifications->delete_notifications('notification.type.pm', $delete_ids);
		}
	}
...
{
	global $db, $user, $template, $src_root_path, $phpEx;
...
{
	global $db, $auth, $config, $phpEx, $template, $user, $src_root_path, $src_container, $src_dispatcher;
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.submit_pm_before', compact($vars)));
	...
	if ($mode == 'edit')
	{
		$src_notifications->update_notifications('notification.type.pm', $pm_data);
	}
	else
	{
		$src_notifications->add_notifications('notification.type.pm', $pm_data);
	}
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.submit_pm_after', compact($vars)));
...
{
	global $db, $user, $config, $template, $src_root_path, $phpEx, $auth;
	...
	$rowset = array();
	$folder_url = append_sid("{$src_root_path}ucp.$phpEx", 'i=pm') . '&amp;folder=';
...
{
	global $db, $src_root_path, $phpEx, $user;
		...
		foreach ($adr_ary as $type => $id_ary)
		{
			foreach ($id_ary as $ug_id => $_id)
			{
				...
				else
				{
					...
					$link = '<a href="' . append_sid("{$src_root_path}memberlist.$phpEx", 'mode=group&amp;g=' . $ug_id) . '"' . $user_colour . '>';
