...
{
	global $config, $auth, $template, $user, $db, $src_path_helper;
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		if (!$display_jumpbox)
		{
			$template->assign_block_vars('jumpbox_forums', array(
				...
				'LINK'			=> $src_path_helper->append_url_params($action, array('f' => $forum_id)),
			));
		...
		$template->assign_block_vars('jumpbox_forums', array(
			...
			'LINK'			=> $src_path_helper->append_url_params($action, array('f' => $row['forum_id'])),
		));
	...
	$url_parts = $src_path_helper->get_url_parts($action);
...
*/
function src_clean_search_string($search_string)
{
...
function generate_text_for_display($text, $uid, $bitfield, $flags, $censor_text = true)
{
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.modify_text_for_display_before', compact($vars)));
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.modify_text_for_display_after', compact($vars)));
...
function generate_text_for_storage(&$text, &$uid, &$bitfield, &$flags, $allow_bbcode = false, $allow_urls = false, $allow_smilies = false)
{
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.modify_text_for_storage_before', compact($vars)));
	...
	*/
	$vars = array('text', 'uid', 'bitfield', 'flags');
	extract($src_dispatcher->trigger_event('core.modify_text_for_storage_after', compact($vars)));
...
function generate_text_for_edit($text, $uid, $flags)
{
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.modify_text_for_edit_before', compact($vars)));
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.modify_text_for_edit_after', compact($vars)));
...
function make_clickable_callback($type, $whitespace, $url, $relative_url, $class)
{
	...
	switch ($type)
	{
		case MAGIC_URL_LOCAL:
			...
			// this url goes to http://domain.tld/path/to/srcrd/ which
...
function parse_attachments($forum_id, &$message, &$attachments, &$update_count, $preview = false)
{
	...
	global $template, $cache, $user, $src_dispatcher;
	global $extensions, $config, $src_root_path, $phpEx;
	...
	foreach ($attachments as $attachment)
	{
		...
		$filename = $src_root_path . $config['upload_path'] . '/' . utf8_basename($attachment['physical_filename']);
		$thumbnail_filename = $src_root_path . $config['upload_path'] . '/thumb_' . utf8_basename($attachment['physical_filename']);
		...
		if (isset($extensions[$attachment['extension']]))
		{
			...
			{
				$upload_icon = '<img src="' . $src_root_path . $config['upload_icons_path'] . '/' . trim($extensions[$attachment['extension']]['upload_icon']) . '" alt="" />';
			}
		}
		...
		if (!$denied)
		{
			...
			$download_link = append_sid("{$src_root_path}download/file.$phpEx", 'id=' . $attachment['attach_id']);
			...
			switch ($display_cat)
			{
				...
				case ATTACHMENT_CATEGORY_IMAGE:
					$inline_link = append_sid("{$src_root_path}download/file.$phpEx", 'id=' . $attachment['attach_id']);
				...
				case ATTACHMENT_CATEGORY_THUMB:
					$thumbnail_link = append_sid("{$src_root_path}download/file.$phpEx", 'id=' . $attachment['attach_id'] . '&amp;t=1');
					$download_link .= '&amp;mode=view';
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.parse_attachments_modify_template_data', compact($vars)));
...
function get_username_string($mode, $user_id, $username, $username_colour = '', $guest_username = false, $custom_profile_url = false)
{
	...
	if (empty($_profile_cache))
	{
		...
		$_profile_cache['base_url'] = append_sid("{$src_root_path}memberlist.$phpEx", 'mode=viewprofile&amp;u={USER_ID}');
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.modify_username_string', compact($vars)));
 ...
 */
function src_add_quickmod_option($url, $option, $lang_string)
{
	global $template, $user, $src_path_helper;

	$lang_string = $user->lang($lang_string);
	$template->assign_block_vars('quickmod', array(
		'VALUE'		=> $option,
		'TITLE'		=> $lang_string,
		'LINK'		=> $src_path_helper->append_url_params($url, array('action' => $option)),
	));
}
...
* @param object $user The src $user object.
*
* @return string String list. Examples: "A"; "A and B"; "A, B, and C"
*/
function src_generate_string_list($items, $user)
{
	if (empty($items))
	{
		return '';
