...
*/
function src_load_extensions_autoloaders($src_root_path)
{
...
/**
* Wrapper function of \src\request\request::variable which exists for backwards compatability.
* See {@link \src\request\request_interface::variable \src\request\request_interface::variable} for
...
* 										If you pass an instance of {@link \src\request\request_interface src_request_interface}
...
* @param	bool			$cookie		This param is mapped to \src\request\request_interface::COOKIE as the last param for
* 										\src\request\request_interface::variable for backwards compatability reasons.
* @param	\src\request\request_interface|null|false	If an instance of \src\request\request_interface is given the instance is stored in
*										a static variable and used for all further calls where this parameters is null. Until
*										the function is called with an instance it automatically creates a new \src\request\request
*										instance on every call. By passing false this per-call instantiation can be restored
*										after having passed in a \src\request\request_interface instance.
*
...
function request_var($var_name, $default, $multibyte = false, $cookie = false, $request = null)
{
	...
	if ($request instanceof \src\request\request_interface)
	{
	...
	if ($tmp_request === null)
	{
		...
		$tmp_request = new \src\request\request(new \src\request\type_cast_helper(), false);
	}

	return $tmp_request->variable($var_name, $default, $multibyte, ($cookie) ? \src\request\request_interface::COOKIE : \src\request\request_interface::REQUEST);
}
...
* @deprecated
*/
function set_config($config_name, $config_value, $is_dynamic = false, \src\config\config $set_config = null)
{
	static $config = null;
...
* @deprecated
*/
function set_config_count($config_name, $increment, $is_dynamic = false, \src\config\config $set_config = null)
{
	static $config = null;
...
*/
function src_gmgetdate($time = false)
{
	if ($time === false)
	{
		$time = time();
...
*/
function src_version_compare($version1, $version2, $operator = null)
{
...
*/
function src_chmod($filename, $perms = CHMOD_READ)
{
	static $_chmod_info;
	...
	if (empty($_chmod_info))
	{
		...
		else
		{
			...
			$common_php_owner = @fileowner($src_root_path . 'common.' . $phpEx);
			$common_php_group = @filegroup($src_root_path . 'common.' . $phpEx);
	...
	switch ($php)
	{
		case 'owner':
			...
			if (is_readable($filename) && src_is_writable($filename))
			{
				break;
			}
		...
		case 'group':
			...
			if ((!($perms & CHMOD_READ) || is_readable($filename)) && (!($perms & CHMOD_WRITE) || src_is_writable($filename)))
			{
				break;
			}
...
* @return mixed
*/
function src_own_realpath($path)
{
	global $request;
...
if (!function_exists('realpath'))
{
	...
	* @ignore
	*/
	function src_realpath($path)
	{
		return src_own_realpath($path);
	}
}
else
{
...
*													16200 => +04:30
*/
function src_format_timezone_offset($tz_offset, $show_null = false)
{
	$sign = ($tz_offset < 0) ? '-' : '+';
...
*/
function src_tz_select_compare($a, $b)
{
	$a_sign = $a[3];
	$b_sign = $b[3];
	if ($a_sign != $b_sign)
...
*/
function src_get_timezone_identifiers($selected_timezone)
{
...
function markread($mode, $forum_id = false, $topic_id = false, $post_time = 0, $user_id = 0)
{
	...
	global $request, $src_container, $src_dispatcher;
	...
	extract($src_dispatcher->trigger_event('core.markread_before', compact($vars)));
	...
	if ($mode == 'all')
	{
		if ($forum_id === false || !sizeof($forum_id))
		{
			...
			$src_notifications->mark_notifications_read(array(
			...
			{
				$tracking_topics = $request->variable($config['cookie_name'] . '_track', '', true, \src\request\request_interface::COOKIE);
				$tracking_topics = ($tracking_topics) ? tracking_unserialize($tracking_topics) : array();

				unset($tracking_topics['tf']);
				unset($tracking_topics['t']);
				unset($tracking_topics['f']);
				$tracking_topics['l'] = base_convert($post_time - $config['srcrd_startdate'], 10, 36);

				$user->set_cookie('track', tracking_serialize($tracking_topics), $post_time + 31536000);
				$request->overwrite($config['cookie_name'] . '_track', tracking_serialize($tracking_topics), \src\request\request_interface::COOKIE);
	...
	else if ($mode == 'topics')
	{
		...
		$src_notifications->mark_notifications_read_by_parent(array(
			'notification.type.topic',
			'notification.type.approve_topic',
		...
		{
			$tracking = $request->variable($config['cookie_name'] . '_track', '', true, \src\request\request_interface::COOKIE);
			...
			foreach ($forum_id as $f_id)
			{
				...
				$tracking['f'][$f_id] = base_convert($post_time - $config['srcrd_startdate'], 10, 36);
			}
			...
			$request->overwrite($config['cookie_name'] . '_track', tracking_serialize($tracking), \src\request\request_interface::COOKIE);
	...
	else if ($mode == 'topic')
	{
		...
		else if ($config['load_anon_lastread'] || $user->data['is_registered'])
		{
			...
			$tracking['t'][$topic_id36] = base_convert($post_time - $config['srcrd_startdate'], 10, 36);

			// If the cookie grows larger than 10000 characters we will remove the smallest value
			// This can result in old topics being unread - but most of the time it should be accurate...
			if (strlen($request->variable($config['cookie_name'] . '_track', '', true, \src\request\request_interface::COOKIE)) > 10000)
			{
				...
				if ($user->data['is_registered'])
				{
					$user->data['user_lastmark'] = intval(base_convert(max($time_keys) + $config['srcrd_startdate'], 36, 10));
...
function get_complete_topic_tracking($forum_id, $topic_ids, $global_announce_list = false)
{
	...
	else if ($config['load_anon_lastread'] || $user->data['is_registered'])
	{
		...
		if (!$user->data['is_registered'])
		{
			$user_lastmark = (isset($tracking_topics['l'])) ? base_convert($tracking_topics['l'], 36, 10) + $config['srcrd_startdate'] : 0;
		}
		else
		{
		...
		foreach ($topic_ids as $topic_id)
		{
			...
			{
				$last_read[$topic_id] = base_convert($tracking_topics['t'][$topic_id36], 36, 10) + $config['srcrd_startdate'];
			}
		}
		...
		if (sizeof($topic_ids))
		{
			...
			{
				$mark_time[$forum_id] = base_convert($tracking_topics['f'][$forum_id], 36, 10) + $config['srcrd_startdate'];
			}
...
function get_unread_topics($user_id = false, $sql_extra = '', $sql_sort = '', $sql_limit = 1001, $sql_limit_offset = 0)
{
	...
	if ($config['load_db_lastread'] && $user->data['is_registered'])
	{
		...
		extract($src_dispatcher->trigger_event('core.get_unread_topics_modify_sql', compact($vars)));
	...
	else if ($config['load_anon_lastread'] || $user->data['is_registered'])
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			if (isset($tracking_topics['t'][$topic_id36]))
			{
				$last_read = base_convert($tracking_topics['t'][$topic_id36], 36, 10) + $config['srcrd_startdate'];
			...
			{
				$mark_time = base_convert($tracking_topics['f'][$forum_id], 36, 10) + $config['srcrd_startdate'];
...
{
	global $db, $tracking_topics, $user, $config, $auth, $request, $src_container;
	...
	if ($mark_time_forum === false)
	{
		...
		else if ($config['load_anon_lastread'] || $user->data['is_registered'])
		{
			...
			if (!$user->data['is_registered'])
			{
				$user->data['user_lastmark'] = (isset($tracking_topics['l'])) ? (int) (base_convert($tracking_topics['l'], 36, 10) + $config['srcrd_startdate']) : 0;
			}

			$mark_time_forum = (isset($tracking_topics['f'][$forum_id])) ? (int) (base_convert($tracking_topics['f'][$forum_id], 36, 10) + $config['srcrd_startdate']) : $user->data['user_lastmark'];
		}
	}
		...
		else
		{
			$sql = 'SELECT t.forum_id
				...
				WHERE t.forum_id = ' . $forum_id . '
					...
					AND ' . $src_content_visibility->get_visibility_sql('topic', $forum_id, 't.') . '
...
* Examples:
* <code>
* append_sid("{$src_root_path}viewtopic.$phpEx?t=1&amp;f=2");
* append_sid("{$src_root_path}viewtopic.$phpEx", 't=1&amp;f=2');
* append_sid("{$src_root_path}viewtopic.$phpEx", 't=1&f=2', false);
* append_sid("{$src_root_path}viewtopic.$phpEx", array('t' => 1, 'f' => 2));
* </code>
*
*/
function append_sid($url, $params = false, $is_amp = true, $session_id = false, $is_route = false)
{
	global $_SID, $_EXTRA_URL, $src_hook, $src_path_helper;
	global $src_dispatcher;
	...
	if (!$is_route && $src_path_helper instanceof \src\path_helper)
	{
		$url = $src_path_helper->update_web_root_path($url);
	}
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.append_sid', compact($vars)));
	...
	if (!empty($src_hook) && $src_hook->call_hook(__FUNCTION__, $url, $params, $is_amp, $session_id))
	{
		if ($src_hook->hook_return(__FUNCTION__))
		{
			return $src_hook->hook_return_result(__FUNCTION__);
		}
	}
...
/**
* Generate srcrd url (example: http://www.example.com/src)
*
* @param bool $without_script_path if set to true the script path gets not appended (example: http://www.example.com)
*
* @return string the generated srcrd url
*/
function generate_srcrd_url($without_script_path = false)
{
	global $config, $user, $request;
...
* This function is intended for urls within the srcrd. It's not meant to redirect to cross-domains.
*
* @param string $url The url to redirect to
* @param bool $return If true, do not redirect but return the sanitized URL. Default is no return.
* @param bool $disable_cd_check If true, redirect() will redirect to an external domain. If false, the redirect point to the srcrds url if it does not match the current domain. Default is false.
*/
function redirect($url, $return = false, $disable_cd_check = false)
{
	global $db, $cache, $config, $user, $src_root_path, $src_filesystem, $src_path_helper, $phpEx, $src_dispatcher;
	...
	if ($url_parts === false)
	{
		...
		$url = generate_srcrd_url() . '/' . $user->page['page'];
	}
	...
	else if ($url[0] == '/')
	{
		...
		$url = generate_srcrd_url(true) . $url;
	}
	else
	{
		// Relative uri
		...
		$url = $src_path_helper->remove_web_root_path($url);
		...
		$url = generate_srcrd_url() . '/' . $url;
	}

	// Clean URL and check if we go outside the forum directory
	$url = $src_path_helper->clean_url($url);

	if (!$disable_cd_check && strpos($url, generate_srcrd_url(true) . '/') !== 0)
	{
		trigger_error('INSECURE_REDIRECT', E_USER_ERROR);
	}
	...
	* @var	bool	disable_cd_check	If true, redirect() will redirect to an external domain. If false, the redirect point to the srcrds url if it does not match the current domain.
	* @since 3.1.0-RC3
	*/
	$vars = array('url', 'return', 'disable_cd_check');
	extract($src_dispatcher->trigger_event('core.functions.redirect', compact($vars)));
...
{
	global $phpEx, $src_root_path;

	if ($url === "index.$phpEx")
	{
		return append_sid("index.$phpEx");
	}
	else if ($url === "{$src_root_path}index.$phpEx")
	{
		return append_sid("{$src_root_path}index.$phpEx");
	}
...
{
	global $config, $user, $src_path_helper;

	$page = $src_path_helper->get_valid_page($user->page['page'], $config['enable_mod_rewrite']);
	...
	if ($strip_vars !== false)
	{
		$redirect = $src_path_helper->strip_url_params($redirect, $strip_vars, false);
	}
	else
	{
...
* @param bool $disable_cd_check If true, meta_refresh() will redirect to an external domain. If false, the redirect point to the srcrds url if it does not match the current domain. Default is false.
*/
...
function send_status_line($code, $message)
{
	...
	{
		$version = src_request_http_version();
...
*/
function src_request_http_version()
{
	global $request;
...
{
	global $config, $template, $user, $src_dispatcher;
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.add_form_key', compact($vars)));
...
function confirm_box($check, $title = '', $hidden = '', $html_body = 'confirm_body.html', $u_action = '')
{
	...
	$confirm = ($user->lang['YES'] === $request->variable('confirm', '', true, \src\request\request_interface::POST));
	...
	$u_action = reapply_sid($src_path_helper->get_valid_page($use_page, $config['enable_mod_rewrite']));
...
{
	global $db, $user, $template, $auth, $phpEx, $src_root_path, $config;
	global $request, $src_container, $src_dispatcher;
	...
	if ($request->is_set_post('login') || ($request->is_set('login') && $request->variable('login', '') == 'external'))
	{
		...
		if ($result['status'] == LOGIN_SUCCESS)
		{
			$redirect = request_var('redirect', "{$src_root_path}index.$phpEx");
			...
			*/
			...
			extract($src_dispatcher->trigger_event('core.login_box_redirect', compact($vars)));
		...
		switch ($result['status'])
		{
			case LOGIN_ERROR_PASSWORD_CONVERT:
				$err = sprintf(
					...
					($config['email_enable']) ? '<a href="' . append_sid("{$src_root_path}ucp.$phpEx", 'mode=sendpassword') . '">' : '',
					($config['email_enable']) ? '</a>' : '',
					'<a href="' . src_get_srcrd_contact_link($config, $src_root_path, $phpEx) . '">',
					'</a>'
				);
			break;
			...
			default:
				...
				{
					$err = sprintf($user->lang[$result['error_msg']], '<a href="' . append_sid("{$src_root_path}memberlist.$phpEx", 'mode=contactadmin') . '">', '</a>');
				}
		...
		extract($src_dispatcher->trigger_event('core.login_box_failed', compact($vars)));
	}
	...
	$template->assign_vars(array(
		...
		'U_SEND_PASSWORD' 		=> ($config['email_enable']) ? append_sid("{$src_root_path}ucp.$phpEx", 'mode=sendpassword') : '',
		'U_RESEND_ACTIVATION'	=> ($config['require_activation'] == USER_ACTIVATION_SELF && $config['email_enable']) ? append_sid("{$src_root_path}ucp.$phpEx", 'mode=resend_act') : '',
		'U_TERMS_USE'			=> append_sid("{$src_root_path}ucp.$phpEx", 'mode=terms'),
		'U_PRIVACY'				=> append_sid("{$src_root_path}ucp.$phpEx", 'mode=privacy'),
	...
	make_jumpbox(append_sid("{$src_root_path}viewforum.$phpEx"));
...
{
	global $db, $src_container, $request, $template, $user, $src_dispatcher;
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.login_forum_box', compact($vars)));
...
*
* @deprecated	Use $src_log->add() instead
*/
function add_log()
{
	global $src_log, $user;
	...
	return $src_log->add($mode, $user_id, $user_ip, $log_operation, time(), $additional_data);
}
...
* Also filters out absolute paths to src root.
*
...
function get_backtrace()
{
	...
	foreach ($backtrace as $trace)
	{
		...
		$trace['file'] = (empty($trace['file'])) ? '(not given by php)' : htmlspecialchars(src_filter_root_path($trace['file']));
		...
		{
			$argument = htmlspecialchars(src_filter_root_path($trace['args'][0]));
		}
...
*					string otherwise
*/
function src_ip_normalise($address)
{
	...
	return src_inet_ntop(src_inet_pton($address));
}
...
*					string otherwise
*/
function src_inet_ntop($in_addr)
{
	...
	switch (strlen($in_addr))
	{
		...
		case 32:
			...
			{
				return src_inet_ntop(pack('H*', substr($in_addr, 24)));
			}
...
*/
function src_inet_pton($address)
{
	$ret = '';
	if (preg_match(get_preg_expression('ipv4'), $address))
	{
	...
	if (preg_match(get_preg_expression('ipv6'), $address))
	{
		...
		if (preg_match(get_preg_expression('ipv4'), $last_part))
		{
			...
			$last_part = src_inet_pton($last_part);
			$embedded_ipv4 = true;
			--$missing_parts;
		}
...
* with === true or === false,
*/
function src_checkdnsrr($host, $type = 'MX')
{
...
function msg_handler($errno, $msg_text, $errfile, $errline)
{
	...
	global $phpEx, $src_root_path, $msg_title, $msg_long_text;
	...
	switch ($errno)
	{
		...
		case E_WARNING:
			...
			if (strpos($errfile, 'cache') === false && strpos($errfile, 'template.') === false)
			{
				...
				$msg_text = src_filter_root_path($msg_text);
				$error_name = ($errno === E_WARNING) ? 'PHP Warning' : 'PHP Notice';
				echo '<b>[src Debug] ' . $error_name . '</b>: in file <b>' . $errfile . '</b> on line <b>' . $errline . '</b>: <b>' . $msg_text . '</b><br />' . "\n";
		...
		case E_USER_ERROR:
			...
			if (!empty($user) && !empty($user->lang))
			{
				...
				$l_return_index = sprintf($user->lang['RETURN_INDEX'], '<a href="' . $src_root_path . '">', '</a>');
				$l_notify = '';

				if (!empty($config['srcrd_contact']))
				{
					$l_notify = '<p>' . sprintf($user->lang['NOTIFY_ADMIN_EMAIL'], $config['srcrd_contact']) . '</p>';
				}
			}
			else
			{
				$msg_title = 'General Error';
				$l_return_index = '<a href="' . $src_root_path . '">Return to index page</a>';
				$l_notify = '';

				if (!empty($config['srcrd_contact']))
				{
					$l_notify = '<p>Please notify the administrator or webmaster: <a href="mailto:' . $config['srcrd_contact'] . '">' . $config['srcrd_contact'] . '</a></p>';
				}
			}
...
{
	global $config, $db, $user, $auth, $src_dispatcher;
	...
	if (sizeof($online_users['online_users']))
	{
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.obtain_users_online_string_sql', compact($vars)));
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.obtain_users_online_string_modify', compact($vars)));
...
*/
function src_optionget($bit, $data)
{
...
*/
function src_optionset($bit, $set, $data)
{
	if ($set && !($data & 1 << $bit))
	{
		$data += 1 << $bit;
...
*
* @param $rule		int			ID of the plural rule we want to use, see http://wiki.src.com/Plural_Rules#Plural_Rules
* @param $number	int|float	The number we want to get the plural case for. Float numbers are floored.
* @return	int		The plural-case we need to use for the number plural-rule combination
*/
function src_get_plural_form($rule, $number)
{
	$number = (int) $number;
...
function src_http_login($param)
{
	...
	{
		if ($request->is_set($k, \src\request\request_interface::SERVER))
		{
...
*/
function src_quoteattr($data, $entities = null)
{
...
*/
function src_get_user_avatar($user_row, $alt = 'USER_AVATAR', $ignore_config = false)
{
	$row = \src\avatar\manager::clean_row($user_row, 'user');
...
* Get avatar
*
* @param array $row Row cleaned by \src\avatar\manager::clean_row
* @param string $alt Optional language string for alt tag within image, can be a language key or text
* @param bool $ignore_config Ignores the config-setting, to be still able to view the avatar in the UCP
*
* @return string Avatar html
*/
function src_get_avatar($row, $alt, $ignore_config = false)
{
	global $user, $config, $cache, $src_root_path, $phpEx;
	global $request;
	...
	$driver = $src_avatar_manager->get_driver($row['avatar_type'], $ignore_config);
	$html = '';
...
{
	global $db, $config, $template, $SID, $_SID, $_EXTRA_URL, $user, $auth, $phpEx, $src_root_path;
	global $src_dispatcher, $request, $src_container, $src_admin_path;
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.page_header', compact($vars)));
	...
	if ($config['gzip_compress'])
	{
		...
		//    beyond that is manual, so the code wrapping src in output buffering
	...
	{
		$u_login_logout = append_sid("{$src_root_path}ucp.$phpEx", 'mode=logout', true, $user->session_id);
		$l_login_logout = $user->lang['LOGOUT'];
	}
	else
	{
		$u_login_logout = append_sid("{$src_root_path}ucp.$phpEx", 'mode=login');
	...
	// Determine srcrd url - we may need it later
	$srcrd_url = generate_srcrd_url() . '/';
	// This path is sent with the base template paths in the assign_vars()
	// call below. We need to correct it in case we are accessing from a
	// controller because the web paths will be incorrect otherwise.
	$src_path_helper = $src_container->get('path_helper');
	$corrected_path = $src_path_helper->get_web_root_path();
	$web_path = (defined('src_USE_srcRD_URL_PATH') && src_USE_srcRD_URL_PATH) ? $srcrd_url : $corrected_path;
	...
	$timezone_offset = $user->lang(array('timezones', 'UTC_OFFSET'), src_format_timezone_offset($dt->getOffset()));
	...
	if ($config['load_notifications'] && $user->data['user_id'] != ANONYMOUS && $user->data['user_type'] != USER_IGNORE)
	{
		...
		$notifications = $src_notifications->load_notifications(array(
			'all_unread'	=> true,
			'limit'			=> 5,
		));
	...
	$template->assign_vars(array(
		...
		'CURRENT_USER_AVATAR'			=> src_get_user_avatar($user->data),
		...
		'U_VIEW_ALL_NOTIFICATIONS'		=> append_sid("{$src_root_path}ucp.$phpEx", 'i=ucp_notifications'),
		'U_MARK_ALL_NOTIFICATIONS'		=> append_sid("{$src_root_path}ucp.$phpEx", 'i=ucp_notifications&amp;mode=notification_list&amp;mark=all&amp;token=' . $notification_mark_hash),
		'U_NOTIFICATION_SETTINGS'		=> append_sid("{$src_root_path}ucp.$phpEx", 'i=ucp_notifications&amp;mode=notification_options'),
		...
		'L_INDEX'			=> ($config['srcrd_index_text'] !== '') ? $config['srcrd_index_text'] : $user->lang['FORUM_INDEX'],
		'L_SITE_HOME'		=> ($config['site_home_text'] !== '') ? $config['site_home_text'] : $user->lang['HOME'],
		'L_ONLINE_EXPLAIN'	=> $l_online_time,

		'U_PRIVATEMSGS'			=> append_sid("{$src_root_path}ucp.$phpEx", 'i=pm&amp;folder=inbox'),
		'U_RETURN_INBOX'		=> append_sid("{$src_root_path}ucp.$phpEx", 'i=pm&amp;folder=inbox'),
		'U_MEMBERLIST'			=> append_sid("{$src_root_path}memberlist.$phpEx"),
		'U_VIEWONLINE'			=> ($auth->acl_gets('u_viewprofile', 'a_user', 'a_useradd', 'a_userdel')) ? append_sid("{$src_root_path}viewonline.$phpEx") : '',
		'U_LOGIN_LOGOUT'		=> $u_login_logout,
		'U_INDEX'				=> append_sid("{$src_root_path}index.$phpEx"),
		'U_SEARCH'				=> append_sid("{$src_root_path}search.$phpEx"),
		'U_SITE_HOME'			=> $config['site_home_url'],
		'U_REGISTER'			=> append_sid("{$src_root_path}ucp.$phpEx", 'mode=register'),
		'U_PROFILE'				=> append_sid("{$src_root_path}ucp.$phpEx"),
		'U_USER_PROFILE'		=> get_username_string('profile', $user->data['user_id'], $user->data['username'], $user->data['user_colour']),
		'U_MODCP'				=> append_sid("{$src_root_path}mcp.$phpEx", false, true, $user->session_id),
		'U_FAQ'					=> append_sid("{$src_root_path}faq.$phpEx"),
		'U_SEARCH_SELF'			=> append_sid("{$src_root_path}search.$phpEx", 'search_id=egosearch'),
		'U_SEARCH_NEW'			=> append_sid("{$src_root_path}search.$phpEx", 'search_id=newposts'),
		'U_SEARCH_UNANSWERED'	=> append_sid("{$src_root_path}search.$phpEx", 'search_id=unanswered'),
		'U_SEARCH_UNREAD'		=> append_sid("{$src_root_path}search.$phpEx", 'search_id=unreadposts'),
		'U_SEARCH_ACTIVE_TOPICS'=> append_sid("{$src_root_path}search.$phpEx", 'search_id=active_topics'),
		'U_DELETE_COOKIES'		=> append_sid("{$src_root_path}ucp.$phpEx", 'mode=delete_cookies'),
		'U_CONTACT_US'			=> ($config['contact_admin_form_enable'] && $config['email_enable']) ? append_sid("{$src_root_path}memberlist.$phpEx", 'mode=contactadmin') : '',
		'U_TEAM'				=> ($user->data['user_id'] != ANONYMOUS && !$auth->acl_get('u_viewprofile')) ? '' : append_sid("{$src_root_path}memberlist.$phpEx", 'mode=team'),
		'U_TERMS_USE'			=> append_sid("{$src_root_path}ucp.$phpEx", 'mode=terms'),
		'U_PRIVACY'				=> append_sid("{$src_root_path}ucp.$phpEx", 'mode=privacy'),
		'U_RESTORE_PERMISSIONS'	=> ($user->data['user_perm_from'] && $auth->acl_get('a_switchperm')) ? append_sid("{$src_root_path}ucp.$phpEx", 'mode=restore_perm') : '',
		'U_FEED'				=> generate_srcrd_url() . "/feed.$phpEx",

		'S_USER_LOGGED_IN'		=> ($user->data['user_id'] != ANONYMOUS) ? true : false,
		'S_AUTOLOGIN_ENABLED'	=> ($config['allow_autologin']) ? true : false,
		'S_srcRD_DISABLED'		=> ($config['srcrd_disable']) ? true : false,
		...
		'S_LOGIN_ACTION'		=> ((!defined('ADMIN_START')) ? append_sid("{$src_root_path}ucp.$phpEx", 'mode=login') : append_sid("{$src_admin_path}index.$phpEx", false, true, $user->session_id)),
		'S_LOGIN_REDIRECT'		=> build_hidden_fields(array('redirect' => $src_path_helper->remove_web_root_path(build_url()))),
	...
	// An array of http headers that src will set. The following event may override these.
	$http_headers = array(
	...
	*									session_forum_id
	* @var	int		item_id				Restrict online users to item id
	* @var	array		http_headers			HTTP headers that should be set by src
	*
	* @since 3.1.0-b3
	*/
	$vars = array('page_title', 'display_online_list', 'item_id', 'item', 'http_headers');
	extract($src_dispatcher->trigger_event('core.page_header_after', compact($vars)));
...
{
	global $db, $config, $template, $user, $auth, $cache, $starttime, $src_root_path, $phpEx;
	global $request, $src_dispatcher, $src_admin_path;
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.page_footer', compact($vars)));
	...
	src_check_and_display_sql_report($request, $auth, $db);

	$template->assign_vars(array(
		'DEBUG_OUTPUT'			=> src_generate_debug_output($db, $config, $auth, $user, $src_dispatcher),
		'TRANSLATION_INFO'		=> (!empty($user->lang['TRANSLATION_INFO'])) ? $user->lang['TRANSLATION_INFO'] : '',
		'CREDIT_LINE'			=> $user->lang('POWERED_BY', 'SourceFlan'),

		'U_ACP' => ($auth->acl_get('a_') && !empty($user->data['is_registered'])) ? append_sid("{$src_admin_path}index.$phpEx", false, true, $user->session_id) : '')
	);

	// Call cron-type script
	$call_cron = false;
	if (!defined('IN_CRON') && !$config['use_system_cron'] && $run_cron && !$config['srcrd_disable'] && !$user->data['is_bot'] && !$cache->get('_cron.lock_check'))
	{
		$call_cron = true;
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.page_footer_after', compact($vars)));
...
function garbage_collection()
{
	...
	if (!empty($src_dispatcher))
	{
		...
		* @since 3.1.0-a1
		*/
		$src_dispatcher->dispatch('core.garbage_collection');
	}
...
{
	global $src_hook, $config;

	if (!empty($src_hook) && $src_hook->call_hook(__FUNCTION__))
	{
		if ($src_hook->hook_return(__FUNCTION__))
		{
			return $src_hook->hook_return_result(__FUNCTION__);
...
/**
* Handler for init calls in src. This function is called in \src\user::setup();
* This function supports hooks.
*/
function src_user_session_handler()
{
	global $src_hook;
...
*/
function src_pcre_utf8_support()
{
...
*/
function src_to_numeric($input)
{
	return ($input > PHP_INT_MAX) ? (float) $input : (int) $input;
}

/**
* Get the srcrd contact details (e.g. for emails)
*
* @param \src\config\config	$config
* @param string					$phpEx
* @return string
*/
function src_get_srcrd_contact(\src\config\config $config, $phpEx)
{
	if ($config['contact_admin_form_enable'])
	{
		return generate_srcrd_url() . '/memberlist.' . $phpEx . '?mode=contactadmin';
	}
	else
	{
		return $config['srcrd_contact'];
	}
}

/**
* Get a clickable srcrd contact details link
*
* @param \src\config\config	$config
...
* @return string
*/
function src_get_srcrd_contact_link(\src\config\config $config, $src_root_path, $phpEx)
{
	if ($config['contact_admin_form_enable'] && $config['email_enable'])
	{
		return append_sid("{$src_root_path}memberlist.$phpEx", 'mode=contactadmin');
	}
	else
	{
		return 'mailto:' . htmlspecialchars($config['srcrd_contact']);
	}
}
