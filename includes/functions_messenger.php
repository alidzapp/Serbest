...
class messenger
{
	...
	*/
	...
	{
		$this->headers('X-AntiAbuse: srcrd servername - ' . mail_encode($config['server_name']));
	...
	*/
	function send($method = NOTIFY_EMAIL, $break = false)
	{
		...
		$this->assign_vars(array(
			'U_srcRD'	=> generate_srcrd_url(),
			'EMAIL_SIG'	=> str_replace('<br />', "\n", "-- \n" . htmlspecialchars_decode($config['srcrd_email_sig'])),
	...
	*/
	...
	{
		global $user, $phpEx, $src_root_path, $config, $request;
	...
	*/
	function generate_message_id()
	{
		...
		$domain = ($config['server_name']) ?: $request->server('SERVER_NAME', 'src.generated');
	...
	*/
	function build_header($to, $cc, $bcc)
	{
		...
		$headers[] = 'Return-Path: <' . $config['srcrd_email'] . '>';
		$headers[] = 'Sender: <' . $config['srcrd_email'] . '>';
		...
		$headers[] = 'X-Mailer: src3';
		$headers[] = 'X-MimeOLE: src3';
		$headers[] = 'X-src-Origin: src://' . str_replace(array('http://', 'https://'), array('', ''), generate_srcrd_url());
	...
	*/
	function msg_email()
	{
		...
		$contact_name = htmlspecialchars_decode($config['srcrd_contact_name']);
		$srcrd_contact = (($contact_name !== '') ? '"' . mail_encode($contact_name) . '" ' : '') . '<' . $config['srcrd_contact'] . '>';

		if (empty($this->replyto))
		{
			$this->replyto = $srcrd_contact;
		}

		if (empty($this->from))
		{
			$this->from = $srcrd_contact;
		}
		...
		if (!$use_queue)
		{
			...
			{
				$result = src_mail($mail_to, $this->subject, $this->msg, $headers, $this->eol, $err_msg);
			}
	...
	*/
	function msg_jabber()
	{
		global $config, $db, $user, $src_root_path, $phpEx;
		...
		if (!$use_queue)
		{
			include_once($src_root_path . 'includes/functions_jabber.' . $phpEx);
	...
	*/
	...
	{
		global $config, $src_path_helper, $user, $src_extension_manager;

		if ($this->template instanceof \src\template\template)
		{
			return;
		}

		$this->template = new \src\template\twig\twig($src_path_helper, $config, $user, new \src\template\context(), $src_extension_manager);
	}
...
class queue
{
	...
	*/
	function queue()
	{
		...
		$this->data = array();
		$this->cache_file = "{$src_root_path}cache/queue.$phpEx";
	...
	*/
	function process()
	{
		global $db, $config, $phpEx, $src_root_path, $user;

		$lock = new \src\lock\flock($this->cache_file);
		$lock->acquire();
			...
			for ($i = 0; $i < $num_items; $i++)
			{
				...
				switch ($object)
				{
					case 'email':
						...
						{
							$result = src_mail($to, $subject, $msg, $headers, $this->eol, $err_msg);
						}
		...
		else
		{
			if ($fp = @fopen($this->cache_file, 'wb'))
			{
				...
				src_chmod($this->cache_file, CHMOD_READ | CHMOD_WRITE);
			}
		}
...
function smtpmail($addresses, $subject, $message, &$err_msg, $headers = false)
{
	...
	if (!class_exists('\src\error_collector'))
	{
		global $src_root_path, $phpEx;
		include($src_root_path . 'includes/error_collector.' . $phpEx);
	}
	$collector = new \src\error_collector;
	$collector->install();
	...
	$smtp->server_send('MAIL FROM:<' . $config['srcrd_email'] . '>');
...
class smtp_class
{
	...
	*/
	function log_into_server($hostname, $username, $password, $default_auth_method)
	{
		...
		// long that the first email send does not work. This is not a biggie on a live srcrd (only
...
*/
function src_mail($to, $subject, $msg, $headers, $eol, &$err_msg)
{
	global $config, $src_root_path, $phpEx;
