...
/**
* Convert the name of a user's primary group to the appropriate equivalent src group id
*
...
/**
* Convert a boolean into the appropriate src constant indicating whether the item is locked
*/
...
/**
* Convert a boolean into the appropriate src constant indicating whether the user is active
*/
...
*
* Note: Deprecated, calls should directly go to src_email_hash()
*/
function gen_email_hash($email)
{
	return src_email_hash($email);
}

/**
* Convert a boolean into the appropriate src constant indicating whether the topic is locked
*/
...
{
	global $config, $convert, $src_root_path, $user;
...
{
	global $config, $convert, $src_root_path, $db, $user;
...
function _import_check($config_var, $source, $use_target)
{
	...
	// copy file will prepend $src_root_path
...
function import_attachment($source, $use_target = false)
{
	...
	global $convert, $src_root_path, $config, $user;
...
{
	global $convert, $src_root_path, $config, $user, $db, $src_db, $same_db;
...
* (the type used is specified in $config_schema, see convert_src20.php for more details)
*/
function get_config()
{
	static $convert_config;
	...
	global $src_db, $same_db, $src_root_path, $config;
	global $convert;
...
* The mapping of fields is specified in $config_schema, see convert_src20.php for more details
*/
...
function get_path($src_path, $src_url, $test_file)
{
	...
	$srcrd_config = get_config();

	$test_file = preg_replace('/\.php$/i', ".$phpEx", $test_file);
	$src_path = path($src_path);

	if (@file_exists($src_root_path . $src_path . $test_file))
	{
		return $src_path;
	}

	if (!empty($src_url) && !empty($srcrd_config['server_name']))
	{
		...
		if ($m[1] != $srcrd_config['server_name'])
		{
			return false;
		}
		...
		$script_path = $srcrd_config['script_path'];
		...
		$src_parts = explode('/', $script_path);
		for ($i = 0; $i < sizeof($url_parts); ++$i)
		{
			if ($i < sizeof($src_parts[$i]) && $url_parts[$i] == $src_parts[$i])
			{
		...
		if (!empty($path))
		{
			if (@file_exists($src_root_path . $path . $test_file))
			{
				return $path;
			}
		}
...
function add_bots()
{
	global $db, $convert, $user, $config, $src_root_path, $phpEx;
...
{
	global $convert, $db, $phpEx, $src_root_path;
...
function copy_file($src, $trg, $overwrite = false, $die_on_failure = true, $source_relative_path = true)
{
	...
	if (!src_is_writable($path))
	{
		@chmod($path, 0777);
	}

	if (!@copy($src_path, $src_root_path . $trg_path))
	{
		$convert->p_master->error(sprintf($user->lang['COULD_NOT_COPY'], $src_path, $src_root_path . $trg_path), __LINE__, __FILE__, !$die_on_failure);
		return;
	}

	if ($perm = @fileperms($src_path))
	{
		@chmod($src_root_path . $trg_path, $perm);
	}
