...
function display_forums($root_data = '', $display_moderators = true, $return_moderators = false)
{
	...
	global $request, $src_dispatcher, $src_container;
		...
		if (check_link_hash(request_var('hash', ''), 'global'))
		{
			...
			if ($request->is_ajax())
			{
				...
				$data = array(
					...
					'U_MARK_FORUMS'		=> ($user->data['is_registered'] || $config['load_anon_lastread']) ? append_sid("{$src_root_path}index.$phpEx", 'hash=' . generate_link_hash('global') . '&mark=forums&mark_time=' . time()) : '',
	...
	*/
	$vars = array('sql_ary');
	extract($src_dispatcher->trigger_event('core.display_forums_modify_sql', compact($vars)));
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.display_forums_modify_row', compact($vars)));
		...
		else if ($config['load_anon_lastread'] || $user->data['is_registered'])
		{
			...
			$forum_tracking_info[$forum_id] = (isset($tracking_topics['f'][$forum_id])) ? (int) (base_convert($tracking_topics['f'][$forum_id], 36, 10) + $config['srcrd_startdate']) : $user->data['user_lastmark'];
		}
		...
		$row['forum_posts'] = $src_content_visibility->get_count('forum_posts', $row, $forum_id);
		$row['forum_topics'] = $src_content_visibility->get_count('forum_topics', $row, $forum_id);
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.display_forums_modify_forum_rows', compact($vars)));
	}
		...
		if (check_link_hash($token, 'global'))
		{
			...
			if ($request->is_ajax())
			{
				...
				$data = array(
					'NO_UNREAD_POSTS'	=> $user->lang['NO_UNREAD_POSTS'],
					'UNREAD_POSTS'		=> $user->lang['UNREAD_POSTS'],
					'U_MARK_FORUMS'		=> ($user->data['is_registered'] || $config['load_anon_lastread']) ? append_sid("{$src_root_path}viewforum.$phpEx", 'hash=' . generate_link_hash('global') . '&f=' . $root_data['forum_id'] . '&mark=forums&mark_time=' . time()) : '',
					'MESSAGE_TITLE'		=> $user->lang['INFORMATION'],
					'MESSAGE_TEXT'		=> $user->lang['FORUMS_MARKED']
				);
				$json_response = new \src\json_response();
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.display_forums_before', compact($vars)));
	...
	foreach ($forum_rows as $row)
	{
		...
		if ($row['parent_id'] == $root_data['forum_id'] && $row['forum_type'] == FORUM_CAT)
		{
			...
			$cat_row = array(
				...
				'FORUM_IMAGE'			=> ($row['forum_image']) ? '<img src="' . $src_root_path . $row['forum_image'] . '" alt="' . $user->lang['FORUM_CAT'] . '" />' : '',
				'FORUM_IMAGE_SRC'		=> ($row['forum_image']) ? $src_root_path . $row['forum_image'] : '',
				'U_VIEWFORUM'			=> append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $row['forum_id']),
			);
			...
			*/
			...
			extract($src_dispatcher->trigger_event('core.display_forums_modify_category_template_vars', compact($vars)));
		...
		if (isset($subforums[$forum_id]))
		{
			foreach ($subforums[$forum_id] as $subforum_id => $subforum_row)
			{
				...
				{
					$subforums_list[] = array(
						'link'		=> append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $subforum_id),
		...
		if ($row['forum_last_post_id'])
		{
			...
			$last_post_url = append_sid("{$src_root_path}viewtopic.$phpEx", 'f=' . $row['forum_id_last_post'] . '&amp;p=' . $row['forum_last_post_id']) . '#p' . $row['forum_last_post_id'];
		}
		else
		{
		...
		$forum_row = array(
			...
			'S_FEED_ENABLED'	=> ($config['feed_forum'] && !src_optionget(FORUM_OPTION_FEED_EXCLUDE, $row['forum_options']) && $row['forum_type'] == FORUM_POST) ? true : false,
			...
			'FORUM_IMAGE'			=> ($row['forum_image']) ? '<img src="' . $src_root_path . $row['forum_image'] . '" alt="' . $user->lang[$folder_alt] . '" />' : '',
			'FORUM_IMAGE_SRC'		=> ($row['forum_image']) ? $src_root_path . $row['forum_image'] : '',
			...
			'U_UNAPPROVED_TOPICS'	=> ($row['forum_id_unapproved_topics']) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=queue&amp;mode=unapproved_topics&amp;f=' . $row['forum_id_unapproved_topics']) : '',
			'U_UNAPPROVED_POSTS'	=> ($row['forum_id_unapproved_posts']) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=queue&amp;mode=unapproved_posts&amp;f=' . $row['forum_id_unapproved_posts']) : '',
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.display_forums_modify_template_vars', compact($vars)));
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.display_forums_add_template_data', compact($vars)));
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.display_forums_after', compact($vars)));
...
function generate_forum_rules(&$forum_data)
{
	...
	global $template, $src_root_path, $phpEx;
...
function generate_forum_nav(&$forum_data)
{
	...
	if (!empty($forum_parents))
	{
		foreach ($forum_parents as $parent_forum_id => $parent_data)
		{
			...
			$navlinks_parents[] = array(
				...
				'U_VIEW_FORUM'	=> append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $parent_forum_id),
			);
		}
	}
	...
	$navlinks = array(
		...
		'U_VIEW_FORUM'	=> append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $forum_data['forum_id']),
	);
	...
	$forum_template_data = array(
		...
		'S_ENABLE_FEEDS_FORUM'	=> ($config['feed_forum'] && $forum_data['forum_type'] == FORUM_POST && !src_optionget(FORUM_OPTION_FEED_EXCLUDE, $forum_data['forum_options'])) ? true : false,
	);
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.generate_forum_nav', compact($vars)));
...
{
	global $config, $template, $db, $src_root_path, $phpEx, $user, $auth;
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		else
		{
			...
			{
				$forum_moderators[$f_id][] = '<a' . (($row['group_colour']) ? ' style="color:#' . $row['group_colour'] . ';"' : '') . ' href="' . append_sid("{$src_root_path}memberlist.$phpEx", 'mode=group&amp;g=' . $row['group_id']) . '">' . $group_name . '</a>';
			}
		}
	}
	$db->sql_freeresult($result);
...
{
	global $db, $template, $user, $src_dispatcher;
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.display_custom_bbcodes_modify_sql', compact($vars)));
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.display_custom_bbcodes_modify_row', compact($vars)));
	...
	* @since 3.1.0-a1
	*/
	$src_dispatcher->dispatch('core.display_custom_bbcodes');
}
...
function display_user_activity(&$userdata)
{
	...
	global $src_container, $src_dispatcher;
	...
	if (!empty($forum_ary))
	{
		...
		$sql = 'SELECT forum_id, COUNT(post_id) AS num_posts
			...
			WHERE poster_id = ' . $userdata['user_id'] . '
				...
				AND ' . $src_content_visibility->get_forums_visibility_sql('post', $forum_ary) . '
			GROUP BY forum_id
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.display_user_activity_modify_actives', compact($vars)));
	...
	$template->assign_vars(array(
		...
		'U_ACTIVE_FORUM'		=> append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $active_f_id),
		'U_ACTIVE_TOPIC'		=> append_sid("{$src_root_path}viewtopic.$phpEx", 't=' . $active_t_id),
...
{
	global $template, $db, $user, $phpEx, $start, $src_root_path;
	global $request;
	...
	if ($user_id != ANONYMOUS)
	{
		...
		if (!is_null($notify_status) && $notify_status !== '')
		{
			if (isset($_GET['unwatch']))
			{
				...
				{
					if ($uid != $user_id || $request->variable('unwatch', '', false, \src\request\request_interface::GET) != $mode)
					{
						$redirect_url = append_sid("{$src_root_path}view$mode.$phpEx", "$u_url=$match_id&amp;start=$start");
		...
		else
		{
			if (isset($_GET['watch']))
			{
				...
				if ($token && check_link_hash($token, "{$mode}_$match_id") || confirm_box(true))
				{
					if ($uid != $user_id || $request->variable('watch', '', false, \src\request\request_interface::GET) != $mode)
					{
						$redirect_url = append_sid("{$src_root_path}view$mode.$phpEx", "$u_url=$match_id&amp;start=$start");
	...
	{
		if ((isset($_GET['unwatch']) && $request->variable('unwatch', '', false, \src\request\request_interface::GET) == $mode) ||
			(isset($_GET['watch']) && $request->variable('watch', '', false, \src\request\request_interface::GET) == $mode))
		{
			login_box();
		}
		else
	...
	if ($can_watch)
	{
		$s_watching['link'] = append_sid("{$src_root_path}view$mode.$phpEx", "$u_url=$match_id&amp;" . (($is_watching) ? 'unwatch' : 'watch') . "=$mode&amp;start=$start&amp;hash=" . generate_link_hash("{$mode}_$match_id"));
		$s_watching['link_toggle'] = append_sid("{$src_root_path}view$mode.$phpEx", "$u_url=$match_id&amp;" . ((!$is_watching) ? 'unwatch' : 'watch') . "=$mode&amp;start=$start&amp;hash=" . generate_link_hash("{$mode}_$match_id"));
...
*/
function src_get_user_rank($user_data, $user_posts)
{
	global $ranks, $config, $src_root_path, $src_path_helper, $src_dispatcher;
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.modify_user_rank', compact($vars)));
	...
	if (!empty($user_data['user_rank']))
	{
		...
		$user_rank_data['img_src'] = (!empty($ranks['special'][$user_data['user_rank']]['rank_image'])) ? $src_path_helper->update_web_root_path($src_root_path . $config['ranks_path'] . '/' . $ranks['special'][$user_data['user_rank']]['rank_image']) : '';
		...
		if (!empty($ranks['normal']))
		{
			foreach ($ranks['normal'] as $rank)
			{
				if ($user_posts >= $rank['rank_min'])
				{
					...
					$user_rank_data['img_src'] = (!empty($rank['rank_image'])) ? $src_path_helper->update_web_root_path($src_root_path . $config['ranks_path'] . '/' . $rank['rank_image']) : '';
...
function src_show_profile($data, $user_notes_enabled = false, $warn_user_enabled = false, $check_can_receive_pm = true)
{
	...
	$can_receive_pm = $check_can_receive_pm && (
		...
		!sizeof(src_get_banned_user_ids($user_id, false)) &&
	...
	$template_data = array(
		...
		'AVATAR_IMG'		=> src_get_user_avatar($data),
		...
		'U_SEARCH_USER' => ($auth->acl_get('u_search')) ? append_sid("{$src_root_path}search.$phpEx", "author_id=$user_id&amp;sr=posts") : '',
		'U_NOTES'		=> ($user_notes_enabled && $auth->acl_getf_global('m_')) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=notes&amp;mode=user_notes&amp;u=' . $user_id, true, $user->session_id) : '',
		'U_WARN'		=> ($warn_user_enabled && $auth->acl_get('m_warn')) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=warn&amp;mode=warn_user&amp;u=' . $user_id, true, $user->session_id) : '',
		'U_PM'			=> ($config['allow_privmsg'] && $auth->acl_get('u_sendpm') && $can_receive_pm) ? append_sid("{$src_root_path}ucp.$phpEx", 'i=pm&amp;mode=compose&amp;u=' . $user_id) : '',
		'U_EMAIL'		=> $email,
		'U_JABBER'		=> ($data['user_jabber'] && $auth->acl_get('u_sendim')) ? append_sid("{$src_root_path}memberlist.$phpEx", 'mode=contact&amp;action=jabber&amp;u=' . $user_id) : '',
...
function src_sort_last_active($first, $second)
{
