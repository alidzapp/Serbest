...
class utf_normalizer
{
	...
	*/
	static function cleanup(&$str)
	{
		...
		if (!isset($GLOBALS['utf_nfc_qc']))
		{
			...
			include($src_root_path . 'includes/utf/data/utf_nfc_qc.' . $phpEx);
		}

		if (!isset($GLOBALS['utf_canonical_decomp']))
		{
			global $src_root_path, $phpEx;
			include($src_root_path . 'includes/utf/data/utf_canonical_decomp.' . $phpEx);
		}
	...
	*/
	static function nfkc(&$str)
	{
		...
		{
			global $src_root_path, $phpEx;
			include($src_root_path . 'includes/utf/data/utf_nfkc_qc.' . $phpEx);
		}

		if (!isset($GLOBALS['utf_compatibility_decomp']))
		{
			global $src_root_path, $phpEx;
			include($src_root_path . 'includes/utf/data/utf_compatibility_decomp.' . $phpEx);
		}
	...
	*/
	static function recompose($str, $pos, $len, &$qc, &$decomp_map)
	{
		...
		{
			global $src_root_path, $phpEx;
			include($src_root_path . 'includes/utf/data/utf_normalizer_common.' . $phpEx);
		}
		...
		{
			global $src_root_path, $phpEx;
			include($src_root_path . 'includes/utf/data/utf_canonical_comp.' . $phpEx);
		}
