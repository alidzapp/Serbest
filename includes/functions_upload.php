...
class filespec
{
	...
	* @access private
	*/
	function filespec($upload_ary, $upload_namespace, \src\mimetype\guesser $mimetype_guesser = null, \src\plupload\plupload $plupload = null)
	{
		if (!isset($upload_ary))
		{
	...
	* The src_root_path variable will be applied to the destination path
	*
	...
	* @param string $chmod Permission mask for chmodding the file after a successful move. The mode entered here reflects the mode defined by {@link src_chmod()}
	*
	* @access public
	*/
	function move_file($destination, $overwrite = false, $skip_image_check = false, $chmod = false)
	{
		...
		$this->destination_path = $src_root_path . $destination;
		...
		else
		{
			...
			src_chmod($this->destination_file, $chmod);
		}
...
class fileupload
{
	...
	*/
	...
	/**
	* Move file from another location to src
	*/
	function local_upload($source_file, $filedata = false, \src\mimetype\guesser $mimetype_guesser = null)
	{
		global $user, $request;
		...
		$this->common_checks($file);
		$request->overwrite('local', $upload, \src\request\request_interface::FILES);
	...
	* @access public
	*/
	function remote_upload($upload_url, \src\mimetype\guesser $mimetype_guesser = null)
	{
		global $user, $src_root_path;
		...
		$tmp_path = (!@ini_get('safe_mode') || strtolower(@ini_get('safe_mode')) == 'off') ? false : $src_root_path . 'cache';
