...
{
	global $db, $user, $config, $template, $src_dispatcher;
	global $phpEx, $src_root_path, $src_container, $src_path_helper;

	$base_url = append_sid("{$src_root_path}posting.$phpEx", 'mode=smilies&amp;f=' . $forum_id);
	...
	if (sizeof($smilies))
	{
		$root_path = (defined('src_USE_srcRD_URL_PATH') && src_USE_srcRD_URL_PATH) ? generate_srcrd_url() . '/' : $src_path_helper->get_web_root_path();
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.generate_smilies_after', compact($vars)));
...
{
	global $src_root_path, $config, $template, $cache;
	...
	if (sizeof($icons))
	{
		$root_path = (defined('src_USE_srcRD_URL_PATH') && src_USE_srcRD_URL_PATH) ? generate_srcrd_url() . '/' : $src_root_path;
...
* @param \src\mimetype\guesser	$mimetype_guesser	The mimetype guesser object if used
* @param \src\plupload\plupload	$plupload		The plupload object if one is being used
*
* @return object filespec
*/
function upload_attachment($form_name, $forum_id, $local = false, $local_storage = '', $is_message = false, $local_filedata = false, \src\mimetype\guesser $mimetype_guesser = null, \src\plupload\plupload $plupload = null)
{
	global $auth, $user, $config, $db, $cache;
	global $src_root_path, $phpEx, $src_dispatcher;

	$filedata = array(
		'error'	=> array()
	);

	include_once($src_root_path . 'includes/functions_upload.' . $phpEx);
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.modify_uploaded_file', compact($vars)));
	...
	if ($free_space = @disk_free_space($src_root_path . $config['upload_path']))
	{
...
function create_thumbnail($source, $destination, $mimetype)
{
	...
	src_chmod($destination, CHMOD_READ | CHMOD_WRITE);
...
{
	global $template, $config, $src_root_path, $phpEx, $user;
	...
	if (sizeof($attachment_data))
	{
		...
		foreach ($attachment_data as $count => $attach_row)
		{
			...
			$download_link = append_sid("{$src_root_path}download/file.$phpEx", 'mode=view&amp;id=' . (int) $attach_row['attach_id'], true, ($attach_row['is_orphan']) ? $user->session_id : false);
...
function load_drafts($topic_id = 0, $forum_id = 0, $id = 0, $pm_action = '', $msg_id = 0)
{
	...
	global $src_root_path, $src_dispatcher, $phpEx;
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.load_drafts_draft_list_result', compact($vars)));
	...
	foreach ($draft_rows as $draft)
	{
			...
			))
		{
			...
			$link_topic = true;
			$view_url = append_sid("{$src_root_path}viewtopic.$phpEx", 'f=' . $topic_forum_id . '&amp;t=' . $draft['topic_id']);
			$title = $topic_rows[$draft['topic_id']]['topic_title'];

			$insert_url = append_sid("{$src_root_path}posting.$phpEx", 'f=' . $topic_forum_id . '&amp;t=' . $draft['topic_id'] . '&amp;mode=reply&amp;d=' . $draft['draft_id']);
		}
		else if ($draft['forum_id'] && $auth->acl_get('f_read', $draft['forum_id']))
		{
			$link_forum = true;
			$view_url = append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $draft['forum_id']);
			$title = $draft['forum_name'];

			$insert_url = append_sid("{$src_root_path}posting.$phpEx", 'f=' . $draft['forum_id'] . '&amp;mode=post&amp;d=' . $draft['draft_id']);
		}
		else
		{
			// Either display as PM draft if forum_id and topic_id are empty or if access to the forums has been denied afterwards...
			$link_pm = true;
			$insert_url = append_sid("{$src_root_path}ucp.$phpEx", "i=$id&amp;mode=compose&amp;d={$draft['draft_id']}" . (($pm_action) ? "&amp;action=$pm_action" : '') . (($msg_id) ? "&amp;p=$msg_id" : ''));
		}
...
function topic_review($topic_id, $forum_id, $mode = 'topic_review', $cur_post_id = 0, $show_quote_button = true)
{
	...
	global $config, $src_root_path, $phpEx, $src_container, $src_dispatcher;
	...
	for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
	{
		...
		$u_show_post = append_sid($src_root_path . 'viewtopic.' . $phpEx, "f=$forum_id&amp;t=$topic_id&amp;p={$row['post_id']}&amp;view=show#p{$row['post_id']}");
		...
		$post_row = array(
			...
			'L_IGNORE_POST'		=> ($row['foe']) ? sprintf($user->lang['POST_BY_FOE'], get_username_string('full', $poster_id, $row['username'], $row['user_colour'], $row['post_username']), "<a href=\"{$u_show_post}\" onclick=\"src.toggleDisplay('{$post_anchor}', 1); return false;\">", '</a>') : '',
			...
			'U_MCP_DETAILS'		=> ($auth->acl_get('m_info', $forum_id)) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=main&amp;mode=post_details&amp;f=' . $forum_id . '&amp;p=' . $row['post_id'], true, $user->session_id) : '',
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.topic_review_modify_row', compact($vars)));
...
{
	global $db, $user, $auth, $src_container;
	global $config, $phpEx, $src_root_path;
	...
	include_once($src_root_path . 'includes/functions_admin.' . $phpEx);
	...
	{
		$src_content_visibility->set_post_visibility(ITEM_DELETED, $post_id, $topic_id, $forum_id, $user->data['user_id'], time(), $softdelete_reason, ($data['topic_first_post_id'] == $post_id), ($data['topic_last_post_id'] == $post_id));
	}
	else if (!$is_soft)
	{
	...
	switch ($post_mode)
	{
		case 'delete_topic':
			...
			{
				$topic_row = array();
				$src_content_visibility->set_topic_visibility(ITEM_DELETED, $topic_id, $forum_id, $user->data['user_id'], time(), $softdelete_reason);
			}
			else
			{
				delete_topics('topic_id', array($topic_id), false);

				$src_content_visibility->remove_topic_from_statistic($data, $sql_data);
	...
	if (($post_mode == 'delete') || ($post_mode == 'delete_last_post') || ($post_mode == 'delete_first_post'))
	{
		if (!$is_soft)
		{
			$src_content_visibility->remove_post_from_statistic($data, $sql_data);
		}
...
{
	global $db, $auth, $user, $config, $phpEx, $template, $src_root_path, $src_container, $src_dispatcher;
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.modify_submit_post_data', compact($vars)));
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.submit_post_modify_sql_data', compact($vars)));
	...
	if (!empty($data['attachment_data']) && $data['post_id'] && in_array($mode, array('post', 'reply', 'quote', 'edit')))
	{
		...
		foreach ($data['attachment_data'] as $pos => $attach_row)
		{
			...
			else
			{
				...
				if (!@file_exists($src_root_path . $config['upload_path'] . '/' . utf8_basename($orphan_rows[$attach_row['attach_id']]['physical_filename'])))
				{
					continue;
				}
	...
	if (($post_mode != 'post' && $post_mode != 'reply') && $data['post_visibility'] != $post_visibility)
	{
		...
		$src_content_visibility = $src_container->get('content.visibility');
		$src_content_visibility->set_post_visibility($post_visibility, $data['post_id'], $data['topic_id'], $data['forum_id'], $user->data['user_id'], time(), '', $is_starter, $is_latest);
	}
	...
	if ($post_visibility == ITEM_APPROVED)
	{
		...
		{
			case 'post':
				$src_notifications->add_notifications(array(
			...
			case 'edit':
			case 'edit_last_post':
				$src_notifications->update_notifications(array(
					'notification.type.quote',
					'notification.type.bookmark',
					'notification.type.topic',
					'notification.type.post',
	...
	else if ($post_visibility == ITEM_UNAPPROVED)
	{
		...
		{
			case 'post':
				$src_notifications->add_notifications('notification.type.topic_in_queue', $notification_data);
			break;

			case 'reply':
			case 'quote':
				$src_notifications->add_notifications('notification.type.post_in_queue', $notification_data);
			break;
	...
	else if ($post_visibility == ITEM_REAPPROVE)
	{
		switch ($mode)
		{
			...
			case 'edit_first_post':
				...
				$src_notifications->delete_notifications('notification.type.approve_post', $notification_data['post_id']);
			break;
	...
	$url = (!$params) ? "{$src_root_path}viewforum.$phpEx" : "{$src_root_path}viewtopic.$phpEx";
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.submit_post_end', compact($vars)));
...
*/
function src_bump_topic($forum_id, $topic_id, $post_data, $bump_time = false)
{
	global $config, $db, $user, $phpEx, $src_root_path;
	...
	$url = append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id&amp;p={$post_data['topic_last_post_id']}") . "#p{$post_data['topic_last_post_id']}";
...
*/
function src_upload_popup($forum_style = 0)
{
...
*/
function src_handle_post_delete($forum_id, $topic_id, $post_id, &$post_data, $is_soft = false, $delete_reason = '')
{
	...
	if ($auth->acl_get("m_$perm_check", $forum_id) || ($post_data['poster_id'] == $user->data['user_id'] && $user->data['is_registered'] && $auth->acl_get("f_$perm_check", $forum_id) && $post_id == $post_data['topic_last_post_id'] && !$post_data['post_edit_locked'] && ($post_data['post_time'] > time() - ($config['delete_time'] * 60) || !$config['delete_time'])))
	{
		...
		if (confirm_box(true))
		{
			...
			if ($next_post_id === false)
			{
				...
				$meta_info = append_sid("{$src_root_path}viewforum.$phpEx", "f=$forum_id");
			...
			else
			{
				...
				$meta_info = append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id&amp;p=$next_post_id") . "#p$next_post_id";
				$message = $user->lang['POST_DELETED'];
			...
			if (!$request->is_ajax())
			{
				$message .= '<br /><br />' . $user->lang('RETURN_FORUM', '<a href="' . append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $forum_id) . '">', '</a>');
			}
