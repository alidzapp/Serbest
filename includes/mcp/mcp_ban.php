...
class mcp_ban
{
	...
	{
		global $db, $user, $auth, $template, $request, $src_dispatcher;
		global $src_root_path, $phpEx;

		include($src_root_path . 'includes/functions_user.' . $phpEx);

		// Include the admin banning interface...
		include($src_root_path . 'includes/acp/acp_ban.' . $phpEx);
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.mcp_ban_main', compact($vars)));
			...
			if ($ban)
			{
				if (confirm_box(true))
				{
					...
					*/
					...
					extract($src_dispatcher->trigger_event('core.mcp_ban_before', compact($vars)));
					...
					*/
					...
					extract($src_dispatcher->trigger_event('core.mcp_ban_after', compact($vars)));
				...
				else
				{
					...
					*/
					...
					extract($src_dispatcher->trigger_event('core.mcp_ban_confirm', compact($vars)));
		...
		$template->assign_vars(array(
			...
			'U_FIND_USERNAME'	=> append_sid("{$src_root_path}memberlist.$phpEx", 'mode=searchuser&amp;form=mcp_ban&amp;field=ban'),
		));
		...
		{
			$post_info = src_get_post_data($post_id, 'm_ban');
