...
class mcp_main
{
	...
	function main($id, $mode)
	{
		...
		global $config, $src_root_path, $phpEx, $request;
		...
		switch ($action)
		{
			...
			default:
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.modify_quickmod_actions', compact($vars)));
			break;
		}
		...
		{
			case 'front':
				include($src_root_path . 'includes/mcp/mcp_front.' . $phpEx);
			...
			case 'forum_view':
				...
				$forum_info = src_get_forum_data($forum_id, 'm_', true);
			...
			case 'topic_view':
				include($src_root_path . 'includes/mcp/mcp_topic.' . $phpEx);
			...
			case 'post_details':
				include($src_root_path . 'includes/mcp/mcp_post.' . $phpEx);
...
{
	global $auth, $user, $db, $phpEx, $src_root_path, $request;
	...
	if (!src_check_ids($ids, $table, $sql_id, array('m_lock')))
	{
		...
		if (!src_check_ids($ids, $table, $sql_id, array('f_user_lock')))
		{
			return;
		}
	}
	...
	if (confirm_box(true))
	{
		...
		$data = ($action == 'lock' || $action == 'unlock') ? src_get_topic_data($ids) : src_get_post_data($ids);
...
function change_topic_type($action, $topic_ids)
{
	...
	$forum_id = src_check_ids($topic_ids, TOPICS_TABLE, 'topic_id', $check_acl, true);
	...
	if (confirm_box(true))
	{
		...
		if (sizeof($topic_ids))
		{
			$data = src_get_topic_data($topic_ids);
...
{
	global $auth, $user, $db, $template, $src_log, $request;
	global $phpEx, $src_root_path;

	// Here we limit the operation to one forum only
	$forum_id = src_check_ids($topic_ids, TOPICS_TABLE, 'topic_id', array('m_move'), true);
	...
	if (confirm_box(true))
	{
		$topic_data = src_get_topic_data($topic_ids);
		...
		foreach ($topic_data as $topic_id => $row)
		{
			...
			$src_log->add('mod', $user->data['user_id'], $user->ip, 'LOG_MOVE', false, array(
	...
	else
	{
		...
		$message .= '<br /><br />' . sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$src_root_path}viewforum.$phpEx", "f=$forum_id") . '">', '</a>');
		$message .= '<br /><br />' . sprintf($user->lang['RETURN_NEW_FORUM'], '<a href="' . append_sid("{$src_root_path}viewforum.$phpEx", "f=$to_forum_id") . '">', '</a>');
...
{
	global $auth, $user, $db, $phpEx, $src_root_path, $request, $src_container;

	if (!src_check_ids($topic_ids, TOPICS_TABLE, 'topic_id', array('m_approve')))
	{
		return;
	}
	...
	if (confirm_box(true))
	{
		...
		foreach ($data as $topic_id => $row)
		{
			$return = $src_content_visibility->set_topic_visibility(ITEM_APPROVED, $topic_id, $row['forum_id'], $user->data['user_id'], time(), '');
			if (!empty($return))
			{
	...
	if (!$request->is_set('quickmod', \src\request\request_interface::REQUEST))
	{
	...
	{
		$redirect = append_sid("{$src_root_path}viewtopic.$phpEx", 't=' . $topic_id);
		$redirect_message = 'TOPIC';
	}
	else
	{
		$redirect = append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $forum_id);
...
function mcp_delete_topic($topic_ids, $is_soft = false, $soft_delete_reason = '', $action = 'delete_topic')
{
	...
	if (!src_check_ids($topic_ids, TOPICS_TABLE, 'topic_id', array($check_permission)))
	{
		return;
	}
	...
	if (confirm_box(true))
	{
		...
		foreach ($data as $topic_id => $row)
		{
			...
			else
			{
				...
				{
					$src_content_visibility = $src_container->get('content.visibility');
					$return = $src_content_visibility->set_topic_visibility(ITEM_DELETED, $topic_id, $row['forum_id'], $user->data['user_id'], time(), $soft_delete_reason);
					if (!empty($return))
					{
...
function mcp_delete_post($post_ids, $is_soft = false, $soft_delete_reason = '', $action = 'delete_post')
{
	...
	$check_permission = ($is_soft) ? 'm_softdelete' : 'm_delete';
	if (!src_check_ids($post_ids, POSTS_TABLE, 'post_id', array($check_permission)))
	{
		return;
	}
	...
	{
		$post_info = src_get_post_data($post_ids);
		...
		{
			$src_content_visibility->set_post_visibility(ITEM_DELETED, $topic_data['posts'], $topic_id, $topic_data['forum_id'], $user->data['user_id'], time(), $soft_delete_reason, isset($topic_data['first_post']), isset($topic_data['last_post']));
		}
		...
		{
			$return_link[] = sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id") . '">', '</a>');
		}
		$return_link[] = sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $forum_id) . '">', '</a>');
	...
	else if (confirm_box(true))
	{
		...
		$post_data = src_get_post_data($post_ids);
	...
	else
	{
		if ($affected_topics != 1 || $deleted_topics || !$topic_id)
		{
			$redirect = append_sid("{$src_root_path}mcp.$phpEx", "f=$forum_id&i=main&mode=forum_view", false);
		}
