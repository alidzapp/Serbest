...
class mcp_queue
{
	...
	public function main($id, $mode)
	{
		...
		global $config, $src_root_path, $phpEx, $action, $src_container;
			...
			case 'delete':
				...
				if (!empty($post_id_list))
				{
					if (!function_exists('mcp_delete_post'))
					{
						...
						include($src_root_path . 'includes/mcp/mcp_main.' . $phpEx);
					}
		...
		switch ($mode)
		{
			case 'approve_details':
				...
				if ($topic_id)
				{
					$topic_info = src_get_topic_data(array($topic_id), 'm_approve');
					if (isset($topic_info[$topic_id]['topic_first_post_id']))
					{
						$post_id = (int) $topic_info[$topic_id]['topic_first_post_id'];

						$src_notifications->mark_notifications_read('notification.type.topic_in_queue', $topic_id, $user->data['user_id']);
					}
					else
					{
						$topic_id = 0;
				...
				$src_notifications->mark_notifications_read('notification.type.post_in_queue', $post_id, $user->data['user_id']);

				$post_info = src_get_post_data(array($post_id), 'm_approve', true);
				...
				$post_url = append_sid("{$src_root_path}viewtopic.$phpEx", 'f=' . $post_info['forum_id'] . '&amp;p=' . $post_info['post_id'] . '#p' . $post_info['post_id']);
				$topic_url = append_sid("{$src_root_path}viewtopic.$phpEx", 'f=' . $post_info['forum_id'] . '&amp;t=' . $post_info['topic_id']);

				$template->assign_vars(array(
					'S_MCP_QUEUE'			=> true,
					'U_APPROVE_ACTION'		=> append_sid("{$src_root_path}mcp.$phpEx", "i=queue&amp;p=$post_id&amp;f=$forum_id"),
					...
					'U_EDIT'				=> ($auth->acl_get('m_edit', $post_info['forum_id'])) ? append_sid("{$src_root_path}posting.$phpEx", "mode=edit&amp;f={$post_info['forum_id']}&amp;p={$post_info['post_id']}") : '',
					'U_MCP_APPROVE'			=> append_sid("{$src_root_path}mcp.$phpEx", 'i=queue&amp;mode=approve_details&amp;f=' . $post_info['forum_id'] . '&amp;p=' . $post_id),
					'U_MCP_REPORT'			=> append_sid("{$src_root_path}mcp.$phpEx", 'i=reports&amp;mode=report_details&amp;f=' . $post_info['forum_id'] . '&amp;p=' . $post_id),
					'U_MCP_USER_NOTES'		=> append_sid("{$src_root_path}mcp.$phpEx", 'i=notes&amp;mode=user_notes&amp;u=' . $post_info['user_id']),
					'U_MCP_WARN_USER'		=> ($auth->acl_get('m_warn')) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=warn&amp;mode=warn_user&amp;u=' . $post_info['user_id']) : '',
					'U_VIEW_POST'			=> $post_url,
					'U_VIEW_TOPIC'			=> $topic_url,

					'MINI_POST_IMG'			=> ($post_unread) ? $user->img('icon_post_target_unread', 'UNREAD_POST') : $user->img('icon_post_target', 'POST'),

					'RETURN_QUEUE'			=> sprintf($user->lang['RETURN_QUEUE'], '<a href="' . append_sid("{$src_root_path}mcp.$phpEx", 'i=queue' . (($topic_id) ? '&amp;mode=unapproved_topics' : '&amp;mode=unapproved_posts')) . '&amp;start=' . $start . '">', '</a>'),
					...
					'U_LOOKUP_IP'			=> ($auth->acl_get('m_info', $post_info['forum_id'])) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=queue&amp;mode=approve_details&amp;f=' . $post_info['forum_id'] . '&amp;p=' . $post_id . '&amp;lookup=' . $post_info['poster_ip']) . '#ip' : '',
				));
			...
			case 'deleted_posts':
				...
				{
					$forum_info = src_get_forum_data(array($forum_id), $m_perm);
				...
				src_mcp_sorting($mode, $sort_days, $sort_key, $sort_dir, $sort_by_sql, $sort_order_sql, $total, $forum_id, $topic_id);
				...
				if (!$is_topics)
				{
					...
					*/
					...
					extract($src_dispatcher->trigger_event('core.mcp_queue_get_posts_query_before', compact($vars)));
				...
				else
				{
					...
					*/
					...
					extract($src_dispatcher->trigger_event('core.mcp_queue_get_posts_for_topics_query_before', compact($vars)));
				...
				foreach ($rowset as $row)
				{
					...
					$template->assign_block_vars('postrow', array(
						'U_TOPIC'			=> append_sid("{$src_root_path}viewtopic.$phpEx", 'f=' . $row['forum_id'] . '&amp;t=' . $row['topic_id']),
						'U_VIEWFORUM'		=> append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $row['forum_id']),
						'U_VIEWPOST'		=> append_sid("{$src_root_path}viewtopic.$phpEx", 'f=' . $row['forum_id'] . '&amp;p=' . $row['post_id']) . (($mode == 'unapproved_posts') ? '#p' . $row['post_id'] : ''),
						'U_VIEW_DETAILS'	=> append_sid("{$src_root_path}mcp.$phpEx", "i=queue&amp;start=$start&amp;mode=approve_details&amp;f={$row['forum_id']}&amp;p={$row['post_id']}" . (($mode == 'unapproved_topics') ? "&amp;t={$row['topic_id']}" : '')),
	...
	*/
	...
	{
		global $db, $template, $user, $config, $request, $src_container, $src_dispatcher;
		...
		if (confirm_box(true))
		{
			...
			foreach ($post_info as $post_id => $post_data)
			{
				...
				$post_url = append_sid("{$src_root_path}viewtopic.$phpEx", "f={$post_data['forum_id']}&amp;t={$post_data['topic_id']}&amp;p={$post_data['post_id']}") . '#p' . $post_data['post_id'];
			...
			{
				$src_content_visibility->set_post_visibility(ITEM_APPROVED, $topic_data['posts'], $topic_id, $topic_data['forum_id'], $user->data['user_id'], time(), '', isset($topic_data['first_post']), isset($topic_data['last_post']));
			}
			...
			if ($action == 'approve')
			{
				...
				foreach ($post_info as $post_id => $post_data)
				{
					...
					if (!$post_data['topic_posts_approved'])
					{
						...
						{
							$src_notifications->add_notifications(array('notification.type.topic'), $post_data);
						}
						if ($post_data['post_visibility'] != ITEM_APPROVED)
						{
							$num_topics++;
					...
					$src_notifications->add_notifications(array('notification.type.quote'), $post_data);
					...
					if ($notify_poster)
					{
						...
						if (!$post_data['topic_posts_approved'])
						{
							$src_notifications->add_notifications('notification.type.approve_topic', $post_data);
						}
						else
						{
							$src_notifications->add_notifications('notification.type.approve_post', $post_data);
						}
					}
				}
			}
			...
			extract($src_dispatcher->trigger_event('core.approve_posts_after', compact($vars)));
