...
function mcp_forum_view($id, $mode, $action, $forum_info)
{
	...
	if ($merge_select)
	{
		...
		$request->overwrite('sk', null, \src\request\request_interface::POST);
		$request->overwrite('sd', null, \src\request\request_interface::POST);
	}
	...
	$url = append_sid("{$src_root_path}mcp.$phpEx?$url_extra");
	...
	src_mcp_sorting('viewforum', $sort_days, $sort_key, $sort_dir, $sort_by_sql, $sort_order_sql, $total, $forum_id);
	...
	$template->assign_vars(array(
		...
		'U_VIEW_FORUM_LOGS'		=> ($auth->acl_gets('a_', 'm_', $forum_id) && $module->loaded('logs')) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=logs&amp;mode=forum_logs&amp;f=' . $forum_id) : '',
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.mcp_view_forum_modify_sql', compact($vars)));
	...
	foreach ($topic_list as $topic_id)
	{
		...
		$topic_row = array(
			...
			'REPLIES'			=> $src_content_visibility->get_count('topic_posts', $row, $row['forum_id']) - 1,
		...
		if ($row['topic_status'] == ITEM_MOVED)
		{
			$topic_row = array_merge($topic_row, array(
				'U_VIEW_TOPIC'		=> append_sid("{$src_root_path}viewtopic.$phpEx", "t={$row['topic_moved_id']}"),
				'U_DELETE_TOPIC'	=> ($auth->acl_get('m_delete', $forum_id)) ? append_sid("{$src_root_path}mcp.$phpEx", "i=$id&amp;f=$forum_id&amp;topic_id_list[]={$row['topic_id']}&amp;mode=forum_view&amp;action=delete_topic") : '',
				'S_MOVED_TOPIC'		=> true,
		...
		else
		{
			...
			$topic_row = array_merge($topic_row, array(
				'U_VIEW_TOPIC'		=> append_sid("{$src_root_path}mcp.$phpEx", "i=$id&amp;f=$forum_id&amp;t={$row['topic_id']}&amp;mode=topic_view"),

				'S_SELECT_TOPIC'	=> ($merge_select && !in_array($row['topic_id'], $source_topic_ids)) ? true : false,
				'U_SELECT_TOPIC'	=> $u_select_topic,
				'U_MCP_QUEUE'		=> $u_mcp_queue,
				'U_MCP_REPORT'		=> ($auth->acl_get('m_report', $forum_id)) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=main&amp;mode=topic_view&amp;t=' . $row['topic_id'] . '&amp;action=reports') : '',
		...
		*/
		...
		extract($src_dispatcher->trigger_event('core.mcp_view_forum_modify_topicrow', compact($vars)));
...
{
	global $auth, $db, $template, $phpEx, $user, $src_root_path;
...
function merge_topics($forum_id, $topic_ids, $to_topic_id)
{
	...
	if (confirm_box(true))
	{
		...
		src_update_rows_avoiding_duplicates_notify_status($db, TOPICS_WATCH_TABLE, 'topic_id', $topic_ids, $to_topic_id);

		// Update the bookmarks table.
		src_update_rows_avoiding_duplicates($db, BOOKMARKS_TABLE, 'topic_id', $topic_ids, $to_topic_id);
