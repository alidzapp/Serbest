...
class mcp_warn
{
	...
	*/
	function mcp_warn_front_view()
	{
		...
		$template->assign_vars(array(
			'U_FIND_USERNAME'	=> append_sid("{$src_root_path}memberlist.$phpEx", 'mode=searchuser&amp;form=mcp&amp;field=username&amp;select_single=true'),
			'U_POST_ACTION'		=> append_sid("{$src_root_path}mcp.$phpEx", 'i=warn&amp;mode=warn_user'),
		));
		...
		foreach ($highest as $row)
		{
			$template->assign_block_vars('highest', array(
				'U_NOTES'		=> append_sid("{$src_root_path}mcp.$phpEx", 'i=notes&amp;mode=user_notes&amp;u=' . $row['user_id']),
	...
	*/
	...
	{
		global $phpEx, $src_root_path, $config, $src_container;
		global $template, $db, $user, $auth;
		...
		$base_url = append_sid("{$src_root_path}mcp.$phpEx", "i=warn&amp;mode=list&amp;st=$st&amp;sk=$sk&amp;sd=$sd");
	...
	*/
	function mcp_warn_post_view($action)
	{
		...
		if ($warning && $action == 'add_warning')
		{
			if (check_form_key('mcp_warn'))
			{
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.mcp_warn_post_before', compact($vars)));
				...
				if ($s_mcp_warn_post)
				{
					...
					*/
					...
					extract($src_dispatcher->trigger_event('core.mcp_warn_post_after', compact($vars)));
				}
			}
			else
			{
		...
		$template->assign_vars(array(
			...
			'L_WARNING_POST_DEFAULT'	=> sprintf($user->lang['WARNING_POST_DEFAULT'], generate_srcrd_url() . "/viewtopic.$phpEx?f=$forum_id&amp;p=$post_id#p$post_id"),
...
function add_warning($user_row, $warning, $send_pm = true, $post_id = 0)
{
	...
	if ($send_pm)
	{
		...
		$user_row['user_lang'] = (file_exists($src_root_path . 'language/' . $user_row['user_lang'] . "/mcp.$phpEx")) ? $user_row['user_lang'] : $config['default_lang'];
		include($src_root_path . 'language/' . basename($user_row['user_lang']) . "/mcp.$phpEx");
