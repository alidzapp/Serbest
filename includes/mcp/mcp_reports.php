...
class mcp_reports
{
	...
	function main($id, $mode)
	{
		...
		global $config, $src_root_path, $phpEx, $action, $src_container, $src_dispatcher;
		...
		switch ($mode)
		{
			case 'report_details':
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.mcp_reports_report_details_query_before', compact($vars)));
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.mcp_reports_report_details_query_after', compact($vars)));
				...
				$src_notifications->mark_notifications_read('notification.type.report_post', $post_id, $user->data['user_id']);
				...
				$post_info = src_get_post_data(array($post_id), 'm_report', true);
				...
				$template->assign_vars(array(
					'S_MCP_REPORT'			=> true,
					'S_CLOSE_ACTION'		=> append_sid("{$src_root_path}mcp.$phpEx", 'i=reports&amp;mode=report_details&amp;f=' . $post_info['forum_id'] . '&amp;p=' . $post_id),
					...
					'U_MCP_REPORTER_NOTES'		=> append_sid("{$src_root_path}mcp.$phpEx", 'i=notes&amp;mode=user_notes&amp;u=' . $report['user_id']),
					'U_MCP_USER_NOTES'			=> append_sid("{$src_root_path}mcp.$phpEx", 'i=notes&amp;mode=user_notes&amp;u=' . $post_info['user_id']),
					'U_MCP_WARN_REPORTER'		=> ($auth->acl_get('m_warn')) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=warn&amp;mode=warn_user&amp;u=' . $report['user_id']) : '',
					'U_MCP_WARN_USER'			=> ($auth->acl_get('m_warn')) ? append_sid("{$src_root_path}mcp.$phpEx", 'i=warn&amp;mode=warn_user&amp;u=' . $post_info['user_id']) : '',
					'U_VIEW_FORUM'				=> append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $post_info['forum_id']),
					'U_VIEW_POST'				=> append_sid("{$src_root_path}viewtopic.$phpEx", 'f=' . $post_info['forum_id'] . '&amp;p=' . $post_info['post_id'] . '#p' . $post_info['post_id']),
					'U_VIEW_TOPIC'				=> append_sid("{$src_root_path}viewtopic.$phpEx", 'f=' . $post_info['forum_id'] . '&amp;t=' . $post_info['topic_id']),

					'EDIT_IMG'				=> $user->img('icon_post_edit', $user->lang['EDIT_POST']),
					'MINI_POST_IMG'			=> ($post_unread) ? $user->img('icon_post_target_unread', 'UNREAD_POST') : $user->img('icon_post_target', 'POST'),
					'UNAPPROVED_IMG'		=> $user->img('icon_topic_unapproved', $user->lang['POST_UNAPPROVED']),

					'RETURN_REPORTS'			=> sprintf($user->lang['RETURN_REPORTS'], '<a href="' . append_sid("{$src_root_path}mcp.$phpEx", 'i=reports' . (($post_info['post_reported']) ? '&amp;mode=reports' : '&amp;mode=reports_closed') . '&amp;start=' . $start . '&amp;f=' . $post_info['forum_id']) . '">', '</a>'),
			...
			case 'reports_closed':
				...
				{
					$forum_info = src_get_forum_data(array($forum_id), 'm_report');
				...
				*/
				...
				extract($src_dispatcher->trigger_event('core.mcp_reports_get_reports_query_before', compact($vars)));
				...
				if (sizeof($report_ids))
				{
					...
					while ($row = $db->sql_fetchrow($result))
					{
						$template->assign_block_vars('postrow', array(
							...
							'U_VIEWPOST'				=> append_sid("{$src_root_path}viewtopic.$phpEx", 'f=' . $row['forum_id'] . '&amp;p=' . $row['post_id']) . '#p' . $row['post_id'],
							'U_VIEW_DETAILS'			=> append_sid("{$src_root_path}mcp.$phpEx", "i=reports&amp;start=$start&amp;mode=report_details&amp;f={$row['forum_id']}&amp;r={$row['report_id']}"),
...
function close_report($report_id_list, $mode, $action, $pm = false)
{
	...
	global $phpEx, $src_root_path, $src_container;
	...
	{
		if (!src_check_ids($post_id_list, POSTS_TABLE, 'post_id', array('m_report')))
		{
			trigger_error('NOT_AUTHORISED');
		}
	}
	...
	if (confirm_box(true))
	{
		$post_info = ($pm) ? src_get_pm_data($post_id_list) : src_get_post_data($post_id_list, 'm_report');
		...
		foreach ($reports as $report)
		{
			if ($pm)
			{
				...
				$src_notifications->delete_notifications('notification.type.report_pm', $report['pm_id']);
			}
			else
			{
				...
				$src_notifications->delete_notifications('notification.type.report_post', $report['post_id']);
			}
		}
		...
		if (sizeof($notify_reporters))
		{
			foreach ($notify_reporters as $report_id => $reporter)
			{
				...
				if ($pm)
				{
					$src_notifications->add_notifications('notification.type.report_pm_closed', array_merge($post_info[$post_id], array(
				...
				{
					$src_notifications->add_notifications('notification.type.report_post_closed', array_merge($post_info[$post_id], array(
						'reporter'			=> $reporter['user_id'],
						'closer_id'			=> $user->data['user_id'],
					)));
				}
	...
	else
	{
		...
		if (!$pm)
		{
			...
			{
				$return_forum = sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . current($forum_ids)) . '">', '</a>') . '<br /><br />';
			}

			if (sizeof($topic_ids) === 1)
			{
				$return_topic = sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("{$src_root_path}viewtopic.$phpEx", 't=' . current($topic_ids) . '&amp;f=' . current($forum_ids)) . '">', '</a>') . '<br /><br />';
			}
		}
