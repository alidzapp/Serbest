...
function src_insert_forums()
{
	...
	while ($row = $src_db->sql_fetchrow($result))
	{
		$sql_ary = array(
			...
			'forum_name'	=> ($row['cat_title']) ? htmlspecialchars(src_set_default_encoding($row['cat_title']), ENT_COMPAT, 'UTF-8') : $user->lang['CATEGORY'],
			'parent_id'		=> 0,
			'forum_parents'	=> '',
			'forum_desc'	=> '',
	...
	while ($row = $src_db->sql_fetchrow($result))
	{
		...
		$sql_ary = array(
			...
			'forum_name'		=> htmlspecialchars(src_set_default_encoding($row['forum_name']), ENT_COMPAT, 'UTF-8'),
			'parent_id'			=> (int) $cats_added[$row['cat_id']],
			'forum_parents'		=> '',
			'forum_desc'		=> htmlspecialchars(src_set_default_encoding($row['forum_desc']), ENT_COMPAT, 'UTF-8'),
...
* @param bool $grab_user_lang if set to true the function tries to use $convert_row['user_lang'] (and falls back to $convert_row['poster_id']) instead of the srcrds default language
*/
function src_set_encoding($text, $grab_user_lang = true)
{
...
/**
* Same as src_set_encoding, but forcing srcrds default language
*/
function src_set_default_encoding($text)
{
	return src_set_encoding($text, false);
}

/**
* Convert Birthday from Birthday MOD to src Format
*/
function src_get_birthday($birthday = '')
{
...
* Everyone's id will be one higher to allow the guest/anonymous user to have a positive id as well
*/
function src_topic_replies_to_posts($num_replies)
{
...
function src_copy_table_fields()
{
}
*/
...
*/
function src_convert_authentication($mode)
{
	...
	if ($mode == 'start')
	{
		...
		while ($row = $src_db->sql_fetchrow($result))
		{
			$user_id = (int) src_user_id($row['user_id']);
			// Set founder admin...
	...
	// Grab user auth information from 2.0.x srcrd
							...
							case AUTH_ACL:
								...
								if (isset($user_access[$forum['forum_id']]))
								{
									foreach ($user_access[$forum['forum_id']] as $index => $access)
									{
										...
										if (isset($access[$old_auth_key]) && $access[$old_auth_key] == 1)
										{
											mass_auth('user', $new_forum_id, (int) src_user_id($access['user_id']), $new_acl, ACL_YES);
										}
									}
								}
							break;
	...
	else if ($mode == 'second')
	{
		...
		while ($row = $src_db->sql_fetchrow($result))
		{
			mass_auth('user_role', 0, (int) src_user_id($row['user_id']), 'USER_NOAVATAR');
		}
		$src_db->sql_freeresult($result);
		...
		while ($row = $src_db->sql_fetchrow($result))
		{
			mass_auth('user_role', 0, (int) src_user_id($row['user_id']), 'USER_NOPM');
		}
		$src_db->sql_freeresult($result);
	}
	else if ($mode == 'third')
	{
		...
		foreach ($user_access as $forum_id => $access_map)
		{
			...
			foreach ($access_map as $access)
			{
				...
				{
					mass_auth('user_role', $forum_id, (int) src_user_id($access['user_id']), 'MOD_STANDARD');
					mass_auth('user_role', $forum_id, (int) src_user_id($access['user_id']), 'FORUM_STANDARD');
					foreach ($mod_post_map as $old => $new)
					{
						...
						{
							mass_auth('user', $forum_id, (int) src_user_id($access['user_id']), $new, ACL_YES);
						}
					}
				}
			}
...
*/
function src_convert_group_name($group_name)
{
	...
	{
		return 'src2 - ' . $group_name;
	}

	return src_set_default_encoding($group_name);
}

/**
* Convert the group type constants
*/
function src_convert_group_type($group_type)
{
	switch ($group_type)
	{
		case 0:
	...
	// Never return GROUP_SPECIAL here, because only src3's default groups are allowed to have this type set.
	return GROUP_HIDDEN;
}
...
*/
function src_convert_topic_type($topic_type)
{
	switch ($topic_type)
	{
		case 0:
...
function src_replace_size($matches)
{
...
*/
function src_prepare_message($message)
{
	global $src_root_path, $phpEx, $db, $convert, $user, $config, $cache, $convert_row, $message_parser;
	...
	// Decode src 2.0.x Message
	if (isset($convert->row['old_bbcode_uid']) && $convert->row['old_bbcode_uid'] != '')
	{
		// Adjust size...
		if (strpos($message, '[size=') !== false)
		{
			$message = preg_replace_callback('/\[size=(\d*):(' . $convert->row['old_bbcode_uid'] . ')\]/', 'src_replace_size', $message);
		}
	...
	// make the post UTF-8
	$message = src_set_encoding($message);
...
*/
function src_post_edit_user()
{
	global $convert_row, $config;

	if (isset($convert_row['post_edit_count']))
	{
		return src_user_id($convert_row['poster_id']);
	}
...
* This is only used if the Attachment MOD was installed
*/
function src_attachment_category($cat_id)
{
	switch ($cat_id)
	{
		case 1:
...
* This is only used if the Attachment MOD was installed
*/
function src_attachment_extension_group_name()
{
	global $db, $src_root_path, $phpEx;
	...
	while ($lang_dir = $db->sql_fetchfield('lang_dir'))
	{
		...
		$lang_file = $src_root_path . 'language/' . $lang_dir . '/acp/attachments.' . $phpEx;
...
*/
function src_attachment_forum_perms($forum_permissions)
{
...
*/
function  src_smilie_html_decode($code)
{
...
*/
function src_import_avatar($user_avatar)
{
	global $convert_row;
...
*/
function src_get_avatar_height($user_avatar)
{
	global $convert_row;
	...
	return get_avatar_height($user_avatar, 'src_avatar_type', $convert_row['user_avatar_type']);
}
...
* Find out about the avatar's dimensions
*/
function src_get_avatar_width($user_avatar)
{
	global $convert_row;
	...
	return get_avatar_width($user_avatar, 'src_avatar_type', $convert_row['user_avatar_type']);
}
...
*/
function src_privmsgs_to_userid($to_userid)
{
	global $config;

	return 'u_' . src_user_id($to_userid);
}

/**
* Calculate whether a private message was unread using the bitfield
*/
function src_unread_pm($pm_type)
{
...
*/
function src_get_savebox_id($user_id)
{
	global $db;

	$user_id = src_user_id($user_id);
...
* This is only used if the Attachment MOD was installed
*/
function src_import_attach_config()
{
...
*/
function src_disallowed_username($username)
{
	// Replace * with %
	$username = src_set_default_encoding(str_replace('*', '%', $username));
	return utf8_htmlspecialchars($username);
}

/**
* Checks whether there are any usernames on the old srcrd that would map to the same
* username_clean on src3. Prints out a list if any exist and exits.
*/
function src_create_userconv_table()
{
...
function src_check_username_collisions()
{
	global $db, $src_db, $convert, $table_prefix, $user, $lang;
	...
	if (sizeof($colliding_names))
	{
		...
		foreach ($colliding_users as $username_clean => $users)
		{
			...
			{
				$list .= sprintf($user->lang['COLLIDING_USER'], $row['user_id'], src_set_default_encoding($row['username']), $row['user_posts']) . "<br />\n";
			}
		}
...
function src_convert_timezone($timezone)
{
	global $config, $db, $src_root_path, $phpEx, $table_prefix;
	$timezone_migration = new \src\db\migration\data\v310\timezone($config, $db, new \src\db\tools($db), $src_root_path, $phpEx, $table_prefix);
	return $timezone_migration->convert_src30_timezone($timezone, 0);
}

function src_add_notification_options($user_notify_pm)
{
	global $convert_row, $db;

	$user_id = src_user_id($convert_row['user_id']);
...
function src_convert_password_hash($hash)
{
	global $src_container;

	$manager = $src_container->get('passwords.manager');
