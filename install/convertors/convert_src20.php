...
* to the config directive in src3. It can either be a direct mapping or use a function.
...
$config_schema = array(
	...
	'settings'		=>	array(
		...
		'srcrd_disable'			=> 'srcrd_disable',
		'sitename'				=> 'src_set_encoding(sitename)',
		'site_desc'				=> 'src_set_encoding(site_desc)',
		'session_length'		=> 'session_length',
		'srcrd_email_sig'		=> 'src_set_encoding(srcrd_email_sig)',
		'posts_per_page'		=> 'posts_per_page',
		'topics_per_page'		=> 'topics_per_page',
		'enable_confirm'		=> 'enable_confirm',
		'srcrd_email_form'		=> 'srcrd_email_form',
		...
		'default_dateformat'	=> 'src_set_encoding(default_dateformat)',
		'srcrd_timezone'		=> 'src_convert_timezone(srcrd_timezone)',
		...
		'srcrd_startdate'		=> 'srcrd_startdate',
	)
);
...
*/
	...
	$convertor = array(
		...
		'upload_path'			=> (defined('MOD_ATTACHMENT')) ? src_get_files_dir() . '/' : '',
		'thumbnails'			=> (defined('MOD_ATTACHMENT')) ? array('thumbs/', 't_') : '',
		'ranks_path'			=> false, // src 2.0.x had no config value for a ranks path
...
//	with this you are able to import all attachment files on the fly. For large srcrds this is not an option, therefore commented out by default.
//	Instead every file gets copied while processing the corresponding attachment entry.
//		if (defined("MOD_ATTACHMENT")) { import_attachment_files(); src_copy_thumbnails(); }

		// src2 allowed some similar usernames to coexist which would have the same
		// username_clean in src3 which is not possible, so we'll give the admin a list
		...
		', '
			src_convert_authentication(\'first\');
		', '
			src_convert_authentication(\'second\');
		', '
			src_convert_authentication(\'third\');
		'),
		...
		'schema' => array(
			array(
				...
				array('username_clean',		'users.username',	array('function1' => 'src_set_encoding', 'function2' => 'utf8_clean_string')),
			),
			...
			array(
				...
				array('in_message',				0,										''),
				array('is_orphan',				0,										''),
				array('poster_id',				'attachments.user_id_1 AS poster_id',	'src_user_id'),
				array('physical_filename',		'attachments_desc.physical_filename',	'import_attachment'),
				array('real_filename',			'attachments_desc.real_filename',		'src_set_encoding'),
				array('download_count',			'attachments_desc.download_count',		''),
				array('attach_comment',			'attachments_desc.comment',				array('function1' => 'src_set_encoding', 'function2' => 'utf8_htmlspecialchars')),
			...
			array(
				...
				array('group_name',				'extension_groups.group_name',			array('function1' => 'src_set_encoding', 'function2' => 'utf8_htmlspecialchars')),
				array('cat_id',					'extension_groups.cat_id',				'src_attachment_category'),
				array('allow_group',			'extension_groups.allow_group',			''),
				array('download_mode',			1,										''),
				array('upload_icon',			'',										''),
				array('max_filesize',			'extension_groups.max_filesize',		''),
				array('allowed_forums',			'extension_groups.forum_permissions',	'src_attachment_forum_perms'),
				array('allow_in_pm',			1,										''),
			),

			array(
				'target'		=> BANLIST_TABLE,
				'execute_first'	=> 'src_check_username_collisions();',
				'query_first'	=> array('target', $convert->truncate_statement . BANLIST_TABLE),

				array('ban_ip',					'banlist.ban_ip',					'decode_ban_ip'),
				array('ban_userid',				'banlist.ban_userid',				'src_user_id'),
			...
			array(
				...
				array('disallow_username',		'disallow.disallow_username',				'src_disallowed_username'),
			),
			...
			array(
				...
				array('rank_title',					'ranks.rank_title',				array('function1' => 'src_set_default_encoding', 'function2' => 'utf8_htmlspecialchars')),
			...
			array(
				...
				array('icon_id',				0,									''),
				array('topic_poster',			'topics.topic_poster AS poster_id',	'src_user_id'),
				array('topic_attachment',		((defined('MOD_ATTACHMENT')) ? 'topics.topic_attachment' : 0), ''),
				array('topic_title',			'topics.topic_title',				'src_set_encoding'),
				array('topic_time',				'topics.topic_time',				''),
				array('topic_views',			'topics.topic_views',				''),
				array('topic_posts_approved',	'topics.topic_replies',				'src_topic_replies_to_posts'),
				...
				array('topic_type',				'topics.topic_type',				'src_convert_topic_type'),
				...
				array('poll_title',				'vote_desc.vote_text',				array('function1' => 'null_to_str', 'function2' => 'src_set_encoding', 'function3' => 'htmlspecialchars_decode', 'function4' => 'utf8_htmlspecialchars')),
			...
			array(
				...
				array('user_id',				'topics_watch.user_id',				'src_user_id'),
			...
			array(
				...
				array('code',					'smilies.code',						array('function1' => 'src_smilie_html_decode', 'function2' => 'src_set_encoding', 'function3' => 'utf8_htmlspecialchars')),
				array('emotion',				'smilies.emoticon',					'src_set_encoding'),
			...
			array(
				...
				array('',						'topics.topic_poster AS poster_id',	'src_user_id'),
				array('poll_option_text',		'vote_results.vote_option_text',	array('function1' => 'src_set_encoding', 'function2' => 'htmlspecialchars_decode', 'function3' => 'utf8_htmlspecialchars')),
			...
			array(
				...
				array('topic_id',				'vote_desc.topic_id',				''),
				array('vote_user_id',			'vote_voters.vote_user_id',			'src_user_id'),
			...
			array(
				...
				array('word',					'words.word',						'src_set_encoding'),
				array('replacement',			'words.replacement',				'src_set_encoding'),
			),
			...
			array(
				...
				array('poster_id',				'posts.poster_id',					'src_user_id'),
				array('icon_id',				0,									''),
				...
				array('post_username',			'posts.post_username',				'src_set_encoding'),
				array('post_subject',			'posts_text.post_subject',			'src_set_encoding'),
				...
				array('post_edit_user',			'',									'src_post_edit_user'),
				array('post_visibility',		ITEM_APPROVED,						''),

				array('bbcode_uid',				'posts.post_time',					'make_uid'),
				array('post_text',				'posts_text.post_text',				'src_prepare_message'),
...
/*				array('',						'',									'search_indexing'),
			...
			array(
				...
				array('root_level',				0,									''),
				array('author_id',				'privmsgs.privmsgs_from_userid AS poster_id',	'src_user_id'),
				array('icon_id',				0,									''),
				...
				array('message_subject',		'privmsgs.privmsgs_subject',		'src_set_encoding'), // Already specialchared in 2.0.x
				...
				array('message_text',			'privmsgs_text.privmsgs_text',			'src_prepare_message'),
				array('',						'privmsgs_text.privmsgs_bbcode_uid AS old_bbcode_uid',			''),
				array('bbcode_bitfield',		'',										'get_bbcode_bitfield'),
				array('to_address',				'privmsgs.privmsgs_to_userid',			'src_privmsgs_to_userid'),
				array('bcc_address',			'',										''),
			...
			array(
				...
				array('user_id',				'users.user_id',						'src_user_id'),
			...
			array(
				...
				array('msg_id',					'privmsgs.privmsgs_id',					''),
				array('user_id',				'privmsgs.privmsgs_to_userid',			'src_user_id'),
				array('author_id',				'privmsgs.privmsgs_from_userid',		'src_user_id'),
				array('pm_deleted',				0,										''),
				array('pm_new',					'privmsgs.privmsgs_type',				'src_new_pm'),
				array('pm_unread',				'privmsgs.privmsgs_type',				'src_unread_pm'),
				array('pm_replied',				0,										''),
				array('pm_marked',				0,										''),
			...
			array(
				...
				array('msg_id',					'privmsgs.privmsgs_id',					''),
				array('user_id',				'privmsgs.privmsgs_from_userid',		'src_user_id'),
				array('author_id',				'privmsgs.privmsgs_from_userid',		'src_user_id'),
				array('pm_deleted',				0,										''),
				array('pm_new',					0,										''),
				array('pm_unread',				0,										''),
			...
			array(
				...
				array('pm_marked',				0,										''),
				array('pm_forwarded',			0,										''),
				array('folder_id',				'privmsgs.privmsgs_to_userid',			'src_get_savebox_id'),
			...
			array(
				...
				array('pm_marked',				0,										''),
				array('pm_forwarded',			0,										''),
				array('folder_id',				'privmsgs.privmsgs_from_userid',		'src_get_savebox_id'),
			...
			array(
				...
				array('group_type',				'groups.group_type',				'src_convert_group_type'),
				array('group_display',			0,									''),
				array('group_legend',			0,									''),
				array('group_name',				'groups.group_name',				'src_convert_group_name'), // src_set_encoding called in src_convert_group_name
				array('group_desc',				'groups.group_description',			'src_set_encoding'),
			...
			array(
				...
				array('group_id',		'groups.group_id',					''),
				array('user_id',		'groups.group_moderator',			'src_user_id'),
			...
			array(
				...
				array('user_id',		'user_group.user_id',				'src_user_id'),
			...
			array(
				...
				array('user_id',				'users.user_id',					'src_user_id'),
				array('',						'users.user_id AS poster_id',		'src_user_id'),
				array('user_type',				'users.user_active',				'set_user_type'),
				array('group_id',				'users.user_level',					'src_set_primary_group'),
				array('user_regdate',			'users.user_regdate',				''),
				array('username',				'users.username',					'src_set_default_encoding'), // recode to utf8 with default lang
				array('username_clean',			'users.username',					array('function1' => 'src_set_default_encoding', 'function2' => 'utf8_clean_string')),
				array('user_password',			'users.user_password',				'src_convert_password_hash'),
				...
				array('user_birthday',			((defined('MOD_BIRTHDAY')) ? 'users.user_birthday' : ''),	'src_get_birthday'),
				...
				array('user_timezone',			'users.user_timezone',				'src_convert_timezone'),
				array('user_dateformat',		'users.user_dateformat',			array('function1' => 'src_set_encoding', 'function2' => 'fill_dateformat')),
				array('user_inactive_reason',	'',									'src_inactive_reason'),
				array('user_inactive_time',		'',									'src_inactive_time'),

				array('user_jabber',			'',									''),
				array('user_rank',				'users.user_rank',					'intval'),
				array('user_permissions',		'',									''),

				array('user_avatar',			'users.user_avatar',				'src_import_avatar'),
				array('user_avatar_type',		'users.user_avatar_type',			'src_avatar_type'),
				array('user_avatar_width',		'users.user_avatar',				'src_get_avatar_width'),
				array('user_avatar_height',		'users.user_avatar',				'src_get_avatar_height'),
				...
				array('user_sig',					'users.user_sig',								'src_prepare_message'),
				...
				array('',						'users.user_notify_pm',				'src_add_notification_options'),
			...
			array(
				...
				array('user_id',				'users.user_id',					'src_user_id'),
				array('pf_src_occupation',	'users.user_occ',					array('function1' => 'src_set_encoding')),
				array('pf_src_interests',		'users.user_interests',				array('function1' => 'src_set_encoding')),
				array('pf_src_location',		'users.user_from',					array('function1' => 'src_set_encoding')),
				array('pf_src_icq',			'users.user_icq',					array('function1' => 'src_set_encoding')),
				array('pf_src_wlm',			'users.user_msnm',					array('function1' => 'src_set_encoding')),
				array('pf_src_yahoo',			'users.user_yim',					array('function1' => 'src_set_encoding')),
				array('pf_src_aol',			'users.user_aim',					array('function1' => 'src_set_encoding')),
				array('pf_src_website',		'users.user_website',				'validate_website'),
