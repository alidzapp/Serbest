...
class convert
{
	...
	// For big srcrds a value of 6000 seems to be optimal
	var $batch_size = 2000;
...
class install_convert extends module
{
	...
	*/
	...
	{
		global $lang, $template, $src_root_path, $phpEx, $cache, $config, $language, $table_prefix;
		global $convert, $request, $src_container, $src_config_php_file;
		...
		switch ($sub)
		{
			case 'intro':
				...
				require($src_root_path . 'includes/functions_convert.' . $phpEx);
	...
	/**
	* Check whether src is installed.
	* Assigns error template vars if not installed.
	*
	* @return bool Returns true if src is installed.
	*/
	public function check_src_installed()
	{
		if (src_check_installation_exists($this->src_root_path, $this->php_ext))
		{
			return true;
		}

		$this->page_title = 'srcRD_NOT_INSTALLED';
		$install_url = append_sid($this->src_root_path . 'install/index.' . $this->php_ext, 'mode=install&amp;language=' . $this->language);

		$this->template->assign_vars(array(
			'S_NOT_INSTALLED'		=> true,
			'BODY'					=> sprintf($this->lang['srcRD_NOT_INSTALLED_EXPLAIN'], $install_url),
		));
	...
	*/
	...
	{
		global $lang, $language, $template, $src_root_path, $phpEx;
	...
	*/
	...
	{
		global $lang, $language, $template, $db, $src_root_path, $phpEx, $config, $cache, $src_config_php_file;
		...
		// The test_file is a file that should be present in the location of the old srcrd.
		if (!isset($test_file))
		{
		...
		if ($submit)
		{
			...
			$src_dbms = $src_config_php_file->convert_30_dbms_to_31($src_dbms);
...
//					'S_HIDDEN'	=> $s_hidden_fields,
	...
	*/
	...
	{
		global $template, $user, $src_root_path, $phpEx, $db, $lang, $config, $cache, $auth;
		global $convert, $convert_row, $message_parser, $skip_rows, $language;
		global $request, $src_config_php_file, $src_dispatcher;
		...
		$user->session_begin();
		$user->page = $user->extract_current_page($src_root_path);
		...
		{
			$search_type = '\src\search\fulltext_native';
		...
		$error = false;
		$convert->fulltext_search = new $search_type($error, $src_root_path, $phpEx, $auth, $config, $db, $user, $src_dispatcher);
		...
		if (!$current_table && !$skip_rows)
		{
			if (!$request->variable('confirm', false))
			{
				...
				foreach ($local_paths as $folder => $local_path)
				{
					if (isset($convert->convertor[$folder]))
					{
						...
						if (!$local_path || !src_is_writable($src_root_path . $local_path))
						{
							if (!$local_path)
							{
								$bad_folders[] = sprintf($user->lang['CONFIG_src_EMPTY'], $folder);
							}
							else
							{
			...
			// Convert the config table and load the settings of the old srcrd
...
//				$percentage = ($skip_rows == 0) ? 0 : floor(100 / ($total_rows / $skip_rows));
	...
	*/
	...
	{
		global $template, $user, $db, $src_root_path, $phpEx, $config, $cache;
		global $convert;
	...
	*/
	...
	{
		global $db, $src_root_path, $phpEx, $convert, $config, $language, $user, $template;
		global $cache, $auth, $src_container, $src_log;
		...
		@unlink($src_root_path . 'cache/data_global.' . $phpEx);
	...
	*/
	...
	{
		global $template, $user, $src_db, $same_db, $db, $src_root_path, $phpEx, $config, $cache;
		global $convert;
	...
	*/
	function jump($jump, $last_statement)
	{
		...
		if ($jump == 1)
		{
			...
			$sql = 'SELECT MIN(user_regdate) AS srcrd_startdate
				FROM ' . USERS_TABLE;
			$result = $db->sql_query($sql);
			$row = $db->sql_fetchrow($result);
			$db->sql_freeresult($result);

			if (!isset($config['srcrd_startdate']) || ($row['srcrd_startdate'] < $config['srcrd_startdate'] && $row['srcrd_startdate'] > 0))
			{
				set_config('srcrd_startdate', $row['srcrd_startdate']);
				$db->sql_query('UPDATE ' . USERS_TABLE . ' SET user_regdate = ' . $row['srcrd_startdate'] . ' WHERE user_id = ' . ANONYMOUS);
			}
	...
	*/
	...
	{
		global $template, $user, $src_root_path, $phpEx, $db, $lang, $config, $cache;
