...
if (!empty($setmodules))
{
	// If src is already installed we do not include this module
	if (src_check_installation_exists($src_root_path, $phpEx) && !file_exists($src_root_path . 'cache/install_lock'))
	{
		return;
	}
...
class install_install extends module
{
	...
	/**
	* Checks that the server we are installing on meets the requirements for running src
	*/
	function check_server_requirements($mode, $sub)
	{
		global $lang, $template, $src_root_path, $phpEx, $language;
		...
		foreach ($directories as $dir)
		{
			...
			if (!file_exists($src_root_path . $dir))
			{
				@mkdir($src_root_path . $dir, 0777);
				src_chmod($src_root_path . $dir, CHMOD_READ | CHMOD_WRITE);
			}

			// Now really check
			if (file_exists($src_root_path . $dir) && is_dir($src_root_path . $dir))
			{
				src_chmod($src_root_path . $dir, CHMOD_READ | CHMOD_WRITE);
				$exists = true;
			}
			...
			$fp = @fopen($src_root_path . $dir . 'test_lock', 'wb');
			if ($fp !== false)
			{
				$write = true;
			}
			...
			@unlink($src_root_path . $dir . 'test_lock');
		...
		foreach ($directories as $dir)
		{
			...
			if (file_exists($src_root_path . $dir))
			{
				if (!src_is_writable($src_root_path . $dir))
				{
					$write = false;
				}
			}
	...
	*/
	function obtain_database_settings($mode, $sub)
	{
		...
		if (!$connect_test)
		{
			...
			$data['table_prefix'] = (!empty($data['table_prefix']) ? $data['table_prefix'] : 'opsrc_');
	...
	*/
	function obtain_admin_settings($mode, $sub)
	{
		...
		if (isset($_POST['check']))
		{
			...
			if ($data['admin_name'] == '' || $data['admin_pass1'] == '' || $data['admin_pass2'] == '' || $data['srcrd_email'] == '')
			{
			...
			if ($data['srcrd_email'] != '' && !preg_match('/^' . get_preg_expression('email') . '$/i', $data['srcrd_email']))
			{
	...
	*/
	...
	{
		global $lang, $template, $src_root_path, $phpEx;
		...
		$fp = @fopen($src_root_path . 'cache/install_lock', 'wb');
		if ($fp === false)
		{
		...
		@chmod($src_root_path . 'cache/install_lock', 0777);

		// Time to convert the data provided into a config file
		$config_data = src_create_config_file_data($data, $available_dbms[$data['dbms']]['DRIVER']);

		// Attempt to write out the config file directly. If it works, this is the easiest way to do it ...
		if ((file_exists($src_root_path . 'config.' . $phpEx) && src_is_writable($src_root_path . 'config.' . $phpEx)) || src_is_writable($src_root_path))
		{
			// Assume it will work ... if nothing goes wrong below
			$written = true;

			if (!($fp = @fopen($src_root_path . 'config.' . $phpEx, 'w')))
			{
			...
			if ($written)
			{
				...
				src_chmod($src_root_path . 'config.' . $phpEx, CHMOD_READ);
			}
		}
		...
		if (isset($_POST['dldone']))
		{
			...
			if (filesize($src_root_path . 'config.' . $phpEx) > 10)
			{
				$written = true;
			}
		}
	...
	*/
	...
	{
		global $db, $lang, $template, $src_root_path, $phpEx, $request;
		...
		if (file_exists($dbms_schema))
		{
			...
			$sql_query = preg_replace('#opsrc_#i', $data['table_prefix'], $sql_query);
			$sql_query = src_remove_comments($sql_query);
			$sql_query = split_sql_file($sql_query, $delimiter);

			foreach ($sql_query as $sql)
			{
				// Ignore errors when the functions or types already exist
				// to allow installing src twice in the same database with
				// a different prefix
				$db->sql_query($sql);
			}
			unset($sql_query);
		...
		{
			global $src_root_path, $phpEx, $table_prefix;
			$table_prefix = 'opsrc_';
			...
			if (!defined('CONFIG_TABLE'))
			{
				...
				include($src_root_path . 'includes/constants.' . $phpEx);
			}

			$finder = new \src\finder(new \src\filesystem(), $src_root_path, null, $phpEx);
			$classes = $finder->core_path('src/db/migration/data/')
				->get_classes();

			$sqlite_db = new \src\db\driver\sqlite();
			$schema_generator = new \src\db\migration\schema_generator($classes, new \src\config\config(array()), $sqlite_db, new \src\db\tools($sqlite_db, true), $src_root_path, $phpEx, $table_prefix);
		...
		switch ($data['dbms'])
		{
			...
			case 'mssql_odbc':
			case 'mssqlnative':
				$sql_query = preg_replace('#\# MSSQL IDENTITY (opsrc_[a-z_]+) (ON|OFF) \##s', 'SET IDENTITY_INSERT \1 \2;', $sql_query);
			break;
		...
		// Change prefix
		$sql_query = preg_replace('# src_([^\s]*) #i', ' ' . $data['table_prefix'] . '\1 ', $sql_query);
		...
		$user_ip = $request->server('REMOTE_ADDR') ? src_ip_normalise($request->server('REMOTE_ADDR')) : '';
		...
		$sql_ary = array(
			'INSERT INTO ' . $data['table_prefix'] . "config (config_name, config_value)
				VALUES ('srcrd_startdate', '$current_time')",
			...
			'UPDATE ' . $data['table_prefix'] . "config
				SET config_value = '" . $db->sql_escape($data['srcrd_email']) . "'
				WHERE config_name = 'srcrd_email'",

			'UPDATE ' . $data['table_prefix'] . "config
				SET config_value = '" . $db->sql_escape($data['srcrd_email']) . "'
				WHERE config_name = 'srcrd_contact'",
			...
			'UPDATE ' . $data['table_prefix'] . "users
				SET username = '" . $db->sql_escape($data['admin_name']) . "', user_password='" . $db->sql_escape(md5($data['admin_pass1'])) . "', user_ip = '" . $db->sql_escape($user_ip) . "', user_lang = '" . $db->sql_escape($data['default_lang']) . "', user_email='" . $db->sql_escape($data['srcrd_email']) . "', user_dateformat='" . $db->sql_escape($lang['default_dateformat']) . "', user_email_hash = " . $db->sql_escape(src_email_hash($data['srcrd_email'])) . ", username_clean = '" . $db->sql_escape(utf8_clean_string($data['admin_name'])) . "'
	...
	*/
	...
	{
		global $db, $lang, $src_root_path, $src_dispatcher, $phpEx, $config, $auth, $user;
		...
		include_once($src_root_path . 'includes/constants.' . $phpEx);
		include_once($src_root_path . 'src/search/fulltext_native.' . $phpEx);

		// We need to fill the config to let internal functions correctly work
		$config = new \src\config\db($db, new \src\cache\driver\null, CONFIG_TABLE);
		set_config(null, null, null, $config);
		set_config_count(null, null, null, $config);

		$error = false;
		$search = new \src\search\fulltext_native($error, $src_root_path, $phpEx, $auth, $config, $db, $user, $src_dispatcher);
	...
	*/
	...
	{
		global $db, $lang, $src_root_path, $phpEx, $src_extension_manager, $config, $src_container;

		// modules require an extension manager
		if (empty($src_extension_manager))
		{
			$src_extension_manager = $src_container->get('ext.manager');
		}

		include_once($src_root_path . 'includes/acp/acp_modules.' . $phpEx);
		...
		while ($row = $db->sql_fetchrow($result))
		{
			foreach ($installed_languages as $lang_id)
			{
				$insert_buffer->insert(array(
					...
					'lang_id'				=> $lang_id,
					'lang_name'				=> strtoupper(substr($row['field_name'], 6)),// Remove src_ from field name
					'lang_explain'			=> '',
					'lang_default_value'	=> '',
				));
			}
	...
	*/
	...
	{
		global $db, $lang, $src_root_path, $phpEx, $config;
	...
	/**
	* Sends an email to the srcrd administrator with their password and some useful links
	*/
	function email_admin($mode, $sub)
	{
		global $auth, $config, $db, $lang, $template, $user, $src_root_path, $src_admin_path, $phpEx;
		...
		if ($config['email_enable'])
		{
			...
			$messenger->to($data['srcrd_email'], $data['admin_name']);
		...
		$template->assign_vars(array(
			...
			'BODY'		=> sprintf($lang['INSTALL_CONGRATS_EXPLAIN'], $config['version'], append_sid($src_root_path . 'install/index.' . $phpEx, 'mode=convert&amp;language=' . $data['language']), '../docs/README.html'),
			'L_SUBMIT'	=> $lang['INSTALL_LOGIN'],
			'U_ACTION'	=> append_sid($src_admin_path . 'index.' . $phpEx, 'i=send_statistics&amp;mode=send_statistics'),
		));
	}
	...
	*/
	function get_submitted_data()
	{
		return array(
			...
			'srcrd_email'	=> strtolower(request_var('srcrd_email', '')),
	...
	*/
	...
	var $admin_config_options = array(
		...
		'srcrd_email'			=> array('lang' => 'CONTACT_EMAIL',				'type' => 'email:25:100', 'explain' => false),
	);
	...
	* 'Bunnybot [Bot]'				'powered by buncat'
	* 'Cosmix [Bot]'				'cfetch/'
