...
{
	die('You are running an unsupported PHP version. Please upgrade to PHP 5.3.3 or higher before trying to install src 3.1');
}

function src_require_updated($path, $optional = false)
{
	global $src_root_path, $table_prefix;

	$new_path = $src_root_path . 'install/update/new/' . $path;
	$old_path = $src_root_path . $path;
...
function src_include_updated($path, $optional = false)
{
...
src_require_updated('includes/startup.' . $phpEx);
...
src_require_updated('src/class_loader.' . $phpEx);
...
$src_container_builder->set_custom_parameters(array(
	'core.root_path'			=> $src_root_path,
	'core.adm_relative_path'	=> $src_adm_relative_path,
	'core.php_ext'				=> $phpEx,
	'core.table_prefix'			=> '',
	'cache.driver.class'		=> 'src\cache\driver\file',
));
...
if ($request->header('Accept-Language') && !$language)
{
	...
	foreach ($accept_lang_ary as $accept_lang)
	{
		...
		if (file_exists($src_root_path . 'language/' . $accept_lang) && is_dir($src_root_path . 'language/' . $accept_lang))
		{
...
if (!file_exists($src_root_path . 'language/' . $language) || !is_dir($src_root_path . 'language/' . $language))
{
	die('No language found!');
}

// And finally, load the relevant language files
$load_lang_files = array('common', 'acp/common', 'acp/srcrd', 'install', 'posting');
$new_path = $src_root_path . 'install/update/new/language/' . $language . '/';
$old_path = $src_root_path . 'language/' . $language . '/';

// NOTE: we can not use "src_include_updated" as the files uses vars which would be required
...
$user = new \src\user('\src\datetime');
$auth = new \src\auth\auth();
...
class module
{
	...
	*/
	...
	{
		global $db, $config, $phpEx, $src_root_path;
	...
	*/
	function page_header()
	{
		...
		global $template, $lang, $stage, $src_admin_path, $path;
		...
		$template->assign_vars(array(
			...
			'T_IMAGE_PATH'			=> htmlspecialchars($src_admin_path) . 'images/',
	...
	*/
	...
	{
		global $lang, $db, $template, $src_admin_path;
		...
		echo '<link href="' . htmlspecialchars($src_admin_path) . 'style/admin.css" rel="stylesheet" type="text/css" media="screen" />';
		echo '</head>';
