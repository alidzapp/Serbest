...
include($src_root_path . 'includes/functions_posting.' . $phpEx);
include($src_root_path . 'includes/functions_display.' . $phpEx);
include($src_root_path . 'includes/message_parser.' . $phpEx);
...
extract($src_dispatcher->trigger_event('core.modify_posting_parameters', compact($vars)));

// Was cancel pressed? If so then redirect to the appropriate page
if ($cancel || ($current_time - $lastclick < 2 && $submit))
{
	$f = ($forum_id) ? 'f=' . $forum_id . '&amp;' : '';
	$redirect = ($post_id) ? append_sid("{$src_root_path}viewtopic.$phpEx", $f . 'p=' . $post_id) . '#p' . $post_id : (($topic_id) ? append_sid("{$src_root_path}viewtopic.$phpEx", $f . 't=' . $topic_id) : (($forum_id) ? append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $forum_id) : append_sid("{$src_root_path}index.$phpEx")));
	redirect($redirect);
}
...
if ($mode == 'popup')
{
	src_upload_popup($post_data['forum_style']);
	return;
}
...
switch ($mode)
{
	...
	case 'soft_delete':
		if (!$is_authed && $user->data['is_registered'] && $src_content_visibility->can_soft_delete($forum_id, $post_data['poster_id'], $post_data['post_edit_locked']))
		{
...
extract($src_dispatcher->trigger_event('core.modify_posting_auth', compact($vars)));
...
if ($mode == 'edit' && !$auth->acl_get('m_edit', $forum_id))
{
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.posting_modify_cannot_edit_conditions', compact($vars)));
...
if ($mode == 'delete' || $mode == 'soft_delete')
{
	...
	src_handle_post_delete($forum_id, $topic_id, $post_id, $post_data, ($mode == 'soft_delete'), $delete_reason);
	return;
}
...
if ($mode == 'bump')
{
	...
	{
		$meta_url = src_bump_topic($forum_id, $topic_id, $post_data, $current_time);
...
$plupload = $src_container->get('plupload');
$mimetype_guesser = $src_container->get('mimetype.guesser');
...
if ($save && $user->data['is_registered'] && $auth->acl_get('u_savedrafts') && ($mode == 'reply' || $mode == 'post' || $mode == 'quote'))
{
	...
	if ($subject && $message)
	{
		if (confirm_box(true))
		{
			...
			$meta_info = ($mode == 'post') ? append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $forum_id) : append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id");

			meta_refresh(3, $meta_info);

			$message = $user->lang['DRAFT_SAVED'] . '<br /><br />';
			$message .= ($mode != 'post') ? sprintf($user->lang['RETURN_TOPIC'], '<a href="' . $meta_info . '">', '</a>') . '<br /><br />' : '';
			$message .= sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $forum_id) . '">', '</a>');
...
if ($submit || $preview || $refresh)
{
	...
	$post_data['post_edit_reason']	= ($request->variable('edit_reason', false, false, \src\request\request_interface::POST) && $mode == 'edit' && $auth->acl_get('m_edit', $forum_id)) ? utf8_normalize_nfc(request_var('edit_reason', '', true)) : '';
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.posting_modify_message_text', compact($vars)));
	...
	if ($submit && $mode == 'edit' && $post_data['post_visibility'] == ITEM_DELETED && !isset($_POST['soft_delete']) && $auth->acl_get('m_approve', $forum_id))
	{
		...
		$updated_post_data = $src_content_visibility->set_post_visibility(ITEM_APPROVED, $post_id, $post_data['topic_id'], $post_data['forum_id'], $user->data['user_id'], time(), '', $is_first_post, $is_last_post);
	...
	*/
	...
	extract($src_dispatcher->trigger_event('core.posting_modify_submission_errors', compact($vars)));
	...
	if (!sizeof($error) && $submit)
	{
		if ($submit)
		{
			...
			// post's poster, not the poster of the current post). See: src3-11769 for more information.
			...
			*/
			...
			extract($src_dispatcher->trigger_event('core.posting_modify_submit_post_before', compact($vars)));
			...
			*/
			...
			extract($src_dispatcher->trigger_event('core.posting_modify_submit_post_after', compact($vars)));
			...
			{
				$delete_reason = $request->variable('delete_reason', '', true);
				src_handle_post_delete($forum_id, $topic_id, $post_id, $post_data, !$request->is_set_post('delete_permanent'), $delete_reason);
				return;
			}
			...
			if ((!$auth->acl_get('f_noapprove', $data['forum_id']) && empty($data['force_approved_state'])) || (isset($data['force_approved_state']) && !$data['force_approved_state']))
			{
				...
				$message .= '<br /><br />' . sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$src_root_path}viewforum.$phpEx", 'f=' . $data['forum_id']) . '">', '</a>');
				trigger_error($message);
			}
...
$s_action = append_sid("{$src_root_path}posting.$phpEx", "mode=$mode&amp;f=$forum_id");
...
$page_data = array(
	...
	'BBCODE_STATUS'			=> ($bbcode_status) ? sprintf($user->lang['BBCODE_IS_ON'], '<a href="' . append_sid("{$src_root_path}faq.$phpEx", 'mode=bbcode') . '">', '</a>') : sprintf($user->lang['BBCODE_IS_OFF'], '<a href="' . append_sid("{$src_root_path}faq.$phpEx", 'mode=bbcode') . '">', '</a>'),
	...
	'U_VIEW_TOPIC'			=> ($mode != 'post') ? append_sid("{$src_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id") : '',
	'U_PROGRESS_BAR'		=> append_sid("{$src_root_path}posting.$phpEx", "f=$forum_id&amp;mode=popup"),
	'UA_PROGRESS_BAR'		=> addslashes(append_sid("{$src_root_path}posting.$phpEx", "f=$forum_id&amp;mode=popup")),
	...
	'S_SOFTDELETE_ALLOWED'		=> ($mode == 'edit' && $src_content_visibility->can_soft_delete($forum_id, $post_data['poster_id'], $lock_post_checked)) ? true : false,
...
extract($src_dispatcher->trigger_event('core.posting_modify_template_vars', compact($vars)));
