...
include($src_root_path . 'includes/functions_mcp.' . $phpEx);
...
if ($forum_action !== '' && $request->variable('sort', false, false, \src\request\request_interface::POST))
{
	$action = $forum_action;
}
...
extract($src_dispatcher->trigger_event('core.mcp_global_f_read_auth_after', compact($vars)));
	...
	switch ($action)
	{
		...
		default:
			...
			*/
			...
			extract($src_dispatcher->trigger_event('core.modify_quickmod_options', compact($vars)));
...
$template->assign_vars(array(
	'U_MCP'			=> append_sid("{$src_root_path}mcp.$phpEx", 'i=main'),
	'U_MCP_FORUM'	=> ($forum_id) ? append_sid("{$src_root_path}mcp.$phpEx", "i=main&amp;mode=forum_view&amp;f=$forum_id") : '',
	'U_MCP_TOPIC'	=> ($forum_id && $topic_id) ? append_sid("{$src_root_path}mcp.$phpEx", "i=main&amp;mode=topic_view&amp;t=$topic_id") : '',
	'U_MCP_POST'	=> ($forum_id && $topic_id && $post_id) ? append_sid("{$src_root_path}mcp.$phpEx", "i=main&amp;mode=post_details&amp;t=$topic_id&amp;p=$post_id") : '',
));
